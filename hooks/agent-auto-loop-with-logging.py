#!/usr/bin/env python3
"""
Agent Auto-Loop Hook System with Command Logging
...
from hooks.command_logger import CommandLogger, get_logger
...
class AgentAutoLoopHookWithLogging:
    """
    Enhanced Agent Auto-Loop Hook with comprehensive logging
    ...
    def __init__(self, config: AgentConfig):
        ...
        log_dir = f"/tmp/claude-code-logs/agent-{config.agent_id}"
    ...
    def fetch_available_tasks(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            with self.cmd_logger.log_command("GH_API", cmd, {"labels": self.config.labels}):
    ...
    def execute_task(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.cmd_logger.log_issue_processing_start(
            ...
            task_title,
            "TASK_EXECUTION",
            {"agent": self.config.agent_id, "priority_score": task.get('priority_score', 0)}
        )
        ...
        try:
            ...
            self.cmd_logger.log_issue_processing_complete(
                task_id,
                "TASK_EXECUTION",
                {"status": execution_result['status'], "duration": duration}
            )
    ...
    def assign_task_to_agent(self, task_id: str):
        ...
        try:
            ...
            # Log command execution
            with self.cmd_logger.log_command("GH_API", cmd, {"task_id": task_id, "assignee": self.config.agent_id}):
    ...
    def create_autonomous_session(self, task: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # Log file creation
        with self.cmd_logger.log_command("FILE_WRITE", str(session_file), {"task_id": task_id}):
...
- **Command Logging**: Enabled at /tmp/claude-code-logs/agent-{self.config.agent_id}
...
- All commands are logged to: /tmp/claude-code-logs/agent-{self.config.agent_id}
...
---
🤖 Autonomous Agent {self.config.agent_id} - Auto-Generated Task Execution (with Logging)
"""
        return instruction
        ...
        while True:
            ...
            if self.check_escalation_needed(task_id):
                ...
                self.cmd_logger.log_issue_processing_start(
                    ...
                    "",
                    "ESCALATION",
                    {"reason": "Escalation signal detected", "elapsed": elapsed}
                )
    ...
    def check_task_completion(self, task_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Log API call
            cmd_id = self.cmd_logger.log_command_start("GH_API", cmd, {"task_id": task_id})
    ...
    def check_escalation_needed(self, task_id: str) -> bool:
        ...
        try:
            ...
            # Log API call
            with self.cmd_logger.log_command("GH_API", cmd, {"task_id": task_id}):
                result = subprocess.run(cmd, shell=True, capture_output=True, text=True)
    ...
    def run_autonomous_loop(self, max_iterations: Optional[int] = None):
        ...
        self.cmd_logger.log_issue_processing_start(
            "SESSION",
            f"Agent {self.config.agent_id} Session",
            "SESSION_START",
        ...
        export_path = self.cmd_logger.export_logs_to_json(
            f"/tmp/agent-{self.config.agent_id}-session-export-{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Agent Auto-Loop Hook System with Logging')
    ...
    hook = AgentAutoLoopHookWithLogging(config)
