.PHONY: help dev test test-full lint typecheck clean setup-dev start-data stop-data status build security-scan pre-commit verify check-merge-ready check-core-tests check-phase-status agent-tasks agent-status agent-report setup-sdad
...
setup-sdad:
	...
	@ls -la docs/claude-code-cluster/*.md 2>/dev/null || echo "   ⚠️  プロンプトファイルが見つかりません"
	@echo ""
	@echo "✨ SDAD環境のセットアップが完了しました！"
	@echo ""
	@echo "📚 次のステップ:"
	@echo "   1. docs/claude-code-cluster/IMPLEMENTATION_GUIDE.md を参照"
