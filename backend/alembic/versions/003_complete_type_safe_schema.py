...
def upgrade() -> None:
    ...
    op.create_index('ix_role_permissions_role_perm', 'role_permissions', ['role_id', 'permission_id'], unique=True)
    ...
    op.create_index('ix_user_roles_valid_period', 'user_roles', ['valid_from', 'valid_to'])
    ...
    permissions_data = [
        ...
        ('roles.update_permissions', 'Update Role Permissions', 'Modify role permissions', 'roles'),

        # System permissions
        ('system.admin', 'System Administration', 'Full system administration', 'system'),
        ('system.config', 'System Configuration', 'Modify system configuration', 'system'),
    ...
    for code, name, description, category in permissions_data:
        ...
        op.execute(permissions_table.insert().values(
            code=code,
            name=name,
