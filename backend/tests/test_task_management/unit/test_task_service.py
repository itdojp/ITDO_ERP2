...
class TestTaskService:
    ...
    def test_list_tasks_sorting(self):
        ...
        for i in range(5):
            ...
            mock_task.description = f"Description for task {i + 1}"
            mock_task.status = "not_started"
            mock_task.priority = ["high", "high", "medium", "low", "low"][i]  # Sorted priority
            mock_task.project_id = 1
            mock_task.parent_task_id = None
            mock_task.due_date = None
            mock_task.estimated_hours = None
...
class TestTaskStatusManagement:
    ...
    def test_update_status_valid_transition(self):
        ...
        # Arrange
        task_id = 1
        status_update = TaskStatusUpdate(status=TaskStatus.IN_PROGRESS, comment="作業開始")
...
class TestTaskDueDateAndPriority:
    ...
    def test_get_overdue_tasks(self):
        ...
        mock_query.offset.return_value.limit.return_value.all.return_value = [mock_task1, mock_task2]
        self.db.query.return_value.options.return_value = mock_query
...
class TestTaskDependencies:
    ...
    def test_add_dependency_success(self):
        ...
        task_queries = {
            task_id: mock_task,
            depends_on_id: mock_depends_on
        }
        self.db.query.return_value.filter.return_value.first.side_effect = lambda: task_queries.get(
        ...
        with patch.object(self.service, '_task_to_response') as mock_response:
            mock_response.return_value = MagicMock()
            result = self.service.add_dependency(task_id, depends_on_id, self.test_user, self.db)
