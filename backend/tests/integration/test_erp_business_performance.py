...
class TestERPPerformanceRequirements:
    ...
    def test_product_search_performance(self, performance_test_db):
        ...
        test_scenarios = [
            ...
            {"params": {"min_price": 1000, "max_price": 1100, "limit": 50}},  # 価格範囲検索
        ]
        ...
        assert avg_search_time < PERFORMANCE_REQUIREMENTS["product_search_max_time"] * 0.5, \
            f"商品検索の平均時間が推奨値を超過: {avg_search_time:.3f}s"
        
        print(f"✅ 商品検索パフォーマンス - 最大: {max_search_time:.3f}s, 平均: {avg_search_time:.3f}s")
    ...
    def test_inventory_update_performance(self, performance_test_db):
        ...
        assert max_update_time < PERFORMANCE_REQUIREMENTS["inventory_update_max_time"], \
            f"在庫更新の最大時間が要件を超過: {max_update_time:.3f}s > {PERFORMANCE_REQUIREMENTS['inventory_update_max_time']}s"
        
        assert avg_update_time < PERFORMANCE_REQUIREMENTS["inventory_update_max_time"] * 0.7, \
            f"在庫更新の平均時間が推奨値を超過: {avg_update_time:.3f}s"
        
        print(f"✅ 在庫更新パフォーマンス - 最大: {max_update_time:.3f}s, 平均: {avg_update_time:.3f}s")
    ...
    def test_concurrent_user_performance(self, performance_test_db):
        ...
        def simulate_user_session(user_id: int) -> Dict[str, Any]:
            ...
            try:
                ...
                operations.append({"operation": "search", "time": search_time, "success": response.status_code == 200})
                
                # 2. 商品詳細取得
                start = time.time()
                response = client.get(f"/api/v1/products/{product_id}")
                detail_time = time.time() - start
                operations.append({"operation": "detail", "time": detail_time, "success": response.status_code == 200})
                
                # 3. 在庫レベル確認
                start = time.time()
                response = client.get(f"/api/v1/inventory/level/{product_id}")
                inventory_time = time.time() - start
                operations.append({"operation": "inventory", "time": inventory_time, "success": response.status_code == 200})
                ...
                if user_id % 5 == 0:  # 20%のユーザーが注文
                    ...
                    operations.append({"operation": "order", "time": order_time, "success": response.status_code == 201})
        ...
        avg_session_time = statistics.mean([r["session_time"] for r in successful_sessions]) if successful_sessions else 0
    ...
    def test_api_response_time_requirements(self, performance_test_db):
        ...
        api_endpoints = [
            ...
            {"method": "POST", "url": f"/api/v1/inventory/add/{product_id}", 
             "json": {"quantity": 10, "reason": "テスト"}},
            {"method": "GET", "url": f"/api/v1/inventory/level/{product_id}", "params": None},
        ...
        for endpoint_key, metrics in response_times.items():
            ...
            assert metrics["avg"] < PERFORMANCE_REQUIREMENTS["api_response_max_time"] * 0.7, \
                f"{endpoint_key} の平均応答時間が推奨値を超過: {metrics['avg']:.3f}s"
        
        print(f"✅ API応答時間パフォーマンス:")
        for endpoint_key, metrics in response_times.items():
            print(f"   - {endpoint_key}: 平均 {metrics['avg']:.3f}s, 最大 {metrics['max']:.3f}s")
...
class TestERPLoadTestingScenarios:
    ...
    def test_peak_hour_simulation(self, performance_test_db):
        ...
        with ThreadPoolExecutor(max_workers=peak_users) as executor:
            futures = [executor.submit(peak_hour_user_behavior, user_id)
        ...
        # パフォーマンス要件検証
        assert success_rate >= 90.0, f"ピーク時間の成功率が低すぎます: {success_rate:.1f}% < 90%"
        
        print(f"✅ ピーク時間シミュレーション結果:")
        ...
        for op_type, times in operation_stats.items():
            if times:
                avg_time = statistics.mean(times)
                max_time = max(times)
                print(f"   - {op_type}: {len(times)}回実行, 平均 {avg_time:.3f}s, 最大 {max_time:.3f}s")
