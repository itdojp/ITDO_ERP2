...
class TestERPCompleteBusinessScenario:
    ...
    def test_complete_business_lifecycle(self, full_integration_db):
        ...
        for product in created_products:
            ...
            response = client.post(
                f"/api/v1/inventory/add/{product_id}", json=initial_stock
            )
            assert response.status_code == 200
            ...
            print(
                f"✅ 在庫レベル確認: {product['code']} - 現在庫{stock_level['current_stock']}個"
            )

            # 2.3 在庫調整（一部商品のみ）
            if len(product["code"]) % 2 == 0:  # 偶数番目の商品のみ
                adjustment = {"quantity": 10, "reason": "在庫調整（追加）"}
        ...
        for i, customer in enumerate(created_customers):
            # 顧客ごとに異なる商品を注文
            order_products = (
                created_products[i : i + 2]
                if i + 2 <= len(created_products)
                else created_products[:2]
            )
            ...
            print(
                f"✅ 注文作成: 顧客{customer['code']} - {len(order_items)}品目, 金額¥{order.get('total_amount', total_expected)}"
            )
        ...
        for product in created_products:
            ...
            print(
                f"✅ 注文後在庫: {product['code']} - 現在庫{current_stock['current_stock']}個"
            )
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ 在庫履歴: {product['code']} - {len(transactions)}件のトランザクション"
                )
        ...
        if response.status_code == 200:
            ...
            print(
                f"✅ 在庫レポート: {len(inventory_report.get('products', []))}商品の在庫状況"
            )
        else:
    ...
    def test_business_performance_integration(self, full_integration_db):
        ...
        if bulk_data["products"]:
            ...
            assert adjustment_time < 2.0, (
                f"在庫調整時間が遅すぎます: {adjustment_time:.3f}s"
            )
        ...
        if bulk_data["products"] and len(bulk_data["products"]) >= 3:
            ...
            for i in range(5):
                order_data = {
                    ...
                    "items": [
                        {
                            "product_id": bulk_data["products"][
                                i % len(bulk_data["products"])
                            ]["id"],
                            "quantity": 2,
                            "unit_price": 1000.0,
                        }
            ...
            assert order_creation_time < 3.0, (
                f"注文作成時間が遅すぎます: {order_creation_time:.3f}s"
            )
...
class TestERPBusinessRuleCompliance:
    ...
    def test_japanese_business_rules(self, full_integration_db):
        ...
        order_data = {
            ...
            "items": [
                {"product_id": product["id"], "quantity": 1, "unit_price": 1000.0}
            ],
        }
        ...
        # 10個の在庫設定
        client.post(
            f"/api/v1/inventory/add/{test_product['id']}",
            json={"quantity": 10, "reason": "マイナス在庫テスト用"},
        )

        # 20個の減少調整（マイナス在庫になる）
        response = client.post(
            f"/api/v1/inventory/adjust/{test_product['id']}",
        ...
        if response.status_code == 400:
            ...
            if stock_response.status_code == 200:
                ...
                print(
                    f"⚠️ マイナス在庫調整が実行されました（現在庫: {stock_level['current_stock']}）"
                )
    ...
    def test_audit_trail_requirements(self, full_integration_db):
        ...
        # 商品更新
        update_data = {"name": "監査テスト商品（更新）", "price": 1200.0}
        ...
        order_data = {
            ...
            "items": [{"product_id": product_id, "quantity": 5, "unit_price": 1200.0}],
        }
        ...
        # 注文ステータス変更
        client.put(
            f"/api/v1/sales-orders/{order_id}/status", params={"status": "confirmed"}
        )
        client.put(
            f"/api/v1/sales-orders/{order_id}/status", params={"status": "shipped"}
        )
