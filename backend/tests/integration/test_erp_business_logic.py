...
class TestERPBusinessWorkflow:
    ...
    def test_inventory_management_workflow(self, test_db):
        ...
        # 2. 初期在庫追加
        initial_stock = {"quantity": 50, "reason": "初期在庫"}
        ...
        # 4. 在庫調整（追加）
        adjustment_data = {"quantity": 20, "reason": "追加入荷"}

        response = client.post(
            f"/api/v1/inventory/adjust/{product_id}", params=adjustment_data
        )
        assert response.status_code == 200
    ...
    def test_concurrent_inventory_operations(self, test_db):
        ...
        for i in range(5):
            response = client.post(
                f"/api/v1/inventory/reserve/{product_id}",
                params={"quantity": 10, "reference_id": f"ORDER-{i}"},
            )
            reservation_results.append(response.status_code)

        # 全ての予約が成功することを確認
        successful_reservations = sum(
            1 for status in reservation_results if status == 200
        )
    ...
    def test_order_cancellation_workflow(self, test_db):
        ...
        # 3. 注文キャンセル
        response = client.put(
            f"/api/v1/sales-orders/{order_id}/status", params={"status": "cancelled"}
        )
        assert response.status_code == 200
        ...
        assert (
            stock_after_cancel["available_stock"]
            >= stock_before_cancel["available_stock"]
        )
...
class TestERPPerformanceBusinessLogic:
    ...
    def test_bulk_product_operations(self, test_db):
        ...
        assert creation_time < 5.0, (
            f"商品作成に{creation_time:.2f}秒かかりました（5秒以内が要件）"
        )
        ...
        assert search_time < 1.0, (
            f"商品検索に{search_time:.2f}秒かかりました（1秒以内が要件）"
        )
    ...
    def test_inventory_transaction_performance(self, test_db):
        ...
        assert transaction_time < 10.0, (
            f"在庫取引に{transaction_time:.2f}秒かかりました（10秒以内が要件）"
        )
        assert successful_transactions >= 90, (
            f"取引成功率が低すぎます：{successful_transactions}/100"
        )
