...
class TestTaskServicePermissions:
    ...
    def test_task_assignment_department_boundaries(
        ...
        dept1 = (
            list(departments["root_department"].children)[0]
            if departments["root_department"].children
            else departments["root_department"]
        )
...
class TestTaskServiceWorkflows:
    ...
    def test_user_task_assignment_workflow(
        ...
        assignment_logs = (
            ...
            .filter_by(user_id=assigner.id, action="task.assign")
            .all()
        )
    ...
    def test_task_completion_workflow(
        ...
        completion_logs = (
            db_session.query(UserActivityLog)
            .filter_by(user_id=task_owner.id, action="task.complete")
            .all()
        )
    ...
    def test_task_escalation_workflow(
        ...
        escalation_logs = (
            db_session.query(UserActivityLog)
            .filter_by(user_id=regular_user.id, action="task.escalate")
            .all()
        )
        receive_logs = (
            db_session.query(UserActivityLog)
            .filter_by(user_id=dept_manager.id, action="task.receive_escalation")
            .all()
        )
