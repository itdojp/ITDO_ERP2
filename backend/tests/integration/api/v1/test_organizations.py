...
class TestOrganizationAPI(BaseAPITestCase[Organization, OrganizationCreate, OrganizationUpdate, OrganizationResponse], SearchTestMixin, HierarchyTestMixin):
    ...
    def test_tree_endpoint_success(
        ...
        tree_data = OrganizationFactory.create_subsidiary_tree(db_session, depth=2, children_per_level=2)
        
        response = client.get(
            f"{self.endpoint_prefix}/tree",
    ...
    def test_get_subsidiaries_endpoint(
        ...
        # Create subsidiaries
        subsidiaries = [
            OrganizationFactory.create_with_parent(db_session, parent.id, name=f"子会社{i}")
            for i in range(3)
        ]
    ...
    def test_get_subsidiaries_recursive(
        ...
        # Create organization hierarchy
        tree_data = OrganizationFactory.create_subsidiary_tree(db_session, depth=3, children_per_level=2)
        root = tree_data['root']
    ...
    def test_create_with_duplicate_code(
        ...
        org1 = OrganizationFactory.create(db_session, code="DUPLICATE")
    ...
    def test_delete_with_active_subsidiaries(
        ...
        parent = OrganizationFactory.create(db_session, name="親会社")
        subsidiary = OrganizationFactory.create_with_parent(db_session, parent.id, name="子会社")
    ...
    def test_list_with_filters(
        ...
        org_it = OrganizationFactory.create_with_specific_industry(db_session, "IT")
        org_finance = OrganizationFactory.create_with_specific_industry(db_session, "金融業")
        ...
        for i in range(5):  # Create 5 levels deep
            if parent_id:
                org = OrganizationFactory.create_with_parent(
                    db_session, 
                    parent_id, 
                    name=f"Level {i} Organization"
                )
            else:
                org = OrganizationFactory.create(db_session, name=f"Level {i} Organization")
