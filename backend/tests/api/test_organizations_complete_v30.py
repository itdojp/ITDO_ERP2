...
from app.models.organization_extended import Organization, Department
...
class TestOrganizationsCompleteV30:
    ...
    async def test_create_organization_success(self, mock_user):
        ...
        with patch('app.api.v1.organizations_complete_v30.require_admin', return_value=mock_user), \
             patch('app.api.v1.organizations_complete_v30.get_db') as mock_db, \
    ...
    async def test_create_organization_duplicate_code(self, mock_user):
        ...
        with patch('app.api.v1.organizations_complete_v30.require_admin', return_value=mock_user), \
            ...
            mock_crud_instance = Mock()
            mock_crud.return_value = mock_crud_instance
            mock_crud_instance.create.side_effect = DuplicateError("Organization code already exists")
    ...
    async def test_list_organizations_with_filters(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.get_current_user', return_value=mock_user), \
             patch('app.api.v1.organizations_complete_v30.get_db') as mock_db, \
             patch('app.crud.organization_extended_v30.OrganizationCRUD') as mock_crud:
    ...
    async def test_delete_organization_success(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.require_admin', return_value=mock_user), \
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.delete(f"/api/v1/organizations/{mock_organization.id}")
            
            assert response.status_code == 204

    @pytest.mark.asyncio
    async def test_delete_organization_with_children(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.require_admin', return_value=mock_user), \
            ...
            mock_crud_instance = Mock()
            mock_crud.return_value = mock_crud_instance
            mock_crud_instance.delete.side_effect = ValueError("Cannot delete organization with child organizations")
            
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.delete(f"/api/v1/organizations/{mock_organization.id}")
            
            assert response.status_code == 400
            assert "Cannot delete organization with child organizations" in response.json()["detail"]
    ...
    async def test_get_organization_hierarchy(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.get_current_user', return_value=mock_user), \
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(f"/api/v1/organizations/{mock_organization.id}/hierarchy")
    ...
    @pytest.mark.asyncio
    async def test_list_departments(self, mock_user, mock_organization, mock_department):
        ...
        with patch('app.api.v1.organizations_complete_v30.get_current_user', return_value=mock_user), \
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(f"/api/v1/organizations/{mock_organization.id}/departments")
    ...
    @pytest.mark.asyncio
    async def test_create_department_success(self, mock_user, mock_organization, mock_department):
    ...
    async def test_list_organization_addresses(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.get_current_user', return_value=mock_user), \
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(f"/api/v1/organizations/{mock_organization.id}/addresses")
    ...
    async def test_list_organization_contacts(self, mock_user, mock_organization):
        ...
        with patch('app.api.v1.organizations_complete_v30.get_current_user', return_value=mock_user), \
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(f"/api/v1/organizations/{mock_organization.id}/contacts")
