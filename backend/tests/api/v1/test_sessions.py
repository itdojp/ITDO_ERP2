...
class TestUserSessions:
    ...
    async def test_get_my_sessions(
        ...
        response = await async_client.get(
            "/api/v1/users/me/sessions", headers=user_headers
        )
    ...
    async def test_terminate_other_session(
        ...
        response = await async_client.delete(
            f"/api/v1/users/me/sessions/{other_session['id']}", headers=user_headers
        )
    ...
    async def test_cannot_terminate_current_session_via_api(
        ...
        current_session = next(
            s for s in response.json()["sessions"] if s["is_current"]
        )

        # 現在のセッションを終了しようとする
        response = await async_client.delete(
            f"/api/v1/users/me/sessions/{current_session['id']}", headers=user_headers
        )
...
class TestSessionSettings:
    ...
    async def test_get_session_settings(
        ...
        """
        response = await async_client.get(
            "/api/v1/users/me/sessions/settings", headers=user_headers
        )
    ...
    async def test_update_session_timeout(
        ...
        """
        settings = {"session_timeout_hours": 4, "idle_timeout_minutes": 30}

        response = await async_client.put(
            "/api/v1/users/me/sessions/settings", json=settings, headers=user_headers
        )
...
class TestAdminSessionMonitoring:
    ...
    async def test_admin_view_sessions_with_pagination(
        ...
        """
        response = await async_client.get(
            "/api/v1/admin/sessions?page=1&per_page=50", headers=admin_headers
        )
    ...
    async def test_admin_terminate_user_session(
        ...
        """
        response = await async_client.delete(
            f"/api/v1/admin/sessions/{active_session['id']}", headers=admin_headers
        )
    ...
    async def test_regular_user_cannot_access_admin_sessions(
        ...
        """
        response = await async_client.get(
            "/api/v1/admin/sessions", headers=user_headers
        )
...
class TestSessionExpiry:
    ...
    async def test_session_refresh_on_activity(
        ...
        # 別のAPIを呼び出す
        await async_client.get("/api/v1/users/me", headers=user_headers)
        ...
        assert (
            current_session_after["last_accessed_at"]
            > current_session_before["last_accessed_at"]
        )
