...
class TestInventoryCompleteV30:
    ...
    async def test_create_warehouse_success(self, mock_user):
        ...
        with (
            patch(
                "app.api.v1.inventory_complete_v30.require_admin",
                return_value=mock_user,
            ),
            patch("app.api.v1.inventory_complete_v30.get_db"),
    ...
    async def test_list_warehouses_with_filters(self, mock_user, mock_warehouse):
        ...
        with (
            patch(
                "app.api.v1.inventory_complete_v30.get_current_user",
                return_value=mock_user,
            ),
            patch("app.api.v1.inventory_complete_v30.get_db"),
            patch("app.crud.inventory_extended_v30.WarehouseCRUD") as mock_crud,
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    "/api/v1/warehouses?search=main&is_active=true&warehouse_type=standard"
                )
    ...
    @pytest.mark.asyncio
    async def test_list_inventory_items_with_filters(
        self, mock_user, mock_inventory_item
    ):
    ...
    async def test_adjust_inventory_insufficient_stock(self, mock_user):
        ...
        with (
            ...
            mock_crud_instance = Mock()
            mock_crud.return_value = mock_crud_instance
            mock_crud_instance.adjust_quantity.side_effect = InsufficientStockError(
    ...
    async def test_get_inventory_summary(self, mock_user):
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    "/api/v1/inventory/summary?product_id=product-1"
                )
    ...
    async def test_get_inventory_valuation(self, mock_user):
        ...
        with (
            ...
            mock_valuation = {
                ...
                "by_warehouse": {
                    "Main Warehouse": Decimal("30000.00"),
                    "Secondary": Decimal("20000.00"),
                },
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.put(
                    ...
                    json={"counted_quantity": 95, "notes": "Minor shortage found"},
                )
