...
class TestProductsCompleteV30:
    ...
    async def test_create_product_success(self, mock_user):
        ...
        with (
            patch(
                "app.api.v1.products_complete_v30.require_admin", return_value=mock_user
            ),
            patch("app.api.v1.products_complete_v30.get_db"),
    ...
    async def test_list_products_with_filters(self, mock_user, mock_product):
        ...
        with (
            patch(
                "app.api.v1.products_complete_v30.get_current_user",
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.put(
                    ...
                    json={"name": "Updated Product Name", "selling_price": "1200.00"},
                )
    ...
    async def test_get_product_inventory_movements(self, mock_user, mock_product):
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    f"/api/v1/products/{mock_product.id}/inventory/movements"
                )
    ...
    async def test_get_product_price_history(self, mock_user, mock_product):
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    f"/api/v1/products/{mock_product.id}/price-history"
                )
    ...
    async def test_list_suppliers(self, mock_user, mock_supplier):
        ...
        with (
            ...
            async with AsyncClient(app=app, base_url="http://test") as ac:
                response = await ac.get(
                    "/api/v1/suppliers?is_active=true&supplier_type=manufacturer"
                )
