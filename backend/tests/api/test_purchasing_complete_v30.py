...
class TestSupplierAPI:
    def test_create_supplier(self, mock_db, mock_current_user):
        ...
        with (
            patch("app.api.v1.purchasing_complete_v30.get_db", return_value=mock_db),
            patch(
                "app.api.v1.purchasing_complete_v30.get_current_user",
            ...
            response = client.post(
                "/api/v1/purchasing/suppliers", json=supplier_data.dict()
            )
...
class TestPurchaseRequestAPI:
    def test_create_purchase_request(self, mock_db, mock_current_user):
        ...
        with (
            ...
            patch(
                "app.api.v1.purchasing_complete_v30.PurchaseRequestCRUD"
            ) as mock_crud,
        ):
            mock_crud.return_value.create.return_value = mock_request

            response = client.post(
                "/api/v1/purchasing/requests", json=request_data.dict()
            )

            assert response.status_code == 201
            mock_crud.return_value.create.assert_called_once_with(
                request_data, mock_current_user["sub"]
            )
    ...
    def test_approve_purchase_request(self, mock_db, mock_current_user):
        ...
        with (
            ...
            response = client.post(
                f"/api/v1/purchasing/requests/{request_id}/approve?approved_budget={approved_budget}"
            )

            assert response.status_code == 200
            mock_crud.return_value.approve_request.assert_called_once_with(
                request_id, mock_current_user["sub"], approved_budget
            )
    ...
    def test_reject_purchase_request(self, mock_db, mock_current_user):
        ...
        with (
            ...
            response = client.post(
                f"/api/v1/purchasing/requests/{request_id}/reject?rejection_reason={rejection_reason}"
            )

            assert response.status_code == 200
            mock_crud.return_value.reject_request.assert_called_once_with(
                request_id, mock_current_user["sub"], rejection_reason
            )
...
class TestPurchaseReceiptAPI:
    def test_create_purchase_receipt(self, mock_db, mock_current_user):
        ...
        with (
            ...
            patch(
                "app.api.v1.purchasing_complete_v30.PurchaseReceiptCRUD"
            ) as mock_crud,
        ):
            mock_crud.return_value.create.return_value = mock_receipt

            response = client.post(
                "/api/v1/purchasing/receipts", json=receipt_data.dict()
            )

            assert response.status_code == 201
            mock_crud.return_value.create.assert_called_once_with(
                receipt_data, mock_current_user["sub"]
            )
...
class TestSupplierProductAPI:
    def test_create_supplier_product(self, mock_db, mock_current_user):
        ...
        with (
            ...
            patch(
                "app.api.v1.purchasing_complete_v30.SupplierProductCRUD"
            ) as mock_crud,
        ):
            mock_crud.return_value.create.return_value = mock_supplier_product

            response = client.post(
                "/api/v1/purchasing/supplier-products",
                json=supplier_product_data.dict(),
            )
