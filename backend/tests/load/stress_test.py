...
class StressTest:
    ...
    async def run_concurrent_requests(
        ...
        print(
            f"🚀 Starting stress test: {concurrent_users} users, {requests_per_user} requests each"
        )
    ...
    def analyze_results(self) -> Dict[str, Any]:
        ...
        analysis = {
            ...
            "throughput": len(successful) / max(response_times)
            if response_times
            else 0,
...
async def main():
    ...
    for scenario in scenarios:
        ...
        analysis = await stress_test.run_concurrent_requests(
            concurrent_users=scenario["users"], requests_per_user=scenario["requests"]
        )
