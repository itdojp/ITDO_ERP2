...
class SystemResourceMonitor:
    ...
    def get_peak_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "avg_memory_rss_mb": statistics.mean(self.metrics["memory_rss_mb"])
            if self.metrics["memory_rss_mb"]
            else 0,
            "monitoring_duration": self.metrics["timestamps"][-1]
    ...
    def detect_memory_leak(self) -> Dict[str, Any]:
        ...
        increase_percent = (
            (memory_increase_mb / start_avg) * 100 if start_avg > 0 else 0
        )

        is_leak = (
            memory_increase_mb > LOAD_TEST_CONFIG["memory_leak_threshold_mb"]
            and increase_percent > 20  # 20%以上の増加
        )
...
class TestERPLongDurationLoad:
    ...
    def test_sustained_load_performance(self, load_test_db):
            ...
            while time.time() < end_time:
                ...
                try:
                    success = self._execute_operation(
                        operation_type, test_products, user_id
                    )
        ...
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    user_results.append(
                        {"user_id": -1, "operations": 0, "errors": 1, "error": str(e)}
                    )
        ...
        avg_response_times = [
            r["avg_response_time"] for r in user_results if r["avg_response_time"] > 0
        ]
        ...
        assert overall_avg_response < 1.0, (
            f"平均応答時間が遅すぎます: {overall_avg_response:.3f}s > 1s"
        )
        assert resource_metrics["peak_memory_rss_mb"] < 500, (
            f"メモリ使用量が多すぎます: {resource_metrics['peak_memory_rss_mb']:.1f}MB"
        )
        assert not memory_leak_analysis["detected"], (
            f"メモリリークが検出されました: +{memory_leak_analysis['memory_increase_mb']:.1f}MB"
        )
        ...
        print(
            f"   メモリリーク: {'検出' if memory_leak_analysis['detected'] else '未検出'}"
        )
    ...
    def test_ramp_up_load_testing(self, load_test_db):
        ...
        LOAD_TEST_CONFIG["ramp_up_seconds"]
        ...
        for phase, target_users in enumerate(user_increments):
            ...
            phase_error_rate = (
                (phase_errors / phase_operations * 100) if phase_operations > 0 else 0
            )
            ...
            print(
                f"   操作数: {phase_operations}, エラー率: {phase_error_rate:.2f}%, スループット: {phase_throughput:.1f} ops/sec"
            )
        ...
        print(
            f"   ピーク スループット: {max([p['throughput'] for p in phase_results]):.1f} ops/sec"
        )
        print(f"   スケーラビリティ効率: {throughput_degradation:.1f}% (低い方が良い)")
        print(
            f"   リソース使用量 - メモリ: {resource_metrics['peak_memory_rss_mb']:.1f}MB, CPU: {resource_metrics['peak_cpu_percent']:.1f}%"
        )

        # スケーラビリティ要件確認
        assert throughput_degradation < 50, (
            f"スケーラビリティ低下が大きすぎます: {throughput_degradation:.1f}%"
        )
    ...
    def _execute_operation(
        self, operation_type: str, test_products: List[Dict], user_id: int
    ) -> bool:
        """操作実行"""
        try:
    ...
    def _analyze_scalability(self, phase_results: List[Dict]) -> float:
        ...
        efficiency_degradation = (
            (first_throughput_per_user - last_throughput_per_user)
            / first_throughput_per_user
        ) * 100
