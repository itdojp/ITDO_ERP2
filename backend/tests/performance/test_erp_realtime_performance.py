...
class RealtimePerformanceMonitor:
    ...
    def _monitor_loop(self):
        ...
        while self.is_monitoring:
            ...
            # 直近1秒間のスループット計算
            recent_requests = [
                t for t in self.metrics["response_times"] if current_time - t < 1.0
            ]

            throughput = len(recent_requests)
            self.metrics["throughput_data"].append(
                {"timestamp": current_time, "rps": throughput}
            )
        ...
        return {
            ...
            * 1000,  # 99パーセンタイル
            ...
            "peak_throughput_rps": max(
                [d["rps"] for d in self.metrics["throughput_data"]], default=0
            ),
        }
...
class TestERPRealtimePerformance:
    ...
    def test_product_search_realtime_performance(self, realtime_performance_db):
        ...
        for i in range(100):  # 100回の検索テスト
            ...
            try:
                    ...
                    failed_requests.append(
                        {
                            ...
                            "requirement_ms": REALTIME_REQUIREMENTS[
                                "product_search_max_ms"
                            ],
                        }
                    )
                ...
                failed_requests.append({"pattern": pattern, "error": str(e)})
        ...
        assert (
            ...
            < REALTIME_REQUIREMENTS["product_search_max_ms"]
        ), (
        ...
        assert (
            ...
            < REALTIME_REQUIREMENTS["product_search_max_ms"] * 1.5
        ), (
        ...
        assert (
            ...
            < REALTIME_REQUIREMENTS["error_rate_max_percent"]
        ), (
        ...
        print("✅ 商品検索パフォーマンス結果:")
        print(
            f"   平均応答時間: {stats['avg_response_time_ms']:.1f}ms (要件: {REALTIME_REQUIREMENTS['product_search_max_ms']}ms)"
        )
    ...
    def test_inventory_update_realtime_performance(self, realtime_performance_db):
        ...
        assert (
            stats["avg_response_time_ms"]
            < REALTIME_REQUIREMENTS["inventory_update_max_ms"]
        ), (
        ...
        assert (
            stats["p95_response_time_ms"]
            < REALTIME_REQUIREMENTS["inventory_update_max_ms"] * 2
        ), (
            f"95パーセンタイル応答時間が要件を大幅超過: {stats['p95_response_time_ms']:.1f}ms"
        )
        ...
        print("✅ 在庫更新パフォーマンス結果:")
        print(
            f"   平均応答時間: {stats['avg_response_time_ms']:.1f}ms (要件: {REALTIME_REQUIREMENTS['inventory_update_max_ms']}ms)"
        )
        print(f"   95%タイル応答時間: {stats['p95_response_time_ms']:.1f}ms")
        print(f"   エラー率: {stats['error_rate_percent']:.2f}%")
        print(f"   スループット: {stats['average_throughput_rps']:.1f} RPS")
    ...
    def test_concurrent_users_realtime_performance(self, realtime_performance_db):
        ...
        def simulate_realtime_user(
            user_id: int, duration_seconds: int = 30
        ) -> Dict[str, Any]:
            """リアルタイムユーザーシミュレーション"""
            ...
            try:
                while time.time() < end_time:
                    # ランダムな業務操作
                    operation = ["search", "detail", "inventory", "order"][
                    ...
                    try:
                        if operation == "search":
                            ...
                            product = test_products[
                                operations_performed % len(test_products)
                            ]
        ...
        with ThreadPoolExecutor(max_workers=concurrent_users) as executor:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    user_results.append(
                        {"success": False, "error": str(e), "user_id": -1}
                    )
        ...
        [r for r in user_results if not r["success"]]

        success_rate = len(successful_users) / len(user_results) * 100
        total_operations = sum(
            r.get("operations_performed", 0) for r in successful_users
        )
        ...
        if all_response_times:
            ...
            overall_p95_response_ms = (
                statistics.quantiles(all_response_times, n=20)[18] * 1000
            )
        else:
        ...
        assert success_rate >= 95.0, (
            f"ユーザー成功率が低すぎます: {success_rate:.1f}% < 95%"
        )
        ...
        assert (
            overall_throughput >= REALTIME_REQUIREMENTS["throughput_min_rps"] * 0.5
        ), (
            f"スループットが低すぎます: {overall_throughput:.1f} < {REALTIME_REQUIREMENTS['throughput_min_rps'] * 0.5}"
        )

        print("✅ 同時ユーザーパフォーマンス結果:")
...
class TestERPStressTestRealtime:
    ...
    def test_spike_load_handling(self, realtime_performance_db):
        ...
        # 回復性能の確認
        recovery_ratio = (
            recovery_results["avg_response_ms"] / baseline_results["avg_response_ms"]
        )
