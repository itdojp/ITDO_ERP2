...
from app.models.inventory import InventoryAlert, InventoryItem
...
class TestInventoryTransactionManager:
    @pytest.mark.asyncio
    async def test_execute_transaction_inbound_success(
        self, transaction_manager, mock_db_session, sample_product, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_transaction_outbound_insufficient_stock(
        self, transaction_manager, mock_db_session, sample_product, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_transaction_product_not_found(
        self, transaction_manager, mock_db_session, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_bulk_transactions_success(
        self, transaction_manager, mock_db_session, sample_product, sample_user
    ):
        ...
        mock_db_session.execute.side_effect = [
            product_result,
            stock_result,
            inventory_result,  # First transaction
            product_result,
            stock_result,
            inventory_result,  # Second transaction
        ...
        results = await transaction_manager.execute_bulk_transactions(
            requests, sample_user.id, "BULK-001"
        )
    ...
    async def test_create_alert_low_stock(self, transaction_manager, mock_db_session):
        ...
        existing_alert_result.scalar_one_or_none.return_value = (
    ...
    @pytest.mark.asyncio
    async def test_process_location_transfer(
        self, transaction_manager, mock_db_session, sample_product
    ):
        ...
        with patch.object(
            transaction_manager,
            "_get_or_create_inventory_item",
            side_effect=[from_item, to_item],
        ):
            mock_db_session.flush = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_transfer_missing_locations(
        self, transaction_manager, sample_product
    ):
        ...
        with pytest.raises(
            ValueError,
            match="Both from_location and to_location required for transfers",
        ):
            await transaction_manager._process_location_transfer(request)
...
class TestReservationManager:
    ...
    async def test_create_reservation_success(
        self, reservation_manager, mock_db_session, sample_product, sample_user
    ):
        ...
        with patch(
            "app.api.v1.inventory_advanced_v57.InventoryTransactionManager"
        ) as mock_tm:
            ...
            # Execute reservation
            result = await reservation_manager.create_reservation(
    ...
    @pytest.mark.asyncio
    async def test_create_reservation_insufficient_stock(
        self, reservation_manager, mock_db_session, sample_product, sample_user
    ):
        ...
        request = ReservationRequest(
            product_id=sample_product.id, quantity=25, reserved_for="ORDER-12345"
        )

        # Execute reservation - should raise BusinessLogicError
        with pytest.raises(
            BusinessLogicError, match="Insufficient stock for reservation"
        ):
...
class TestInventoryAdvancedAPI:
    ...
    def test_bulk_transaction_request_validation(self):
        ...
        bulk_request = BulkTransactionRequest(
            transactions=transactions, batch_reference="BULK-001", auto_commit=True
        )

        assert len(bulk_request.transactions) == 2
        assert bulk_request.batch_reference == "BULK-001"
        assert bulk_request.auto_commit
    ...
    def test_inventory_search_request_validation(self):
        ...
        assert not request.include_zero_stock
...
class TestPerformanceAndEdgeCases:
    @pytest.mark.asyncio
    async def test_large_bulk_transaction_performance(
        self, transaction_manager, mock_db_session, sample_product, sample_user
    ):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_reservations_same_product(
        self, reservation_manager, mock_db_session, sample_product, sample_user
    ):
        ...
        with patch(
            ...
            successful_reservations = [
                r for r in results if isinstance(r, ReservationResponse)
            ]
            ...
            assert mock_tm_instance.execute_transaction.call_count == len(
                successful_reservations
            )
