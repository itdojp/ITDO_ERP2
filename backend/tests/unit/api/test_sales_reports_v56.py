...
class TestSalesAnalyticsService:
    @pytest.mark.asyncio
    async def test_generate_revenue_summary_success(
    ...
    @pytest.mark.asyncio
    async def test_generate_revenue_summary_with_filters(
        self, analytics_service, mock_db_session
    ):
        ...
        start_date = date(2024, 1, 1)
        end_date = date(2024, 1, 31)
        filters = {"customer_ids": [uuid4(), uuid4()], "product_categories": [uuid4()]}

        result = await analytics_service.generate_revenue_summary(
    ...
    @pytest.mark.asyncio
    async def test_get_product_performance_success(
        self, analytics_service, mock_db_session, sample_product_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_customer_analysis_success(
        self, analytics_service, mock_db_session
    ):
    ...
    async def test_get_trend_analysis_monthly(self, analytics_service, mock_db_session):
        ...
        result = await analytics_service.get_trend_analysis(
            start_date, end_date, ReportPeriod.MONTHLY
        )
