...
from tests.factories import UserFactory, OrganizationFactory, RoleFactory
...
class TestUserRepository:
    ...
    def test_search_users(self, db_session: Session) -> None:
        ...
        # Create test users
        user1 = UserFactory.create(db_session, full_name="John Doe", email="john@example.com")
        user2 = UserFactory.create(db_session, full_name="Jane Smith", email="jane@example.com")
        user3 = UserFactory.create(db_session, full_name="Bob Johnson", email="bob@example.com")
    ...
    def test_get_locked_users(self, db_session: Session) -> None:
        ...
        expired_lock = UserFactory.create(
            db_session,
            locked_until=datetime.utcnow() - timedelta(minutes=1)
        )
        not_locked = UserFactory.create(db_session, locked_until=None)
    ...
    def test_get_superusers(self, db_session: Session) -> None:
        ...
        inactive_super = UserFactory.create(db_session, is_superuser=True, is_active=False)
        normal_user = UserFactory.create(db_session, is_superuser=False)
