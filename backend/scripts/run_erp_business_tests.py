...
class ERPBusinessTestRunner:
    ...
    def run_test_suite(self, test_type="all", verbose=False, coverage=False):
        ...
        if test_type == "all":
            ...
            selected_suites = [k for k, v in test_suites.items() if v["priority"] == "high"]
        ...
        print(f"   テスト成功率: {(total_passed/total_test_count*100) if total_test_count > 0 else 0:.1f}%")
    ...
    def _save_results(self):
        """テスト結果保存"""
        results_file = self.backend_path / "test-results" / f"erp_business_test_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def main():
    ...
    parser.add_argument("--type", choices=["all", "critical", "basic", "performance", "security", "integration"],
                       default="all", help="実行するテスト種別")
    parser.add_argument("--verbose", action="store_true", help="詳細出力")
    parser.add_argument("--coverage", action="store_true", help="カバレッジ測定")
    parser.add_argument("--continuous", type=int, metavar="INTERVAL", help="継続的テスト実行（秒間隔）")
