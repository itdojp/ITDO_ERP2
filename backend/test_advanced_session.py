...
class SimpleSecurityService:
    ...
    def is_ip_suspicious(self, ip_address: str, user: User) -> bool:
        ...
        known_ip = self.db.query(UserSession).filter(
    ...
    def get_session_analytics(self, user: User) -> dict:
        ...
        activities = self.db.query(
            ...
            self.db.query(SessionActivity).filter(
...
def test_advanced_session():
    ...
    events = db.query(SessionActivity).filter(
        SessionActivity.user_id == user1.id
    ).all()
    assert len(events) >= 2
    ...
    print(f"✓ Analytics: {analytics['total_sessions']} total, {analytics['active_sessions']} active")
