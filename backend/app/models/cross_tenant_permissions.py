...
class CrossTenantPermissionRule(BaseModel):
    ...
    rule_type: Mapped[str] = mapped_column(
        String(20), nullable=False, default="allow", comment="Rule type: allow, deny"
    )
    ...
    is_active: Mapped[bool] = mapped_column(
        Boolean, nullable=False, default=True, comment="Whether the rule is active"
    )
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True), nullable=True, comment="When the rule expires"
    )
    notes: Mapped[Optional[str]] = mapped_column(
        Text, nullable=True, comment="Additional notes about the rule"
    )

    # Relationships
    source_organization: Mapped["Organization"] = relationship(
        "Organization", foreign_keys=[source_organization_id], lazy="joined"
    )
    target_organization: Mapped["Organization"] = relationship(
        "Organization", foreign_keys=[target_organization_id], lazy="joined"
    )
...
class CrossTenantAuditLog(BaseModel):
    ...
    permission: Mapped[str] = mapped_column(
        String(200), nullable=False, comment="Permission that was checked/used"
    )
    action: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Action: check, grant, deny, revoke"
    )
    result: Mapped[str] = mapped_column(
        String(50), nullable=False, comment="Result: allowed, denied"
    )
    ...
    ip_address: Mapped[Optional[str]] = mapped_column(
        String(45), nullable=True, comment="IP address of the request"
    )
    user_agent: Mapped[Optional[str]] = mapped_column(
        String(500), nullable=True, comment="User agent of the request"
    )
    ...
    rule: Mapped[Optional["CrossTenantPermissionRule"]] = relationship(
        "CrossTenantPermissionRule", foreign_keys=[rule_id], lazy="joined"
    )
