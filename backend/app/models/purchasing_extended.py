...
class PurchaseRequestItem(Base):
    ...
    purchase_request_id = Column(
        String, ForeignKey("purchase_requests.id"), nullable=False
    )
...
class PurchaseOrder(Base):
    ...
    status = Column(
        String(50), default="draft"
    )  # draft, sent, acknowledged, shipped, received, cancelled
    receipt_status = Column(
        String(50), default="pending"
    )  # pending, partial, complete, over_received
...
class PurchaseReceiptItem(Base):
    ...
    purchase_receipt_id = Column(
        String, ForeignKey("purchase_receipts.id"), nullable=False
    )
    purchase_order_item_id = Column(
        String, ForeignKey("purchase_order_items.id"), nullable=False
    )
