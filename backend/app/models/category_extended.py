...
class Category(Base):
    ...
    path = Column(
        String(500)
    )  # Full category path like "Electronics > Computers > Laptops"
    path_ids = Column(String(500))  # ID path like "cat1,cat2,cat3"

    # 分類タイプ
    category_type = Column(
        String(50), default="product"
    )  # product, service, expense, asset, customer, supplier
    ...
    pricing_rules = relationship(
        "CategoryPricingRule", back_populates="category", cascade="all, delete-orphan"
    )
...
class CategoryAttribute(Base):
    ...
    option_values = Column(
        JSON, default=[]
    )  # [{"value": "red", "label": "赤", "label_en": "Red"}]
...
class CategoryPricingRule(Base):
    ...
    # ルール設定
    rule_type = Column(
        String(30), default="markup"
    )  # markup, discount, fixed_price, cost_plus, competitive
