...
class UserSession(Base):
    ...
    user: Mapped["User"] = relationship(
        "User", foreign_keys=[user_id], back_populates="sessions"
    )
    ...
    def revoke(
        self, db: Session, revoked_by: int | None = None, reason: str | None = None
    ) -> None:
        """Revoke this session."""
...
class SessionActivity(Base):
    ...
    session_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("user_sessions.id"), nullable=False
    )
    user_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("users.id"), nullable=False
    )
...
class SessionConfiguration(Base):
    ...
    trusted_devices: Mapped[str | None] = mapped_column(
        Text
