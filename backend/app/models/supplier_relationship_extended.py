...
class SupplierRelationship(Base):
    ...
    relationship_type = Column(
        String(50), default="vendor"
    )  # vendor, partner, strategic, preferred
    partnership_level = Column(
        String(50), default="standard"
    )  # strategic, preferred, standard, conditional
    contract_type = Column(
        String(50)
    )  # master_agreement, project_based, spot_buy, framework
    ...
    performance_reviews = relationship(
        ...
        back_populates="supplier_relationship",
...
class SupplierPerformanceReview(Base):
    ...
    id = Column(String, primary_key=True, default=lambda: str(uuid.uuid4()))
    supplier_relationship_id = Column(
        String, ForeignKey("supplier_relationships.id"), nullable=False
    )
    ...
    review_type = Column(
        String(50), default="quarterly"
    )  # monthly, quarterly, semi_annual, annual
    review_status = Column(
        String(50), default="draft"
    )  # draft, in_review, completed, approved
    ...
    # リレーション
    supplier_relationship = relationship(
        "SupplierRelationship", back_populates="performance_reviews"
    )
...
class SupplierNegotiation(Base):
    ...
    negotiation_type = Column(
        String(50), default="contract_renewal"
    )  # contract_renewal, price_adjustment, terms_revision, new_contract
    description = Column(Text)

    # ステータス・フェーズ
    status = Column(
        String(50), default="preparation"
    )  # preparation, active, on_hold, completed, cancelled
    current_phase = Column(
        String(50), default="planning"
    )  # planning, proposal, negotiation, agreement, finalization
    ...
    # リレーション
    supplier_relationship = relationship(
        "SupplierRelationship", back_populates="negotiations"
    )
    lead_negotiator = relationship("User", foreign_keys=[lead_negotiator_id])
    approver = relationship("User", foreign_keys=[approved_by])
    meetings = relationship(
        "NegotiationMeeting", back_populates="negotiation", cascade="all, delete-orphan"
    )
...
class NegotiationMeeting(Base):
    ...
    negotiation_id = Column(
        String, ForeignKey("supplier_negotiations.id"), nullable=False
    )
    ...
    meeting_type = Column(
        String(50), default="negotiation"
    )  # kickoff, negotiation, review, finalization
    meeting_date = Column(DateTime, nullable=False)
    duration_minutes = Column(Integer)
    location = Column(String(200))
    meeting_format = Column(
...
class SupplierContract(Base):
    ...
    contract_type = Column(
        String(50), default="purchase_agreement"
    )  # purchase_agreement, service_agreement, nda, framework
...
class ContractAmendment(Base):
    ...
    amendment_type = Column(
        String(50), default="terms_change"
    )  # terms_change, price_adjustment, scope_change, extension
...
class ContractMilestone(Base):
    ...
    milestone_type = Column(
        String(50), default="deliverable"
    )  # deliverable, payment, review, renewal, expiry
    description = Column(Text)
    ...
    )  # upcoming, due, completed, overdue, cancelled
