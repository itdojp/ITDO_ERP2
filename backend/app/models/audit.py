...
class AuditLog(Base):
    ...
    resource_id: int = Column(Integer, nullable=False, index=True)
    organization_id: Optional[int] = Column(Integer, ForeignKey("organizations.id"), index=True)
    changes: Dict[str, Any] = Column(JSON, default=dict)
    ip_address: Optional[str] = Column(String(45))
    user_agent: Optional[str] = Column(Text)
    checksum: Optional[str] = Column(String(64))  # SHA-256 hash for integrity
