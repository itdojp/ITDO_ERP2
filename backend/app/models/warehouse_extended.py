...
class Warehouse(Base):
    ...
    warehouse_type = Column(
        String(50), default="distribution"
    )  # distribution, manufacturing, retail, cross_dock, 3pl
    ...
    operating_days = Column(
        JSON, default=["mon", "tue", "wed", "thu", "fri"]
    )  # Days of operation
    ...
    storage_systems = Column(
        JSON, default=[]
    )  # ["pallet_rack", "shelf", "floor", "bulk"]
    ...
    security_certifications = Column(
        JSON, default=[]
    )  # ISO certifications, security standards
    ...
    # パフォーマンス指標
    utilization_target = Column(
        Decimal(5, 2), default=85
    ...
    cycle_counts = relationship(
        "CycleCount", back_populates="warehouse", cascade="all, delete-orphan"
    )
...
class WarehouseZone(Base):
    ...
    zone_type = Column(
        String(50), default="storage"
    )  # storage, receiving, shipping, staging, quality_control
    ...
    access_equipment_required = Column(
        JSON, default=[]
    )  # Required equipment/credentials
    ...
    locations = relationship(
        "WarehouseLocation", back_populates="zone", cascade="all, delete-orphan"
    )
...
class WarehouseLocation(Base):
    ...
    access_method = Column(
        String(30), default="manual"
    )  # manual, forklift, crane, automated
    ...
    primary_product_id = Column(
        String, ForeignKey("products.id")
    )  # Primary product if single-SKU
    ...
    )  # available, occupied, blocked, damaged, maintenance
...
class InventoryMovement(Base):
    ...
    )  # receipt, shipment, adjustment, transfer, cycle_count
    movement_subtype = Column(
        String(50)
    )  # purchase_receipt, sales_shipment, internal_transfer, etc.
    ...
    from_location_id = Column(
        String, ForeignKey("warehouse_locations.id")
    )  # Source location
    ...
    to_warehouse_id = Column(
        String, ForeignKey("warehouses.id")
    ...
    quality_status = Column(
        String(30), default="approved"
    )  # approved, rejected, quarantine, pending
    ...
    reversal_movement_id = Column(
        String, ForeignKey("inventory_movements.id")
    )  # Reference to reversal
    ...
    location = relationship(
        "WarehouseLocation", foreign_keys=[location_id], back_populates="movements"
    )
    ...
    reversal_movement = relationship(
        "InventoryMovement", foreign_keys=[reversal_movement_id], remote_side=[id]
    )
    creator = relationship("User", foreign_keys=[created_by])
...
class CycleCount(Base):
    ...
    # 範囲・対象
    count_scope = Column(
        String(30), default="zone"
    )  # zone, location, product, category, all
...
class WarehousePerformance(Base):
    ...
    # 期間情報
    performance_period = Column(
        String(20), nullable=False
    )  # YYYY-MM or YYYY-Wxx format
    period_type = Column(
