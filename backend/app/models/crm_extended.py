...
class CRMCustomer(Base):
    ...
    )  # new, contacted, qualified, opportunity, customer, lost
    customer_stage = Column(
        String(50), default="prospect"
    )  # prospect, lead, opportunity, customer, advocate
    lifecycle_stage = Column(
        String(50), default="subscriber"
    ...
    interactions = relationship(
        "CustomerInteraction", back_populates="customer", cascade="all, delete-orphan"
    )
    opportunities = relationship(
        "SalesOpportunity", back_populates="customer", cascade="all, delete-orphan"
    )
...
class CustomerInteraction(Base):
    ...
    )  # call, email, meeting, demo, webinar, chat
    ...
    outcome = Column(
        String(100)
    )  # interested, not_interested, follow_up, meeting_scheduled
...
class SalesOpportunity(Base):
    ...
    loss_reason = Column(
        String(100)
    )  # price, competitor, no_budget, timing, feature_gap
...
class OpportunityActivity(Base):
    ...
    opportunity_id = Column(
        String, ForeignKey("sales_opportunities.id"), nullable=False
    )
    ...
    activity_type = Column(
        String(50), nullable=False
    )  # call, email, meeting, proposal_sent, demo
    subject = Column(String(300))
    description = Column(Text)
...
class CustomerActivity(Base):
    ...
    activity_type = Column(
        String(50), nullable=False
    )  # task, note, call, email, meeting, webpage_visit, form_submit
...
class CustomerSegment(Base):
    ...
    customers = relationship(
        "CRMCustomer", secondary="customer_segment_members", back_populates="segments"
    )
...
class MarketingCampaign(Base):
    ...
    campaign_type = Column(
        String(50)
    )  # email, social_media, ppc, content, event, webinar
