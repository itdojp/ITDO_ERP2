...
class BusinessRulesEngine:
    ...
    def _register_default_evaluators(self) -> dict:
    ...
    async def _get_applicable_rules(
        ...
        query = (
            ...
            .where(
                and_(BusinessRule.entity_type == entity_type, BusinessRule.is_active)
            )
        )
    ...
    async def _execute_single_rule(
        self, rule: BusinessRule, context: RuleContext
    ) -> RuleResult:
            ...
            for action in rule.actions:
                ...
                if not action_result.get("success", True):
                    result.errors.append(
                        f"Action {action.action_type} failed: {action_result.get('error')}"
                    )
                    if rule.is_blocking:
    ...
    async def _execute_set_field(
        ...
        return {"success": True, "field_name": field_path, "new_value": new_value}
    ...
    async def _execute_create_task(
        ...
        return {"success": True, "action": "task_created", "task_data": task_data}
    ...
    async def _execute_log_event(
        ...
        await self.audit_service.log_event(
            ...
            details={
                "message": message,
                "rule_execution_id": str(context.execution_id),
            },
        )
    ...
    async def _log_rule_execution(
        self, rule: BusinessRule, context: RuleContext, result: RuleResult
    ):
    ...
    async def _log_execution_summary(
        self, execution_id: UUID, context: RuleContext, results: List[RuleResult]
    ):
        """Log execution summary"""
    ...
    async def _log_execution_error(
        self, execution_id: UUID, context: RuleContext, error: str
    ):
        """Log execution error"""
        ...
        await self.audit_service.log_event(
            ...
            entity_id=context.entity_id,
            user_id=context.user_id,
            details={"execution_id": str(execution_id), "error": error},
        )
    ...
    def register_condition_evaluator(self, operator: str, evaluator: Callable) -> dict:
        """Register custom condition evaluator"""
        self.condition_evaluators[operator] = evaluator

    def register_action_executor(self, action_type: str, executor: Callable) -> dict:
...
async def validate_business_rules(
    ...
    return await execute_business_rules(
        entity_type, entity_id, data, user_id, session, rule_type=RuleType.VALIDATION
    )
...
async def execute_calculation_rules(
    ...
    return await execute_business_rules(
        entity_type, entity_id, data, user_id, session, rule_type=RuleType.CALCULATION
    )
