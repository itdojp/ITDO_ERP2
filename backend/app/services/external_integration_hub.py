...
from app.models.integration import (
    ExternalSystem, IntegrationEndpoint, DataMapping, SyncJob,
    WebhookSubscription, ApiKeyManagement, IntegrationLog
)
...
class WebhookManager:
    ...
    async def process_webhook(
        ...
        if processor:
            ...
            return {'status': 'no_processor', 'message': f'No processor for event type: {event_type}'}
...
class ExternalIntegrationHub:
    ...
    async def sync_data(
        ...
        try:
            ...
            adapter_class = self.adapters.get(IntegrationType(system.integration_type))
            if not adapter_class:
                raise IntegrationError(f"Unsupported integration type: {system.integration_type}")
    ...
    async def process_webhook(
        ...
        return await self.webhook_manager.process_webhook(
    ...
    async def get_system_metrics(
        ...
        avg_execution_time = sum(j.execution_time or 0 for j in jobs) / total_jobs if total_jobs > 0 else 0
        ...
        return {
            ...
            'performance': {
                ...
                'throughput_per_second': total_records / sum(j.execution_time or 0 for j in jobs) if sum(j.execution_time or 0 for j in jobs) > 0 else 0
            }
        }
...
async def register_external_system(
    ...
    return await integration_hub.register_system(name, integration_type, config, session)
...
async def sync_with_external_system(
    ...
    return await integration_hub.sync_data(system_id, endpoint, direction, data, session=session)
