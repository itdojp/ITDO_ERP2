...
class SessionService:
    ...
    def create_session(
        ...
        session = UserSession(
            ...
            expires_at=datetime.now(UTC) + timedelta(hours=config.session_timeout_hours),
            refresh_expires_at=datetime.now(UTC) + timedelta(days=config.refresh_token_days),
        )
    ...
    def refresh_session(
        ...
        config = self.db.query(SessionConfiguration).filter(
            SessionConfiguration.user_id == session.user_id
        ).first()
        ...
        session.expires_at = datetime.now(UTC) + timedelta(hours=config.session_timeout_hours)
        session.refresh_expires_at = datetime.now(UTC) + timedelta(days=config.refresh_token_days)
    ...
    def update_session_config(
        ...
        if session_timeout_hours is not None:
            if session_timeout_hours < 1 or session_timeout_hours > config.max_session_timeout_hours:
        ...
        if max_concurrent_sessions is not None:
            if max_concurrent_sessions < 1 or max_concurrent_sessions > 10:
                raise BusinessLogicError("同時セッション数は1〜10の間で設定してください")
