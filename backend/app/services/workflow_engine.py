...
from app.models.workflow import (
    Workflow, WorkflowInstance, WorkflowNode, WorkflowConnection,
    WorkflowTask, WorkflowVariable, WorkflowHistory, WorkflowTemplate
)
from app.models.user import User
from app.services.audit_service import AuditService
from app.services.business_rules_engine import business_rules_engine, RuleContext
...
class WorkflowEngine:
    ...
    async def _execute_node(
        ...
        try:
            ...
            if not executor:
                raise WorkflowError(f"No executor found for node type: {node.node_type}")
    ...
    async def _execute_end_node(
        ...
        instance = await self._get_workflow_instance(context.instance_id, context.session)
        if instance:
    ...
    async def _execute_decision_node(
        ...
        for condition in conditions:
            if await self._evaluate_condition(condition, context):
                ...
                if connection_id:
                    target_node = await self._get_connection_target(connection_id, context.session)
                    if target_node:
    ...
    async def _execute_subprocess_node(
        ...
        return NodeExecutionResult(
            node_id=node.id,
            success=False,
            error="No subprocess workflow specified"
        )
    ...
    async def _execute_script_node(
        ...
        if script:
            result = await self._execute_script(script, context)
            next_nodes = await self._get_next_nodes(node.id, context.session) if result.get('success', True) else []
    ...
    # Helper Methods
    async def _get_workflow(self, workflow_id: UUID, session: AsyncSession) -> Optional[Workflow]:
        """Get workflow by ID"""
        if not session:
            return None
    ...
    async def _get_workflow_instance(self, instance_id: UUID, session: AsyncSession) -> Optional[WorkflowInstance]:
    ...
    async def _get_workflow_node(self, node_id: UUID, session: AsyncSession) -> Optional[WorkflowNode]:
    ...
    async def _get_incoming_nodes(self, node_id: UUID, session: AsyncSession) -> List[UUID]:
    ...
    async def _get_connection_target(self, connection_id: UUID, session: AsyncSession) -> Optional[UUID]:
    ...
    async def _cancel_pending_tasks(self, instance_id: UUID, session: AsyncSession):
    ...
    async def _handle_node_error(self, node: WorkflowNode, context: WorkflowContext, error: str):
        ...
        await self._log_workflow_event(
            "workflow_error",
            context,
            {"node_id": str(node.id), "error": error}
        )
