...
from app.models.integration import (
    IntegrationEndpoint, IntegrationLog, DataMapping, SyncJob,
    APICredential, WebhookEndpoint, MessageQueue, ExternalSystem
)
from app.services.audit_service import AuditService
from app.services.validation_framework import validation_framework
    ...
    async def send_data(self, context: IntegrationContext) -> IntegrationResult:
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=self.timeout) as client:
                ...
                if method == 'GET':
                    response = await client.get(url, headers=headers, params=context.data)
                elif method == 'POST':
                    response = await client.post(url, headers=headers, json=context.data)
                elif method == 'PUT':
...
class WebhookAdapter(BaseIntegrationAdapter):
    ...
    async def receive_data(self, context: IntegrationContext) -> IntegrationResult:
        ...
        return IntegrationResult(
            success=False,
            error_message="Webhooks cannot actively receive data"
        )
...
class DataMapper:
    ...
    def register_mapping(self, integration_id: str, field_mappings: Dict[str, str]):
    ...
    def map_data(self, integration_id: str, data: Dict[str, Any], reverse: bool = False) -> Dict[str, Any]:
    ...
    def transform_data(self, data: Dict[str, Any], transformations: List[Dict[str, Any]]) -> Dict[str, Any]:
...
class IntegrationManager:
    ...
    async def receive_data(
        ...
        if result.success and result.data:
            result.data = self.data_mapper.map_data(integration_id, result.data, reverse=True)
        ...
        try:
            if sync_direction == SyncDirection.OUTBOUND:
                ...
                outbound_result = await self.sync_data(
                    integration_id, entity_type, SyncDirection.OUTBOUND, filters, user_id, session
                )
                inbound_result = await self.sync_data(
                    integration_id, entity_type, SyncDirection.INBOUND, filters, user_id, session
                )
                ...
                result = IntegrationResult(
                    ...
                    execution_time_ms=outbound_result.execution_time_ms + inbound_result.execution_time_ms,
                    records_processed=outbound_result.records_processed + inbound_result.records_processed
                )
