...
class PaymentProcessingService:
        ...
        try:
            ...
            if risk_score > 80:
                ...
                await self._log_processing_step(
                    transaction, "fraud_check", PaymentStatus.FAILED,
            ...
            if auth_result.get("success", False):
                ...
                if request.amount < Decimal("100000"):  # Auto-capture under 100,000 JPY
                    ...
                    if capture_result.get("success", False):
                        ...
                        transaction.failure_message = capture_result.get("error_message")
            else:
                transaction.status = PaymentStatus.FAILED
            ...
            # Log processing step
            await self._log_processing_step(
                transaction, "payment_processing", transaction.status,
            ...
            return PaymentResponse(
                ...
                message=transaction.failure_message if transaction.status == PaymentStatus.FAILED else "Payment successful",
    ...
    async def refund_payment(
        ...
        try:
            ...
            total_refunded = sum(
                refund.amount for refund in transaction.refunds 
            ...
            return PaymentResponse(
                ...
                message="Refund processed successfully" if refund.status == PaymentStatus.COMPLETED else "Refund failed"
            )
    ...
    async def get_payment_status(self, transaction_id: str) -> Optional[PaymentTransaction]:
    ...
    async def get_payment_analytics(
        ...
        metrics_query = select(
            ...
            func.count().filter(PaymentTransaction.status == PaymentStatus.COMPLETED).label("successful_transactions"),
            func.count().filter(PaymentTransaction.status == PaymentStatus.FAILED).label("failed_transactions"),
            func.sum(PaymentTransaction.amount).label("total_amount"),
            func.sum(PaymentTransaction.amount).filter(
        ...
        ).where(and_(*conditions)).group_by(PaymentTransaction.status)
        ...
        provider_query = select(
            ...
            func.count(PaymentTransaction.id).label("count"),
            func.sum(PaymentTransaction.amount).label("amount")
        ).where(and_(*conditions)).group_by(PaymentTransaction.payment_provider)
        ...
        method_query = select(
            ...
            func.count(PaymentTransaction.id).label("count"),
            func.sum(PaymentTransaction.amount).label("amount")
        ).where(and_(*conditions)).group_by(PaymentTransaction.payment_method)
        ...
        return {
            ...
            "totals": {
                ...
                "success_rate": float(
                    (metrics.successful_transactions or 0) / max(metrics.total_transactions or 1, 1) * 100
                ),
            ...
            recent_failures = await self.db.execute(
                ...
                .where(
                    and_(
                        ...
                        PaymentTransaction.created_at >= datetime.now(timezone.utc) - timedelta(hours=1)
                    )
                )
            )
    ...
    async def _save_payment_method_info(
        ...
        if request.card_number:
            ...
            payment_method_info.card_brand = provider_response.get("card_brand", "unknown")
...
class MockStripeProvider(BasePaymentProvider):
    ...
    async def authorize_payment(self, request: PaymentRequest) -> Dict[str, Any]:
        ...
        return {
            ...
            "card_brand": "visa" if request.card_number and request.card_number.startswith("4") else "mastercard",
            "card_type": "credit",
            "3ds_used": True,
            "cvv_verified": True,
...
class MockGMOProvider(BasePaymentProvider):
    ...
    async def authorize_payment(self, request: PaymentRequest) -> Dict[str, Any]:
        ...
        return {
            ...
            "card_brand": "jcb" if request.card_number and request.card_number.startswith("35") else "visa",
            "card_type": "credit"
        }
