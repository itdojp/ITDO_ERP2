...
from app.models.security import RolePermission, UserRole
...
class PasswordPolicy:
    ...
    def validate_password(
        self, password: str, user_id: Optional[UUID] = None
    ) -> Tuple[bool, List[str]]:
...
class RateLimiter:
    ...
    def set_limit(self, key: str, max_attempts: int, window_seconds: int) -> dict:
    ...
    def is_allowed(self, key: str, identifier: str) -> Tuple[bool, int]:
        ...
        if limit_key in self.attempts:
            ...
            self.attempts[limit_key] = [
                attempt for attempt in self.attempts[limit_key] if attempt > cutoff
            ]
        else:
            self.attempts[limit_key] = []
...
class SecurityTokenManager:
    ...
    def __init__(self, secret_key: str, algorithm: str = "HS256") -> dict:
...
class ThreatDetector:
    ...
    def _detect_sql_injection(
        ...
        def check_dict(data: Dict[str, Any]) -> dict:
    ...
    async def _detect_brute_force(
        ...
        recent_failures = len(
            [
                ...
                if (
                    t.threat_type == "login_failure"
                    and t.source_ip == context.ip_address
                    and t.detected_at
...
class PermissionManager:
    ...
    async def check_permission(
        self, user_id: UUID, resource: str, permission: str, session: AsyncSession
    ) -> bool:
        ...
        for higher_perm in self.permission_hierarchy.get(
    ...
    async def revoke_permission(
        self, role_id: UUID, resource: str, permission: str, session: AsyncSession
    ):
...
class AdvancedSecurityManager:
    ...
    async def authenticate_user(
        ...
        allowed, remaining = self.rate_limiter.is_allowed(
            "login", context.ip_address or "unknown"
        )
        if not allowed:
        ...
        if any(
            t.threat_level in [ThreatLevel.HIGH, ThreatLevel.CRITICAL] for t in threats
        ):
        ...
        await self._log_security_event(
            SecurityEventType.LOGIN_SUCCESS, context, {"user_id": str(user.id)}
        )
    ...
    async def create_session(
        self, user: User, context: SecurityContext, session: AsyncSession
    ) -> Dict[str, str]:
    ...
    async def change_password(
        ...
        is_valid, validation_errors = self.password_policy.validate_password(
    ...
    def get_security_metrics(self) -> Dict[str, Any]:
        ...
        recent_threats = [
            t for t in self.threat_detector.detected_threats if t.detected_at > last_24h
        ]

        threat_counts = {}
        for threat in recent_threats:
            threat_counts[threat.threat_type] = (
                threat_counts.get(threat.threat_type, 0) + 1
            )
        ...
        return {
            ...
            "critical_threats": len(
                [t for t in recent_threats if t.threat_level == ThreatLevel.CRITICAL]
            ),
            "high_threats": len(
                [t for t in recent_threats if t.threat_level == ThreatLevel.HIGH]
            ),
            "active_sessions": 0,  # Would count from session storage
            "failed_logins_24h": len(
                [t for t in recent_threats if t.threat_type == "login_failure"]
            ),
...
async def authenticate_user(
    ...
    """Authenticate user"""
    context = SecurityContext(ip_address=ip_address, user_agent=user_agent)
    return await security_manager.authenticate_user(
        username, password, context, session
    )


async def authorize_user_access(
    user_id: UUID, resource: str, permission: str, session: AsyncSession
) -> bool:
    """Check user authorization"""
    context = SecurityContext(user_id=user_id)
    return await security_manager.authorize_access(
        user_id, resource, permission, context, session
    )
