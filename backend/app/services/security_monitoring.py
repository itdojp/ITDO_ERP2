...
class SecurityThreat:
    ...
    def __init__(
        self, threat_type: str, severity: str, description: str, details: Dict[str, Any]
    ):
...
class SecurityMonitoringService:
    ...
    async def monitor_failed_logins(
        ...
        if isinstance(self.db, AsyncSession):
            # Async query
            query = (
                select(func.count(AuditLog.id), AuditLog.user_id, AuditLog.ip_address)
                .where(
                    and_(
    ...
    async def monitor_bulk_data_access(
        ...
        if isinstance(self.db, AsyncSession):
            query = (
                select(func.count(AuditLog.id), AuditLog.user_id)
                .where(
                    and_(
        ...
        for count, access_user_id in access_counts:
            if count >= self.BULK_ACCESS_THRESHOLD:
                severity = (
                    ...
                    if count >= self.BULK_ACCESS_THRESHOLD * 3
    ...
    async def monitor_privilege_escalation(
        ...
        if isinstance(self.db, AsyncSession):
            query = select(AuditLog).where(
                and_(
                    AuditLog.action.in_(
                        ["permission_grant", "role_change", "admin_access"]
                    ),
                    AuditLog.created_at >= time_threshold,
                )
            )
            ...
            if len(logs) >= 2:  # Multiple privilege changes in 24h
                threats.append(
                    SecurityThreat(
                        ...
                        description="Multiple privilege escalations detected",
                        details={
    ...
    async def monitor_unusual_access_patterns(
        self, user_id: Optional[int] = None
    ) -> List[SecurityThreat]:
        ...
        if isinstance(self.db, AsyncSession):
            query = (
                select(AuditLog.user_id, func.count(func.distinct(AuditLog.ip_address)))
                .where(
                    and_(
            ...
            if distinct_ips >= 3:  # Login from 3+ different IPs in 1 hour
                threats.append(
                    SecurityThreat(
                        ...
                        description="Multiple IP addresses used for login",
                        details={
    ...
    async def get_security_dashboard(
        ...
        if isinstance(self.db, AsyncSession):
            ...
            if organization_id:
                success_logins_query = success_logins_query.where(
                    AuditLog.organization_id == organization_id
                )
            ...
            "metrics": {
                ...
                "login_success_rate": (
                    (
                        ...
                        / (successful_logins_today + failed_logins_today)
                        * 100
                    )
    ...
    async def generate_security_report(
        ...
        return {
            ...
            "top_activities": {
                ...
                "resources": dict(
                    sorted(resource_access.items(), key=lambda x: x[1], reverse=True)[
                        :10
                    ]
                ),
            },
            "security_insights": {
                ...
                "privilege_changes": action_counts.get("permission_grant", 0)
                + action_counts.get("role_change", 0),
                "data_exports": action_counts.get("export", 0)
                + action_counts.get("download", 0),
            },
        }
