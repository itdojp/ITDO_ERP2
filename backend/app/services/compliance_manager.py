...
from app.models.compliance import ComplianceCheck, DataRetentionPolicy
...
class DataRetentionRule(BaseComplianceRule):
    ...
    async def check_compliance(
        ...
        classification = context.metadata.get(
...
class ComplianceManager:
    ...
    def _register_default_rules(self) -> dict:
        ...
        # HIPAA rules
        hipaa_rules = [
            HIPAAPrivacyRule(uuid4(), "Privacy Rule", ComplianceStandard.HIPAA, {})
        ]
        ...
        retention_rules = [
            DataRetentionRule(uuid4(), "Data Retention", ComplianceStandard.CUSTOM, {})
        ]
    ...
    async def create_remediation_plan(
        self, violations: List[ComplianceCheckResult], priority: str = "severity"
    ) -> Dict[str, Any]:
        ...
        for violation in violations:
            ...
            task = {
                ...
                "priority": severity_order.get(violation.severity, 1)
                if priority == "severity"
                else 1,
            }
        ...
        return {
            "remediation_plan": {
                ...
                "total_estimated_effort": sum(
                    self._estimate_remediation_effort(v) for v in violations
                ),
    ...
    async def check_data_retention_compliance(
        self, entity_type: str, session: AsyncSession
    ) -> List[Dict[str, Any]]:
    ...
    def _generate_recommendations(self, checks: List[ComplianceCheck]) -> List[str]:
        ...
        for check in checks:
            if check.status == ComplianceStatus.NON_COMPLIANT:
                ...
                common_violations[violation_type] = (
                    common_violations.get(violation_type, 0) + 1
                )
        ...
        for violation_type, count in common_violations.items():
            if count > 5:  # Threshold for pattern
                recommendations.append(
                    f"Address recurring {violation_type} violations ({count} instances)"
                )

        # Add general recommendations
        if len([c for c in checks if c.severity == ViolationSeverity.CRITICAL]) > 0:
            recommendations.append(
                "Prioritize critical severity violations for immediate remediation"
            )
    ...
    async def _log_compliance_check(
        self, context: ComplianceContext, results: List[ComplianceCheckResult]
    ):
...
async def create_remediation_plan(
    violations: List[ComplianceCheckResult], priority: str = "severity"
) -> Dict[str, Any]:
...
def register_custom_compliance_rule(
    rule: BaseComplianceRule, standard: ComplianceStandard = ComplianceStandard.CUSTOM
):
    """Register custom compliance rule"""
