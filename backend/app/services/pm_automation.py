...
class PMAutomationService:
    ...
    async def auto_assign_tasks(
        ...
        if assignment_strategy == "balanced":
            assignments = await self._balanced_assignment(unassigned_tasks, team_members)
        elif assignment_strategy == "skill_based":
            assignments = await self._skill_based_assignment(unassigned_tasks, team_members)
        elif assignment_strategy == "workload_based":
            assignments = await self._workload_based_assignment(unassigned_tasks, team_members)
        else:
    ...
    async def generate_progress_report(
        ...
        trends = await self._calculate_completion_trends(project_id, start_date, end_date)
    ...
    async def _create_agile_template(self, project_id: int, user: User) -> Dict[str, Any]:
        """Create agile project template."""
        tasks = [
            {"title": "プロジェクト計画", "priority": "high", "estimated_hours": 8},
            {"title": "ユーザーストーリー作成", "priority": "high", "estimated_hours": 16},
            {"title": "スプリント計画", "priority": "medium", "estimated_hours": 4},
            {"title": "開発環境セットアップ", "priority": "high", "estimated_hours": 8},
            {"title": "CI/CDパイプライン構築", "priority": "medium", "estimated_hours": 12},
            {"title": "スプリント1実行", "priority": "high", "estimated_hours": 80},
            {"title": "レビューとレトロスペクティブ", "priority": "medium", "estimated_hours": 4},
        ]
    ...
    async def _create_waterfall_template(self, project_id: int, user: User) -> Dict[str, Any]:
    ...
    async def _create_kanban_template(self, project_id: int, user: User) -> Dict[str, Any]:
    ...
    async def _calculate_completion_trends(
        ...
        return {
            ...
            "average_daily_completion": sum(daily_completions.values()) / len(daily_completions) if daily_completions else 0
        }
    ...
    async def _predict_risk_probability(self, project_id: int) -> Dict[str, Any]:
        ...
        risk_score = len([r for r in risks if r["severity"] == "high"]) * 0.7 + \
