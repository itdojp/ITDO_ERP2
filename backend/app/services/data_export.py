...
class DataExportService:
    ...
    async def get_export_progress(self, job_id: int) -> Optional[ExportProgressResponse]:
    ...
    async def export_to_pdf(
        ...
        # Add metadata
        metadata_text = f"Generated on: {datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')}<br/>Total Records: {len(data)}"
        ...
        if not data:
            ...
            if len(data) > max_rows:
                note = Paragraph(
                    f"<br/>Note: Only first {max_rows} records shown. Total records: {len(data)}",
                    styles['Italic']
                )
                story.append(note)
