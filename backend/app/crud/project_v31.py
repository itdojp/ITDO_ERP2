...
def create_project(db: Session, project_data: Any) -> ProjectExtended:
    ...
    if hasattr(project_data, "project_manager_id") and project_data.project_manager_id:
        ...
        manager = (
            db.query(User).filter(User.id == project_data.project_manager_id).first()
        )
        if not manager:
    ...
    if not hasattr(project_data, "project_code") or not project_data.project_code:
        ...
        count = (
            db.query(ProjectExtended)
            .filter(ProjectExtended.organization_id == org_id)
            .count()
        )
    ...
    # Validate date logic
    if (
        hasattr(project_data, "planned_start_date")
        and hasattr(project_data, "planned_end_date")
        and project_data.planned_start_date
        and project_data.planned_end_date
    ):
...
def get_project(db: Session, project_id: str) -> Optional[ProjectExtended]:
    ...
    return (
        ...
        .filter(ProjectExtended.id == project_id)
        .first()
    )
...
def get_projects(
    ...
    if filters:
        if "organization_id" in filters:
            query = query.filter(
                ProjectExtended.organization_id == filters["organization_id"]
            )
        if "status" in filters:
            query = query.filter(ProjectExtended.status == filters["status"])
        if "project_manager_id" in filters:
            query = query.filter(
                ProjectExtended.project_manager_id == filters["project_manager_id"]
            )
        ...
        if "project_type" in filters:
            query = query.filter(
                ProjectExtended.project_type == filters["project_type"]
            )
        if "start_date_from" in filters:
            query = query.filter(
                ProjectExtended.planned_start_date >= filters["start_date_from"]
            )
        if "end_date_to" in filters:
            query = query.filter(
                ProjectExtended.planned_end_date <= filters["end_date_to"]
            )
...
def update_project(
    db: Session, project_id: str, project_data: Any
    ...
    if project.tasks:
        ...
        completed_tasks = len(
            [t for t in project.tasks if t.status == TaskStatus.COMPLETED]
        )
        if total_tasks > 0:
...
def create_task(db: Session, task_data: Any) -> TaskExtended:
    ...
    project = (
        db.query(ProjectExtended)
        .filter(ProjectExtended.id == task_data.project_id)
        .first()
    )
    if not project:
    ...
    if not hasattr(task_data, "task_number") or not task_data.task_number:
        count = (
            ...
            .filter(TaskExtended.project_id == task_data.project_id)
            .count()
        )
    ...
    if hasattr(task_data, "parent_task_id") and task_data.parent_task_id:
        parent = (
            db.query(TaskExtended)
            .filter(TaskExtended.id == task_data.parent_task_id)
            .first()
        )
...
def get_task(db: Session, task_id: str) -> Optional[TaskExtended]:
    ...
    return (
        ...
        .filter(TaskExtended.id == task_id)
        .first()
    )
...
def get_tasks(
    ...
    if filters:
        ...
        if "assigned_to_id" in filters:
            query = query.filter(
                TaskExtended.assigned_to_id == filters["assigned_to_id"]
            )
        if "status" in filters:
    ...
    return (
        query.options(
            joinedload(TaskExtended.assigned_to), joinedload(TaskExtended.project)
        )
        .offset(skip)
        .limit(limit)
        .all()
...
def create_task_dependency(db: Session, dependency_data: Any) -> TaskDependencyExtended:
    ...
    task = (
        db.query(TaskExtended)
        .filter(TaskExtended.id == dependency_data.task_id)
        .first()
    )
    dependent_task = (
        db.query(TaskExtended)
        .filter(TaskExtended.id == dependency_data.dependent_task_id)
        .first()
    )
    ...
    if would_create_circular_dependency(
        db, dependency_data.task_id, dependency_data.dependent_task_id
    ):
...
def create_project_resource(db: Session, resource_data: Any) -> ProjectResource:
    ...
    project = (
        db.query(ProjectExtended)
        .filter(ProjectExtended.id == resource_data.project_id)
        .first()
    )
    if not project:
        raise ValueError("Project not found")
    ...
    if resource_data.allocation_percentage > 80:
        overlapping = (
            ...
            .filter(
                ...
                ProjectResource.start_date <= resource_data.end_date
                if resource_data.end_date
                else True,
                or_(
        ...
        if overlapping:
            raise ValueError(
                "User is already allocated to another project during this period"
            )
...
def get_project_resources(
    db: Session, project_id: str, active_only: bool = True
...
def update_resource_allocation(
    db: Session, resource_id: str, resource_data: Any
) -> Optional[ProjectResource]:
    """Update resource allocation."""
    resource = (
        db.query(ProjectResource).filter(ProjectResource.id == resource_id).first()
    )
    if not resource:
        return None
...
def create_time_entry(db: Session, time_data: Any) -> TimeEntry:
    ...
    project = (
        db.query(ProjectExtended)
        .filter(ProjectExtended.id == time_data.project_id)
        .first()
    )
    if not project:
        raise ValueError("Project not found")
...
def approve_time_entry(
    db: Session, entry_id: str, approver_id: str
) -> Optional[TimeEntry]:
    """Approve time entry."""
...
def update_risk_status(
    db: Session, risk_id: str, status: RiskStatus
) -> Optional[ProjectRisk]:
    """Update risk status."""
...
def get_project_dashboard_metrics(db: Session, project_id: str) -> Dict[str, Any]:
    ...
    # Task metrics
    total_tasks = (
        db.query(TaskExtended).filter(TaskExtended.project_id == project_id).count()
    )
    completed_tasks = (
        db.query(TaskExtended)
        .filter(
    ...
    billable_hours = (
        db.query(func.sum(TimeEntry.hours))
        .filter(TimeEntry.project_id == project_id, TimeEntry.is_billable)
        .scalar()
        or 0
    )
    ...
    # Budget metrics
    total_cost = (
        db.query(func.sum(TimeEntry.billing_amount))
        .filter(TimeEntry.project_id == project_id, TimeEntry.is_billable)
        .scalar()
        or 0
    )
    ...
    return {
        ...
        "budget_utilization": (float(total_cost) / float(project.total_budget) * 100)
        ...
        "is_on_schedule": project.planned_end_date >= date.today()
        if project.planned_end_date
        else True,
...
def get_organization_project_summary(
    db: Session, organization_id: str
) -> Dict[str, Any]:
    """Get organization-wide project summary."""
    # Project status distribution
    status_counts = (
        db.query(ProjectExtended.status, func.count(ProjectExtended.id))
        .filter(
            ProjectExtended.organization_id == organization_id,
            ProjectExtended.is_active,
        )
        .group_by(ProjectExtended.status)
        .all()
    )
    ...
    # Budget summary
    total_budget = (
        db.query(func.sum(ProjectExtended.total_budget))
        .filter(
            ProjectExtended.organization_id == organization_id,
            ProjectExtended.is_active,
        )
    ...
    actual_cost = (
        db.query(func.sum(ProjectExtended.actual_cost))
        .filter(
            ProjectExtended.organization_id == organization_id,
            ProjectExtended.is_active,
        )
    ...
    return {
        ...
        "budget_utilization": (float(actual_cost) / float(total_budget) * 100)
        if total_budget > 0
        else 0,
...
def calculate_project_health_score(db: Session, project_id: str) -> Dict[str, Any]:
    ...
    if project.planned_end_date:
        days_total = (
            (project.planned_end_date - project.planned_start_date).days
            if project.planned_start_date
            else 365
        )
        days_remaining = (project.planned_end_date - date.today()).days
        schedule_health = (
            min(max((days_remaining / days_total) * 100, 0), 100)
            if days_total > 0
            else 50
        )
    else:
        ...
        if factor_data["score"] < 60:
            if factor_name == "schedule":
                ...
                recommendations.append(
                    "Review budget allocation and cost control measures"
                )
