...
class WorkflowService:
    ...
    async def create_workflow_definition(
        self, db: Session, definition_data: Dict[str, Any]
    ) -> WorkflowDefinition:
        ...
        try:
            ...
            existing = (
                ...
                .filter(WorkflowDefinition.code == definition_data["code"])
                .first()
            )
            if existing:
                raise ValueError(
                    f"Workflow definition with code '{definition_data['code']}' already exists"
                )
    ...
    async def get_workflow_definition(
        self, db: Session, definition_id: str
    ...
    async def update_workflow_definition(
        self, db: Session, definition_id: str, update_data: Dict[str, Any]
    ) -> Optional[WorkflowDefinition]:
    ...
    async def start_workflow_instance(
        self, db: Session, definition_id: str, instance_data: Dict[str, Any]
    ) -> WorkflowInstance:
        ...
        try:
            ...
            instance = WorkflowInstance(
                ...
                or f"{definition.name} - {instance_number}",
            ...
            if definition.sla_minutes:
                instance.sla_deadline = datetime.now() + timedelta(
    ...
    async def list_workflow_instances(
        ...
        instances = (
            query.order_by(desc(WorkflowInstance.created_at))
    ...
    async def advance_workflow_instance(
        self, db: Session, instance_id: str, task_id: str, action_data: Dict[str, Any]
    ) -> WorkflowInstance:
        ...
        try:
            ...
            next_step = await self._determine_next_step(
                db, instance, task.step_id, action_data
            )
    ...
    async def create_workflow_task(
        self, db: Session, task_data: Dict[str, Any]
    ) -> WorkflowTask:
        ...
        try:
            ...
            if task_data.get("estimated_duration_minutes"):
                task.due_date = datetime.now() + timedelta(
                    minutes=task_data["estimated_duration_minutes"]
                )
    ...
    async def assign_task(
        self, db: Session, task_id: str, assignee_id: str, assigned_by: str
    ) -> WorkflowTask:
        ...
        await self._log_workflow_activity(
            ...
            ActionType.COMPLETE,
            "assign_task",
            f"Task assigned to user {assignee_id}",
            assigned_by,
    ...
    async def delegate_task(
        ...
        await self._log_workflow_activity(
            ...
            ActionType.DELEGATE,
            "delegate_task",
            f"Task delegated to {delegate_to}. Reason: {reason or 'Not specified'}",
            delegated_by,
        )
    ...
    async def escalate_task(
        self, db: Session, task_id: str, escalate_to: str, escalation_reason: str
    ) -> WorkflowTask:
        ...
        instance = (
            db.query(WorkflowInstance)
            .filter(WorkflowInstance.id == task.instance_id)
            .first()
        )
        if instance:
        ...
        await self._log_workflow_activity(
            ...
            ActionType.ESCALATE,
            "escalate_task",
            f"Task escalated to {escalate_to}. Reason: {escalation_reason}",
            None,
        )
    ...
    async def add_task_comment(
        self, db: Session, task_id: str, comment_data: Dict[str, Any]
    ) -> WorkflowComment:
        ...
        try:
            ...
            if not comment.parent_comment_id:
                ...
                parent = (
                    ...
                    .filter(WorkflowComment.id == comment.parent_comment_id)
                    .first()
                )
                if parent:
    ...
    async def add_task_attachment(
        self, db: Session, task_id: str, attachment_data: Dict[str, Any]
    ) -> WorkflowAttachment:
    ...
    async def generate_workflow_analytics(
        ...
        try:
            ...
            analytics.completed_instances = len(
                [i for i in instances if i.status == WorkflowInstanceStatus.COMPLETED]
            )
            analytics.failed_instances = len(
                [i for i in instances if i.status == WorkflowInstanceStatus.FAILED]
            )
            analytics.cancelled_instances = len(
                [i for i in instances if i.status == WorkflowInstanceStatus.CANCELLED]
            )
            analytics.active_instances = len(
                [i for i in instances if i.status == WorkflowInstanceStatus.RUNNING]
            )
            ...
            completed_instances = [
                i
                for i in instances
                if i.status == WorkflowInstanceStatus.COMPLETED and i.duration_minutes
            ]
            if completed_instances:
                durations = [i.duration_minutes for i in completed_instances]
                analytics.average_completion_time_hours = Decimal(
                    str(sum(durations) / len(durations) / 60)
                )
                analytics.median_completion_time_hours = Decimal(
                    str(sorted(durations)[len(durations) // 2] / 60)
                )
            ...
            if analytics.total_instances > 0:
                analytics.sla_compliance_rate = Decimal(
                    str((1 - sla_breaches / analytics.total_instances) * 100)
                )
            ...
            analytics.completed_tasks = len(
                [t for t in tasks if t.status == TaskStatus.COMPLETED]
            )
            analytics.overdue_tasks = len(
                [
                    t
                    for t in tasks
                    if t.due_date
            ...
            # Quality metrics
            approved_tasks = len(
                [t for t in tasks if t.completion_result == "approved"]
            )
            rejected_tasks = len(
                [t for t in tasks if t.completion_result == "rejected"]
            )
            if analytics.completed_tasks > 0:
                analytics.approval_rate = Decimal(
                    str(approved_tasks / analytics.completed_tasks * 100)
                )
                analytics.rejection_rate = Decimal(
                    str(rejected_tasks / analytics.completed_tasks * 100)
                )
    ...
    async def get_workflow_performance_dashboard(
        self, db: Session, organization_id: str, period_days: int = 30
    ) -> Dict[str, Any]:
        ...
        if completed_instances:
            ...
            avg_completion_time = (
                total_time / len(completed_instances) / 60
            )  # Convert to hours
    ...
    async def _create_steps_from_schema(
        ...
        for i, step_config in enumerate(steps_config):
            step = WorkflowStep(
                ...
                estimated_duration_minutes=step_config.get(
    ...
    async def _generate_instance_number(
        self, db: Session, definition: WorkflowDefinition
    ) -> str:
        """Generate unique instance number."""
        count = (
            db.query(WorkflowInstance)
            .filter(WorkflowInstance.definition_id == definition.id)
            .count()
        )
    ...
    async def _start_first_step(
        self, db: Session, instance: WorkflowInstance, definition: WorkflowDefinition
    ):
        ...
        if first_step:
            ...
            # Create first task
            await self.create_workflow_task(
                db,
                {
    ...
    async def _complete_task(
        self, db: Session, task: WorkflowTask, action_data: Dict[str, Any]
    ):
    ...
    async def _determine_next_step(
        ...
        current_step = (
            db.query(WorkflowStep).filter(WorkflowStep.id == current_step_id).first()
        )
        if not current_step:
            return None
        ...
        if len(next_step_ids) == 1:
            return (
                db.query(WorkflowStep)
                .filter(WorkflowStep.id == next_step_ids[0])
                .first()
            )

        # Multiple next steps - apply decision logic
        return await self._apply_decision_logic(
            db, current_step, next_step_ids, action_data
        )
    ...
    async def _apply_decision_logic(
        ...
        return (
            db.query(WorkflowStep).filter(WorkflowStep.id == next_step_ids[0]).first()
        )

    async def _start_workflow_step(
        self, db: Session, instance: WorkflowInstance, step: WorkflowStep
    ):
    ...
    async def _complete_workflow_instance(
        self, db: Session, instance: WorkflowInstance, action_data: Dict[str, Any]
    ):
    ...
    async def _update_instance_progress(self, db: Session, instance: WorkflowInstance) -> dict:
        ...
        if instance.total_steps > 0:
            instance.progress_percentage = Decimal(
                str((completed_tasks / instance.total_steps) * 100)
            )
    ...
    async def _log_workflow_activity(
        ...
        activity = WorkflowActivity(
            organization_id=db.query(WorkflowInstance)
            .filter(WorkflowInstance.id == instance_id)
            .first()
            .organization_id,
    ...
    async def _send_task_notification(
        self, db: Session, task: WorkflowTask, notification_type: str
    ):
    ...
    async def _create_new_version(
        ...
        new_version = WorkflowDefinition(
            **{
                **{
                    k: v
                    for k, v in current_definition.__dict__.items()
