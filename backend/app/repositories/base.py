...
from app.types import CreateSchemaType, ModelType, SearchParams, UpdateSchemaType
...
class BaseRepository(Generic[ModelType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_ids(self, ids: List[int]) -> List[ModelType]:
        ...
        return list(
            self.db.scalars(
                select(self.model).where(getattr(self.model, "id").in_(ids))
            )
        )
    ...
    def search(self, params: SearchParams) -> tuple[List[ModelType], int]:
        ...
        if params.query and hasattr(self.model, "name"):
            query = query.where(getattr(self.model, "name").ilike(f"%{params.query}%"))
    ...
    def bulk_upsert(
        ...
        for obj_in in objs_in:
            ...
            if conditions:
                existing = self.db.scalar(select(self.model).where(and_(*conditions)))
