...
class DepartmentRepository(
    ...
    def get_tree(
        ...
        # Order by depth first if available, then display order
    ...
    def get_children(
        ...
        # All descendants
        parent = self.get(parent_id)
        if not parent or not hasattr(parent, 'path') or not parent.path:
            return []
    ...
    def move_department(
        ...
        if new_parent_id:
            ...
            if hasattr(new_parent, 'path') and new_parent.path and str(department_id) in new_parent.path:
                return False

        # Update parent
        dept.parent_id = new_parent_id
        if hasattr(dept, 'update_path'):
            dept.update_path()

        # Update all descendants' paths
        if hasattr(dept, 'update_subtree_paths'):
    ...
    def get_ancestors(self, department_id: DepartmentId) -> List[Department]:
        ...
        dept = self.get(department_id)
        if not dept or not hasattr(dept, 'path') or not dept.path:
            return []
