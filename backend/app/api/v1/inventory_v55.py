...
class InventoryItemCreate(BaseModel):
    ...
    def validate_maximum_stock(cls, v, values) -> dict:
    ...
    def validate_reserved_quantity(cls, v, values) -> dict:
...
class InventoryMovementCreate(BaseModel):
    ...
    def validate_quantity(cls, v, values) -> dict:
        ...
        if (
            movement_type in [MovementType.OUT, MovementType.DAMAGE, MovementType.LOSS]
            and v > 0
        ):
            raise ValueError(f"Quantity must be negative for {movement_type} movements")
        elif (
            movement_type in [MovementType.IN, MovementType.FOUND, MovementType.RETURN]
            and v < 0
        ):
...
async def validate_location_exists(db: AsyncSession, location_id: UUID) -> bool:
    ...
    result = await db.execute(
        select(InventoryLocation).where(
            and_(InventoryLocation.id == location_id, InventoryLocation.is_active)
        )
    )
...
async def create_stock_alert(
    db: AsyncSession, item: InventoryItem, alert_type: AlertType, message: str
) -> None:
...
async def check_and_create_alerts(db: AsyncSession, item: InventoryItem) -> None:
    ...
    # Check for out of stock
        ...
        await create_stock_alert(
            db, item, AlertType.OUT_OF_STOCK, "Item is out of stock"
        )
    ...
    if item.expiry_date:
        ...
        if days_until_expiry <= 0:
            await create_stock_alert(
                db, item, AlertType.EXPIRED, f"Item has expired on {item.expiry_date}"
            )
...
async def create_location(
    ...
    if location.is_default:
        ...
        await db.execute(update(InventoryLocation).values(is_default=False))
    ...
    for location in locations:
        if include_stats:
            ...
            if location.capacity and location.total_items > 0:
                location.utilization_percentage = round(
                    (location.total_items / location.capacity) * 100, 2
                )
            else:
...
async def get_location(
    ...
    value_result = await db.execute(
        select(func.sum(InventoryItem.quantity * InventoryItem.cost_per_unit)).where(
            and_(
                InventoryItem.location_id == location.id,
                InventoryItem.cost_per_unit.isnot(None),
            )
...
async def delete_location(
    location_id: UUID = Path(..., description="Location ID"),
    force: bool = Query(
        False, description="Force delete even if inventory items exist"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    await db.execute(
        delete(InventoryItem).where(InventoryItem.location_id == location_id)
    )
    await db.execute(
        delete(InventoryLocation).where(InventoryLocation.id == location_id)
    )
...
async def list_inventory_items(
    ...
    product_variant_id: Optional[UUID] = Query(
        None, description="Filter by product variant"
    ),
    low_stock_only: bool = Query(False, description="Show only low stock items"),
    expired_only: bool = Query(False, description="Show only expired items"),
    search: Optional[str] = Query(
        None, min_length=1, description="Search in product name or SKU"
    ),
    sort_by: str = Query(
        "updated_at", regex="^(name|quantity|expiry_date|updated_at)$"
    ),
    ...
    if search:
        ...
        query = query.where(
            or_(Product.name.ilike(search_term), ProductVariant.sku.ilike(search_term))
        )
    ...
    for item in items:
        ...
        total_value = (
            (item.quantity * item.cost_per_unit) if item.cost_per_unit else Decimal("0")
        )
...
async def get_inventory_item(
    ...
    result = await db.execute(
        ...
        .options(
            joinedload(InventoryItem.product_variant).joinedload(
...
async def get_inventory_analytics(
    ...
    movement_stats = {"by_type": {}, "total_movements": 0, "total_quantity_moved": 0}
    ...
    # Active alerts count
    alerts_query = select(func.count(StockAlert.id)).where(
        StockAlert.status == AlertStatus.ACTIVE
    )
    if location_id:
        alerts_query = alerts_query.join(InventoryItem).where(
            InventoryItem.location_id == location_id
        )
    ...
    top_products_query = (
        select(
            Product.name,
            ProductVariant.sku,
            func.sum(InventoryItem.quantity * InventoryItem.cost_per_unit).label(
                "total_value"
            ),
        )
        .join(ProductVariant)
    ...
    top_products_query = (
        top_products_query.group_by(Product.name, ProductVariant.sku)
...
async def bulk_inventory_adjustment(
    ...
    for adj_data in adjustment.adjustments:
        try:
            ...
            if not item:
                failed_items.append(
                    {"item_id": str(item_id), "error": "Item not found"}
                )
                continue
            ...
            failed_items.append(
                {"item_id": adj_data.get("item_id", "unknown"), "error": str(e)}
            )
