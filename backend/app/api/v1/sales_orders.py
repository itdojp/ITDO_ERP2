...
async def confirm_sales_order(order_id: str):
    ...
    if order["status"] != OrderStatus.DRAFT:
        raise HTTPException(status_code=400, detail="確定できるのは下書き状態のオーダーのみです")
    
    order["status"] = OrderStatus.CONFIRMED
    order["updated_at"] = datetime.utcnow()
    
    return {"message": f"オーダー {order['order_number']} が確定されました", "order": order}
...
async def ship_sales_order(order_id: str, tracking_number: Optional[str] = None):
    ...
    if order["status"] not in [OrderStatus.CONFIRMED, OrderStatus.PROCESSING]:
        raise HTTPException(status_code=400, detail="出荷できるのは確定済みまたは処理中のオーダーのみです")
    ...
    return {"message": f"オーダー {order['order_number']} が出荷されました", "tracking_number": tracking_number}
...
async def cancel_sales_order(order_id: str, reason: Optional[str] = None):
    ...
    if order["status"] in [OrderStatus.SHIPPED, OrderStatus.DELIVERED]:
        raise HTTPException(status_code=400, detail="出荷済みまたは配達済みのオーダーはキャンセルできません")
    ...
    return {"message": f"オーダー {order['order_number']} がキャンセルされました", "reason": reason}
