    ...
    async def update_service_health(self, service_id: str, status: ServiceStatus, 
        ...
        try:
            ...
            if status == ServiceStatus.UNHEALTHY:
                current_failures = await self.redis.hget(service_key, "consecutive_failures")
...
class LoadBalancer:
    ...
    async def select_service(self, service_name: str, strategy: RoutingStrategy, 
    ...
    def _round_robin_selection(self, service_name: str, services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _weighted_round_robin_selection(self, services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _least_connections_selection(self, services: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _ip_hash_selection(self, services: List[Dict[str, Any]], client_ip: str) -> Dict[str, Any]:
...
class CircuitBreaker:
    ...
    async def get_state(self, service_name: str, route_path: str) -> CircuitBreakerState:
    ...
    async def record_failure(self, service_name: str, route_path: str) -> CircuitBreakerState:
            ...
            if state == CircuitBreakerState.CLOSED:
                ...
                if next_attempt:
                    ...
                    if datetime.utcnow() >= next_attempt_time:
                        await self.redis.hset(key, "state", CircuitBreakerState.HALF_OPEN.value)
                        return True
...
class RateLimiter:
    ...
    async def is_allowed(self, key: str, limit: int, window: int = 60) -> tuple[bool, Dict[str, Any]]:
...
class APIGatewayCore:
    ...
    async def proxy_request(self, request: Request, route_config: Dict[str, Any]) -> Response:
        """Proxy request to backend service"""
        try:
            service_name = route_config["service_name"]
            
            # Check circuit breaker
            if not await self.circuit_breaker.can_execute(service_name, request.url.path):
            ...
            try:
                ...
                # Record success
                await self.circuit_breaker.record_success(service_name, request.url.path)
                ...
                # Record failure
                await self.circuit_breaker.record_failure(service_name, request.url.path)
                await self.service_registry.update_service_health(
...
class GatewayMetrics:
    ...
    async def record_request(self, route: str, method: str, status_code: int, 
                           response_time: float, service_name: str) -> None:
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            successful_requests = await self.redis.get("gateway:successful_requests") or 0
            ...
            rps = 0
            for i in range(60):
                minute_requests = await self.redis.get(f"requests:{current_minute - i}") or 0
...
async def configure_route(
    request: RouteConfigurationRequest,
    db: AsyncSession = Depends(get_db)
):
    ...
    try:
        ...
        await db.execute(route_query, {
            ...
            "rate_limit": json.dumps(request.rate_limit) if request.rate_limit else None,
...
async def create_security_policy(
    ...
    try:
        ...
        await db.execute(policy_query, {
            ...
            "rate_limits": json.dumps({k.value: v for k, v in request.rate_limits.items()}),
                    ...
                    async with httpx.AsyncClient(timeout=10.0) as client:
                        ...
                        if response.status_code == 200:
                            await gateway_core.service_registry.update_service_health(
                                service["service_id"], ServiceStatus.HEALTHY, 
