...
from app.models.testing import TestSuite, TestCase, TestExecution, TestResult, TestPlan
from app.models.user import User
from app.services.test_automation_engine import (
    test_automation_engine, TestType, TestStatus, TestPriority,
    TestEnvironment, TestResult as EngineTestResult, TestSuiteResult,
...
class TestPlanRequest(BaseModel):
    ...
    schedule_expression: Optional[str] = Field(None, description="Cron expression for scheduled execution")
    test_environment: TestEnvironment = TestEnvironment.TESTING
...
# Test Execution
@router.post("/execute", response_model=TestExecutionResponse, status_code=status.HTTP_201_CREATED)
...
# API Test Creation
@router.post("/api-tests", response_model=TestCaseResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/performance-tests", response_model=TestCaseResponse, status_code=status.HTTP_201_CREATED)
...
async def get_coverage_report(
    ...
    coverage_report = await test_automation_engine.test_runner.generate_coverage_report()
...
async def get_test_statistics(
    ...
    execution_stats = await db.execute(
        select(
            ...
            func.count(TestExecution.id).filter(TestExecution.status == 'completed').label('completed_executions'),
            func.count(TestExecution.id).filter(TestExecution.status == 'failed').label('failed_executions'),
    ...
    result_stats = await db.execute(
        select(
            ...
            func.count(TestResult.id).filter(TestResult.status == TestStatus.PASSED).label('passed_results'),
            func.count(TestResult.id).filter(TestResult.status == TestStatus.FAILED).label('failed_results'),
    ...
    return {
        ...
        'test_results': {
            ...
            'success_rate': (passed_results / total_test_results * 100) if total_test_results > 0 else 0,
...
async def _run_ci_pipeline():
    ...
    try:
        ...
        from app.services.test_automation_engine import initialize_test_suites, run_ci_pipeline
