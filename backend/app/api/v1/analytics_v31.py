...
@router.post(
    "/data-sources",
    response_model=AnalyticsDataSourceResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_data_source(
    request: AnalyticsDataSourceCreateRequest, db: Session = Depends(get_db)
):
...
@router.get(
    "/data-sources/{data_source_id}", response_model=AnalyticsDataSourceResponse
)
...
async def update_data_source(
    ...
    try:
        data_source = await analytics_service.update_data_source(
            db, data_source_id, request.dict(exclude_unset=True)
        )
        if not data_source:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Data source not found"
...
@router.post(
    "/data-sources/{data_source_id}/test-connection", response_model=Dict[str, Any]
)
async def test_data_source_connection(
    ...
    try:
        result = await analytics_service.test_data_source_connection(
            db, data_source_id, request.dict()
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def sync_data_source(
    data_source_id: str, request: DataSourceSyncRequest, db: Session = Depends(get_db)
):
    """Sync data from an analytics data source."""
    try:
        result = await analytics_service.sync_data_source(
            db, data_source_id, request.dict()
        )
        return result
    except ValueError as e:
...
async def create_metric(
    request: AnalyticsMetricCreateRequest, db: Session = Depends(get_db)
):
...
async def update_metric(
    metric_id: str, request: AnalyticsMetricUpdateRequest, db: Session = Depends(get_db)
):
    """Update an analytics metric."""
    try:
        metric = await analytics_service.update_metric(
            db, metric_id, request.dict(exclude_unset=True)
        )
        if not metric:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Metric not found"
...
async def calculate_metric(
    metric_id: str, request: MetricCalculationRequest, db: Session = Depends(get_db)
):
...
async def compare_metrics(
    request: MetricComparisonRequest, db: Session = Depends(get_db)
):
...
@router.post(
    "/data-points",
    response_model=AnalyticsDataPointResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_data_point(
    request: AnalyticsDataPointCreateRequest, db: Session = Depends(get_db)
):
...
async def query_data_points(
    request: DataPointQueryRequest, db: Session = Depends(get_db)
):
...
async def get_data_point(data_point_id: str, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get data point: {str(e)}",
        )
...
async def create_dashboard(
    request: AnalyticsDashboardCreateRequest, db: Session = Depends(get_db)
):
...
async def update_dashboard(
    ...
    try:
        dashboard = await analytics_service.update_dashboard(
            db, dashboard_id, request.dict(exclude_unset=True)
        )
        if not dashboard:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Dashboard not found"
...
async def export_dashboard(
    dashboard_id: str, request: DashboardExportRequest, db: Session = Depends(get_db)
):
    """Export dashboard to various formats."""
    try:
        result = await analytics_service.export_dashboard(
            db, dashboard_id, request.dict()
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
async def create_report(
    request: AnalyticsReportCreateRequest, db: Session = Depends(get_db)
):
...
async def list_reports(
    ...
    is_scheduled: Optional[bool] = Query(
        None, description="Filter by scheduled status"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    per_page: int = Query(50, ge=1, le=500, description="Items per page"),
    db: Session = Depends(get_db),
...
async def update_report(
    report_id: str, request: AnalyticsReportUpdateRequest, db: Session = Depends(get_db)
):
    """Update an analytics report."""
    try:
        report = await analytics_service.update_report(
            db, report_id, request.dict(exclude_unset=True)
        )
        if not report:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Report not found"
...
async def generate_report(
    report_id: str, request: ReportGenerationRequest, db: Session = Depends(get_db)
):
    """Generate analytics report execution."""
    try:
        execution = await analytics_service.generate_report(
            db, report_id, request.dict()
        )
        return execution
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
@router.get(
    "/reports/{report_id}/executions", response_model=List[ReportExecutionResponse]
)
async def list_report_executions(
    ...
    try:
        executions = await analytics_service.list_report_executions(
            db, report_id, page, per_page
        )
        return executions
    except Exception as e:
        raise HTTPException(
...
@router.post(
    "/alerts",
    response_model=AnalyticsAlertResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_alert(
    request: AnalyticsAlertCreateRequest, db: Session = Depends(get_db)
):
...
async def list_alerts(
    ...
    is_active: Optional[bool] = Query(None, description="Filter by active status"),
    is_triggered: Optional[bool] = Query(
        None, description="Filter by triggered status"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    per_page: int = Query(50, ge=1, le=500, description="Items per page"),
    db: Session = Depends(get_db),
...
async def update_alert(
    alert_id: str, request: AnalyticsAlertUpdateRequest, db: Session = Depends(get_db)
):
    """Update an analytics alert."""
    try:
        alert = await analytics_service.update_alert(
            db, alert_id, request.dict(exclude_unset=True)
        )
        if not alert:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Alert not found"
...
async def evaluate_alert(
    alert_id: str, request: AlertEvaluationRequest, db: Session = Depends(get_db)
):
...
@router.post(
    "/predictions",
    response_model=AnalyticsPredictionResponse,
    status_code=status.HTTP_201_CREATED,
)
async def create_prediction(
    request: AnalyticsPredictionCreateRequest, db: Session = Depends(get_db)
):
...
async def update_prediction(
    ...
    try:
        prediction = await analytics_service.update_prediction(
            db, prediction_id, request.dict(exclude_unset=True)
        )
        if not prediction:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Prediction not found"
...
async def train_prediction_model(
    ...
    try:
        result = await analytics_service.train_prediction_model(
            db, prediction_id, request.dict()
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to train prediction model: {str(e)}",
        )
...
async def get_prediction_forecast(
    ...
    try:
        result = await analytics_service.get_prediction_forecast(
            db, prediction_id, days_ahead
        )
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get prediction forecast: {str(e)}",
        )
...
async def generate_insights(
    request: InsightGenerationRequest, db: Session = Depends(get_db)
):
...
async def execute_analytics_query(
    request: AnalyticsQueryRequest, db: Session = Depends(get_db)
):
...
async def get_analytics_health(db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Failed to get analytics health: {str(e)}",
        )
