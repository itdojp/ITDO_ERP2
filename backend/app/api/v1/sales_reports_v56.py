...
class SalesReportRequest(BaseModel):
    ...
    metrics: List[str] = Field(
        default_factory=lambda: ["revenue", "quantity", "orders"]
    )
...
class SalesAnalyticsRequest(BaseModel):
    metrics: List[str] = Field(
        default_factory=lambda: ["revenue", "growth_rate", "conversion_rate"]
    )
...
class SalesAnalyticsService:
    ...
    async def generate_revenue_summary(
        self, start_date: date, end_date: date, filters: Dict[str, Any] = None
    ) -> Dict[str, Any]:
        ...
        base_query = (
            ...
            .select_from(Order.__table__.join(OrderItem.__table__))
            .where(
                and_(
        ...
        if filters:
            if "customer_ids" in filters:
                base_query = base_query.where(
                    Order.customer_id.in_(filters["customer_ids"])
                )
        ...
        conversion_rate = (
            (summary.total_orders / total_sessions * 100) if total_sessions > 0 else 0
        )
    ...
    async def get_product_performance(
        ...
        query = (
            ...
            .group_by(Product.id, Product.name, Product.sku, ProductCategory.name)
        )
    ...
    async def get_customer_analysis(
        self, start_date: date, end_date: date, segment_by: str = "value"
        ...
        if segment_by == "value":
            ...
            query = (
                ...
                .select_from(Customer.__table__.join(Order.__table__))
                .where(
                    and_(
                        Order.order_date >= start_date,
                        Order.order_date <= end_date,
        ...
        return [
            CustomerSegmentItem(
                ...
                average_revenue_per_customer=segment.total_revenue
                / segment.customer_count
                if segment.customer_count > 0
                else Decimal("0"),
                average_order_frequency=segment.total_orders / segment.customer_count
                if segment.customer_count > 0
                else 0.0,
            )
    ...
    async def get_trend_analysis(
        ...
        for trend in trends:
            ...
            trend_data.append(
                {
                    "period": trend.period_start.isoformat()
                    if hasattr(trend.period_start, "isoformat")
                    else str(trend.period_start),
...
async def generate_sales_report(
    ...
    try:
        ...
        metrics_data = await analytics_service.generate_revenue_summary(
            request.start_date, request.end_date, request.filters
        )
...
async def get_product_performance_report(
    ...
    try:
        products = await analytics_service.get_product_performance(
            start_date=start_date, end_date=end_date, top_n=top_n, sort_by=sort_by
        )
...
async def get_customer_analysis_report(
    ...
    segment_by: str = Query(
        "value", description="Segment by: value, geography, industry"
    ),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_active_user),
):
    ...
    try:
        segments = await analytics_service.get_customer_analysis(
            start_date=start_date, end_date=end_date, segment_by=segment_by
        )
...
async def get_sales_trend_analysis(
    ...
    period: ReportPeriod = Query(
        ReportPeriod.MONTHLY, description="Aggregation period"
    ),
        ...
        return {
            ...
            "summary": {
                ...
                "average_growth_rate": round(
                    sum(
                        t.get("growth_rate", 0) or 0
                        for t in trends
                        if t.get("growth_rate")
                    )
                    / len([t for t in trends if t.get("growth_rate")]),
                    2,
                )
                if trends
                else 0,
                "best_period": max(trends, key=lambda x: x["revenue"])["period"]
                if trends
                else None,
...
async def export_report(
    ...
    if format == ExportFormat.CSV:
        ...
        return StreamingResponse(
            ...
            media_type="text/csv",
            headers={
                "Content-Disposition": f"attachment; filename=sales_report_{report_id}.csv"
            },
        )
        ...
        return StreamingResponse(
            ...
            headers={
                "Content-Disposition": f"attachment; filename=sales_report_{report_id}.json"
            },
        )
