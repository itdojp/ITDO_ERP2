...
class PromotionalCampaign(BaseTable):
    ...
    name = Column(String(200), nullable=False)
    description = Column(Text)
    campaign_type = Column(String(50), nullable=False)  # seasonal, flash_sale, clearance, etc.
...
class CampaignDiscount(BaseTable):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    campaign_id = Column(UUID(as_uuid=True), ForeignKey("promotional_campaigns.id"), nullable=False)
...
class PriceAdjustment(BaseTable):
    ...
    adjustment_type = Column(String(50), nullable=False)  # percentage, fixed, rule_based
...
class CustomerPricing(BaseTable):
    ...
    __table_args__ = (
        ...
        UniqueConstraint("customer_id", "product_id", name="uq_customer_product_pricing"),
...
class PricingEngine:
    ...
    async def calculate_product_price(
        ...
        if customer_id:
            customer_price = await self._get_customer_specific_price(product_id, customer_id, quantity)
            if customer_price:
        ...
        pricing_rules = await self._get_applicable_pricing_rules(product_id, customer_id, quantity)
        for rule in pricing_rules:
            rule_discount = await self._apply_pricing_rule(rule, current_price, quantity)
            if rule_discount > 0:
                applied_discounts.append({
                    "type": "pricing_rule",
                    "name": rule["name"],
        ...
        if campaign_id:
            campaign_discounts = await self._get_campaign_discounts(campaign_id, product_id)
        ...
        if discount_codes:
            for code in discount_codes:
                ...
                if discount and await self._is_discount_applicable(discount, product_id, customer_id, quantity):
                    discount_amount = await self._calculate_discount_amount(
                        discount, current_price, quantity
                    )
                    if discount_amount > 0:
        ...
        bulk_discount = await self._calculate_bulk_pricing(product_id, quantity, current_price)
        if bulk_discount > 0:
            applied_discounts.append({
                "type": "bulk_pricing",
                "name": "Bulk Discount",
    ...
    async def _get_applicable_pricing_rules(
        ...
        applicable_rules = []
        for rule in rules:
            if await self._evaluate_rule_conditions(rule, product_id, customer_id, quantity):
    ...
    async def _apply_pricing_rule(
        ...
        if "percentage_discount" in actions:
            ...
            if dynamic_config.get("type") == "inventory_based":
                ...
                inventory_level = await self._get_product_inventory_level(rule["product_id"])
                if inventory_level < dynamic_config.get("low_stock_threshold", 10):
                    # Increase price for low stock
                    price_increase = current_price * (Decimal(str(dynamic_config.get("low_stock_multiplier", 1.1))) - 1)
                    discount_amount = -price_increase  # Negative discount = price increase
                elif inventory_level > dynamic_config.get("high_stock_threshold", 100):
                    # Decrease price for high stock
                    discount_amount = current_price * (Decimal(str(dynamic_config.get("high_stock_discount", 0.05))))
    ...
    async def _get_campaign_discounts(
        ...
        query = select(Discount).join(CampaignDiscount).where(
            and_(
            ...
            for tier in tiers:
                if quantity >= tier["min_quantity"]:
                    if tier["discount_type"] == "percentage":
                        discount_amount = current_price * (Decimal(str(tier["discount_value"])) / 100)
                    else:
    ...
    async def _is_discount_applicable(
        ...
        # Check usage limits
        if discount.get("max_total_usage") and discount.get("current_usage", 0) >= discount["max_total_usage"]:
            return False
    ...
    async def _is_discount_applicable_to_product(
        ...
        if scope == DiscountScope.GLOBAL:
            ...
            if "categories" in target_criteria:
                ...
                """)
                result = await self.db.execute(product_category_query, {"product_id": product_id})
                product_data = result.fetchone()
                
                if product_data and str(product_data.category_id) in target_criteria["categories"]:
                    return True
            ...
            if "brands" in target_criteria:
                ...
                """)
                result = await self.db.execute(product_brand_query, {"product_id": product_id})
                product_data = result.fetchone()
                
                if product_data and str(product_data.brand_id) in target_criteria["brands"]:
                    return True
        ...
        for rule in bulk_rules:
            if await self._is_discount_applicable_to_product(rule.__dict__, product_id):
                ...
                for tier in tiers:
                    if quantity >= tier["min_quantity"]:
                        if tier["discount_type"] == "percentage":
                            discount = current_price * (Decimal(str(tier["discount_value"])) / 100)
                        else:
    ...
    async def calculate_bulk_pricing(
        ...
        order_discounts = await self._get_order_level_discounts(subtotal, items[0].customer_id if items else None)
