...
from app.models.sales import (
    SalesTarget, SalesAchievement, SalesTerritory, SalesRegion, SalesQuota,
    SalesCommission, SalesLead, SalesOpportunity, SalesActivity, SalesPipeline,
    SalesForecast, SalesReport, SalesMetric, SalesAnalytics
)
...
async def generate_lead_number(db: AsyncSession, prefix: str = "LEAD") -> str:
    ...
    result = await db.execute(
        select(func.max(SalesLead.lead_number))
        .where(SalesLead.lead_number.like(f"{full_prefix}%"))
    )
...
async def generate_opportunity_number(db: AsyncSession, prefix: str = "OPP") -> str:
    ...
    result = await db.execute(
        select(func.max(SalesOpportunity.opportunity_number))
        .where(SalesOpportunity.opportunity_number.like(f"{full_prefix}%"))
    )
...
async def calculate_sales_metrics(db: AsyncSession, period_start: date, period_end: date, territory_id: Optional[UUID] = None) -> Dict[str, Any]:
...
async def create_lead(
    ...
    auto_assign: bool = Query(False, description="Automatically assign to territory sales rep"),
    ...
    return SalesLeadResponse(
        ...
        assigned_to_name=current_user.username if db_lead.assigned_to == current_user.id else None,
...
async def list_leads(
    ...
    sort_by: str = Query("created_at", regex="^(name|company|score|created_at|estimated_value)$"),
...
@router.post("/leads/{lead_id}/convert", response_model=SalesOpportunityResponse, status_code=status.HTTP_201_CREATED)
async def convert_lead_to_opportunity(
    ...
    db_opportunity = SalesOpportunity(
        ...
        name=opportunity_data.get("name", f"Opportunity for {lead.company or lead.first_name}"),
        lead_id=lead_id,
        stage=SalesStage.PROSPECTING,
        status=OpportunityStatus.OPEN,
        probability=opportunity_data.get("probability", 10),
        estimated_value=opportunity_data.get("estimated_value", lead.estimated_value or Decimal('0')),
        estimated_close_date=opportunity_data.get("estimated_close_date", lead.estimated_close_date),
        territory_id=lead.territory_id,
    ...
    db_opportunity.weighted_value = await update_opportunity_weighted_value(db_opportunity)
...
async def list_opportunities(
    ...
    search: Optional[str] = Query(None, min_length=1),
    sort_by: str = Query("created_at", regex="^(name|stage|estimated_value|estimated_close_date|created_at)$"),
    sort_order: str = Query("desc", regex="^(asc|desc)$"),
    db: AsyncSession = Depends(get_db)
):
...
async def get_sales_dashboard(
    ...
    if territory_id:
        pipeline_query = pipeline_query.where(SalesOpportunity.territory_id == territory_id)
    
    if assigned_to:
        pipeline_query = pipeline_query.where(SalesOpportunity.assigned_to == assigned_to)
    ...
    for i in range(6):  # Last 6 months
        ...
        month_metrics = await calculate_sales_metrics(db, month_start, month_end, territory_id)
...
async def get_conversion_analytics(
    ...
    lead_conversion_rate = (total_opportunities / total_leads * 100) if total_leads > 0 else 0
    ...
    opportunity_conversion_rate = (total_orders / total_opportunities * 100) if total_opportunities > 0 else 0
    
    # Average conversion times
    avg_lead_to_opp_time = await db.execute(
        select(func.avg(
            func.extract('epoch', SalesOpportunity.created_at - SalesLead.created_at) / 86400
        ))
    ...
    return {
        "conversion_funnel": {
            ...
            "overall_conversion_rate": round((total_orders / total_leads * 100) if total_leads > 0 else 0, 2)
        },
        "conversion_times": {
...
async def create_sales_target(
    ...
    achievement_percentage = (db_target.achieved_value / db_target.target_value * 100) if db_target.target_value > 0 else Decimal('0')
...
@router.post("/forecasts", response_model=SalesForecastResponse, status_code=status.HTTP_201_CREATED)
...
async def bulk_update_lead_status(
    ...
    leads_result = await db.execute(
        select(SalesLead).where(SalesLead.id.in_(lead_ids))
    )
...
async def get_pipeline_report(
    ...
    # Build query
    query = select(SalesOpportunity).where(SalesOpportunity.status == OpportunityStatus.OPEN)
    ...
    return {
        "summary": {
            ...
            "average_deal_size": float(total_value / len(opportunities)) if opportunities else 0,
            "weighted_close_probability": float(total_weighted_value / total_value * 100) if total_value > 0 else 0
        },
