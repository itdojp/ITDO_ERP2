...
class ProductAttribute(BaseTable):
    ...
    attribute_type = Column(String(50), nullable=False)  # text, number, boolean, select, multiselect
    ...
    product_attributes = relationship("ProductAttributeMapping", back_populates="attribute")
...
class ProductInventoryLog(BaseTable):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    product_id = Column(UUID(as_uuid=True), ForeignKey("products.id"), nullable=False)
    change_type = Column(String(50), nullable=False)  # adjustment, sale, purchase, return
...
class ProductManagementService:
    ...
    async def create_slug(self, name: str, model_class: Any, existing_id: Optional[uuid.UUID] = None) -> str:
    ...
    # Category Management
    async def create_category(self, category_data: ProductCategoryCreate) -> ProductCategory:
        """Create new product category"""
        if not category_data.slug:
            category_data.slug = await self.create_slug(category_data.name, ProductCategory)
        
        # Calculate level if parent exists
        level = 0
        if category_data.parent_id:
            parent_query = select(ProductCategory).where(ProductCategory.id == category_data.parent_id)
        ...
        category = ProductCategory(
            **category_data.dict(exclude={"slug"}),
    ...
    async def get_brands(
        ...
        brand_responses = []
        for brand in brands:
            products_query = select(func.count(Product.id)).where(Product.brand_id == brand.id)
            products_result = await self.db.execute(products_query)
            products_count = products_result.scalar()
    ...
    async def create_product(self, product_data: ProductCreate) -> Product:
        ...
        if product_data.category_id:
            cat_query = select(ProductCategory).where(ProductCategory.id == product_data.category_id)
        ...
        product = Product(
            **product_data.dict(exclude={"slug"}),
            ...
            published_at=datetime.utcnow() if product_data.status == ProductStatus.ACTIVE else None
        )
    ...
    async def search_products(
        ...
        if filters:
            ...
            if filters.is_visible is not None:
                ...
                count_query = count_query.where(Product.is_visible == filters.is_visible)
    ...
    async def get_product_by_id(self, product_id: uuid.UUID) -> Optional[Product]:
        ...
        query = select(Product).options(
            ...
            selectinload(Product.attributes).selectinload(ProductAttributeMapping.attribute)
        ...
        if product:
            # Cache for 10 minutes
            await self.redis.setex(cache_key, 600, json.dumps(product.__dict__, default=str))
    ...
    async def update_inventory(
        ...
        if not product.track_inventory:
            raise HTTPException(status_code=400, detail="Product does not track inventory")
...
async def create_category(
    ...
    """Create new product category"""
    service = ProductManagementService(db, await aioredis.from_url("redis://localhost:6379"))
