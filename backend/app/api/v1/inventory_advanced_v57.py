...
class BulkTransactionRequest(BaseModel):
    transactions: List[InventoryTransactionRequest] = Field(..., min_items=1, max_items=100)
...
class ReservationRequest(BaseModel):
    product_id: UUID
    quantity: int = Field(..., gt=0)
    reserved_for: str = Field(..., min_length=1, max_length=100)  # Order ID, Customer, etc.
...
class InventoryTransactionManager:
        ...
        async with self.db.begin():  # Database transaction
            try:
                ...
                if request.transaction_type == TransactionType.OUTBOUND:
                    available = await self._get_available_stock(request.product_id, request.from_location)
                    if available < request.quantity:
                        raise BusinessLogicError(f"Insufficient stock. Available: {available}, Required: {request.quantity}")
    ...
    async def _get_available_stock(self, product_id: UUID, location: Optional[str] = None) -> int:
    ...
    async def _create_transaction_record(
        ...
        transaction = InventoryTransaction(
            ...
            total_cost=request.cost_per_unit * request.quantity if request.cost_per_unit else None,
    ...
    async def _update_inventory_levels(self, request: InventoryTransactionRequest):
    ...
    async def _get_or_create_inventory_item(self, product_id: UUID, location: str) -> InventoryItem:
    ...
    async def _check_and_create_alerts(self, product_id: UUID):
    ...
    async def _create_alert(self, product_id: UUID, alert_data: Dict[str, Any]):
    ...
    async def _process_location_transfer(self, request: InventoryTransactionRequest):
        """Process inventory transfer between locations"""
        
        if not request.from_location or not request.to_location:
            raise ValueError("Both from_location and to_location required for transfers")
        
        # Decrease from source location
        from_item = await self._get_or_create_inventory_item(request.product_id, request.from_location)
        ...
        to_item = await self._get_or_create_inventory_item(request.product_id, request.to_location)
    ...
    async def _validate_transaction_request(self, request: InventoryTransactionRequest):
        ...
        if request.transaction_type == TransactionType.OUTBOUND:
            available = await self._get_available_stock(request.product_id, request.from_location)
            if available < request.quantity:
                raise BusinessLogicError(f"Insufficient stock for product {request.product_id}")
...
class ReservationManager:
    ...
    async def create_reservation(
        ...
        async with self.db.begin():
            ...
            if available < request.quantity:
                raise BusinessLogicError(f"Insufficient stock for reservation. Available: {available}")
...
async def list_inventory_transactions(
    ...
    if transaction_type:
        query = query.where(InventoryTransaction.transaction_type == transaction_type.value)
    ...
    try:
        ...
        for transaction in transactions:
            ...
            responses.append(InventoryTransactionResponse(
                transaction_id=transaction.id,
                product_id=transaction.product_id,
...
async def get_inventory_levels(
    product_ids: Optional[List[UUID]] = Query(None, description="Filter by product IDs"),
    ...
    if low_stock_only:
        query = query.where(InventoryItem.available_quantity <= InventoryItem.reorder_point)
    ...
    try:
        ...
        for item in items:
            ...
            days_of_supply = int(item.available_quantity / velocity) if velocity > 0 else None
...
async def create_inventory_adjustment(
    ...
    transaction_request = InventoryTransactionRequest(
        ...
        quantity=request.quantity,
        reason=request.reason,
        cost_per_unit=request.cost_impact / abs(request.quantity) if request.cost_impact else None
    )
    
    transaction_manager = InventoryTransactionManager(db)
    
    try:
        result = await transaction_manager.execute_transaction(transaction_request, current_user.id)
        return result
...
async def get_inventory_alerts(
    ...
    # Build query
    query = select(InventoryAlert).options(
        selectinload(InventoryAlert.product)
    )
    ...
    try:
        ...
        for alert in alerts:
            ...
            responses.append(InventoryAlertResponse(
                alert_id=alert.id,
...
async def get_inventory_analytics(
    ...
    try:
        ...
        transaction_query = select(
            ...
            func.sum(
                case(
                    (InventoryTransaction.transaction_type == 'inbound', InventoryTransaction.quantity),
                    else_=0
                )
            ).label('total_inbound'),
            func.sum(
                case(
                    (InventoryTransaction.transaction_type == 'outbound', InventoryTransaction.quantity),
                    else_=0
                )
        ...
        if product_id:
            transaction_query = transaction_query.where(InventoryTransaction.product_id == product_id)
        ...
        return {
            ...
            "transaction_analytics": {
                ...
                "net_change": (transaction_stats.total_inbound or 0) - (transaction_stats.total_outbound or 0)
            },
            "stock_analytics": {
                ...
                "stock_utilization": round(
                    (stock_stats.total_reserved or 0) / (stock_stats.total_stock or 1) * 100, 2
                )
            },
            "alert_analytics": {
...
async def send_transaction_notification(transaction_id: UUID, email: str):
    """Send transaction notification (background task)"""
    # Simulate notification sending
    await asyncio.sleep(0.1)
    print(f"Notification sent for transaction {transaction_id} to {email}")

async def send_bulk_transaction_notification(transaction_ids: List[UUID], email: str):
