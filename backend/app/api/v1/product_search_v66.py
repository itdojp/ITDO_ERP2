...
class ElasticsearchService:
    ...
    async def _prepare_product_document(self, product_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        doc = {
            ...
            "compare_at_price": float(product_data.get("compare_at_price", 0)) if product_data.get("compare_at_price") else None,
            ...
            "created_at": product_data["created_at"].isoformat() if product_data.get("created_at") else None,
            "updated_at": product_data["updated_at"].isoformat() if product_data.get("updated_at") else None,
            "published_at": product_data.get("published_at").isoformat() if product_data.get("published_at") else None
        }
    ...
    async def _get_product_popularity(self, product_id: str) -> Optional[Dict[str, Any]]:
        ...
        # Query from database
        query = select(ProductPopularity).where(ProductPopularity.product_id == product_id)
    ...
    async def search_products(self, request: SearchRequest) -> SearchResponse:
        ...
        try:
            ...
            # Log search query
            await self._log_search_query(request, response["hits"]["total"]["value"], execution_time)
            ...
            return SearchResponse(
                ...
                page=request.page,
                size=request.size,
                pages=(response["hits"]["total"]["value"] + request.size - 1) // request.size,
    ...
    def _build_filter_query(self, field: str, filter_value: Any) -> Optional[Dict[str, Any]]:
    ...
    async def _build_facet_aggregations(self, requested_facets: Optional[List[str]]) -> Dict[str, Any]:
    ...
    async def _log_search_query(self, request: SearchRequest, results_count: int, execution_time: int):
    ...
    async def get_recommendations(self, request: RecommendationRequest) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if request.recommendation_type == "related" and request.product_id:
                ...
                product_query = text("SELECT category_id, brand_id, tags FROM products WHERE id = :product_id")
                result = await self.db.execute(product_query, {"product_id": request.product_id})
                product_data = result.fetchone()
                
                if product_data:
                    query["query"]["bool"]["should"] = [
                        {"term": {"category.id": str(product_data.category_id)}},
                        {"term": {"brand.id": str(product_data.brand_id)}} if product_data.brand_id else {},
...
async def get_search_facets(
    ...
    query = select(SearchFacet).where(SearchFacet.is_active == True).order_by(SearchFacet.sort_order)
...
async def get_search_analytics(
    ...
    # No results queries
    no_results_query = select(SearchQuery.query_text).where(
        and_(
            SearchQuery.created_at >= start_date,
