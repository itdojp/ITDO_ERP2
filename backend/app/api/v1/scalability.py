...
from app.models.scaling import ScalingPolicy, ScalingEvent, ResourcePool, LoadBalancer
from app.models.user import User
from app.services.scalability_manager import (
    scalability_manager, ScalingType, ScalingDirection, ResourceType,
    LoadBalancingStrategy, InstanceStatus, ScalingTrigger,
    ScalingMetrics, ScalingDecision, InstanceConfig
)
...
async def evaluate_scaling(
    ...
    return [
        ScalingDecisionResponse(
            ...
            estimated_completion_time_minutes=decision.estimated_completion_time.total_seconds() / 60,
...
async def execute_scaling(
    ...
    if not decision:
        raise HTTPException(
            ...
            detail="Scaling decision not found"
        )
    
    # Execute scaling in background
    background_tasks.add_task(
        _execute_scaling_background,
        request.decision_id,
...
async def _execute_scaling_background(decision_id: UUID, notes: Optional[str]):
    """Execute scaling in background"""
    try:
...
async def configure_load_balancer(
    ...
    scalability_manager.load_balancer.health_check_interval = config.health_check_interval
...
async def get_load_balancer_status(
    ...
    unhealthy_instances = [i for i in instances if i['status'] != InstanceStatus.RUNNING]
...
async def remove_instance_from_load_balancer(
    ...
    success = await scalability_manager.load_balancer.remove_instance(instance_id, graceful)
...
# Capacity Planning
@router.post("/capacity-plan", response_model=CapacityPlanResponse, status_code=status.HTTP_201_CREATED)
...
async def get_scaling_metrics(
    ...
    return {
        ...
        "current_status": {
            ...
            "load_balancing_strategy": metrics.get('load_balancing_strategy', 'round_robin'),
