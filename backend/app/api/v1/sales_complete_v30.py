...
from app.crud.sales_extended_v30 import (
    CustomerCRUD, SalesOrderCRUD, QuoteCRUD, InvoiceCRUD, 
    PaymentCRUD, ShipmentCRUD, NotFoundError, DuplicateError, InvalidStatusError
)
from app.schemas.sales_complete_v30 import (
    ...
    SalesStatsResponse, SalesAnalyticsResponse
)
...
@router.post("/invoices/from-order/{order_id}", response_model=InvoiceResponse, status_code=status.HTTP_201_CREATED)
...
def get_sales_stats(
    ...
    orders_this_month = db.query(SalesOrder).filter(SalesOrder.created_at >= current_month).count()
    ...
    channel_stats = db.query(
        SalesOrder.sales_channel, func.sum(SalesOrder.total_amount)
    ).filter(SalesOrder.status != "cancelled").group_by(SalesOrder.sales_channel).all()
    by_channel = {channel: amount for channel, amount in channel_stats}
    
    # トップ顧客
    top_customers = db.query(Customer).filter(Customer.total_sales > 0).order_by(
        Customer.total_sales.desc()
    ).limit(5).all()
