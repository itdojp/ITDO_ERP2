...
async def get_department_tree(
    ...
    include_inactive: bool = Query(False, description="Include inactive sub-departments"),
...
async def get_department_statistics(
    department_id: int = Path(..., description="Department ID"),
    include_sub_departments: bool = Query(False, description="Include sub-department statistics"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> DepartmentWithStats:
    """
...
async def move_department(
    ...
    new_parent_id: Optional[int] = Query(None, description="New parent department ID (null for root level)"),
    new_organization_id: Optional[int] = Query(None, description="New organization ID (for cross-org moves)"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> Dict[str, Any]:
    """
    ...
    if not (current_user.is_superuser or "dept_admin" in [role.name for role in current_user.roles]):
        raise HTTPException(
...
async def get_all_sub_departments(
    ...
    include_inactive: bool = Query(False, description="Include inactive sub-departments"),
    flatten: bool = Query(False, description="Return flat list instead of hierarchical"),
    db: AsyncSession = Depends(get_db),
    current_user: User = Depends(get_current_user),
...
async def get_department_users_recursive(
    department_id: int = Path(..., description="Department ID"),
    include_sub_departments: bool = Query(True, description="Include users from sub-departments"),
