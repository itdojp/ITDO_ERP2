...
from app.crud.purchasing_extended_v30 import (
    SupplierCRUD, PurchaseRequestCRUD, PurchaseOrderCRUD, PurchaseReceiptCRUD, 
    SupplierProductCRUD, NotFoundError, DuplicateError, InvalidStatusError, InsufficientBudgetError
)
from app.schemas.purchasing_complete_v30 import (
    SupplierCreate, SupplierUpdate, SupplierResponse, SupplierListResponse,
    PurchaseRequestCreate, PurchaseRequestUpdate, PurchaseRequestResponse, PurchaseRequestListResponse,
    ...
    PurchaseStatsResponse, PurchaseAnalyticsResponse
)
...
def list_suppliers(
    ...
    supplier_crud = SupplierCRUD(db)
    suppliers, total = supplier_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
def approve_purchase_request(
    ...
    try:
        request_crud = PurchaseRequestCRUD(db)
        return request_crud.approve_request(request_id, current_user["sub"], approved_budget)
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except InvalidStatusError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def reject_purchase_request(
    ...
    try:
        request_crud = PurchaseRequestCRUD(db)
        return request_crud.reject_request(request_id, current_user["sub"], rejection_reason)
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except InvalidStatusError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
...
def get_purchase_stats(
    ...
    """購買統計取得"""
    from app.models.purchasing_extended import Supplier, PurchaseOrder, PurchaseReceipt
    ...
    orders_this_month = db.query(PurchaseOrder).filter(PurchaseOrder.created_at >= current_month).count()
    ...
    avg_order_value = total_purchases / total_purchase_orders if total_purchase_orders > 0 else Decimal('0')
    ...
    # サプライヤータイプ別統計
    type_stats = db.query(
        Supplier.supplier_type, func.sum(PurchaseOrder.total_amount)
    ).join(PurchaseOrder).filter(PurchaseOrder.status != "cancelled").group_by(Supplier.supplier_type).all()
    by_supplier_type = {supplier_type: amount for supplier_type, amount in type_stats}
    
    # トップサプライヤー
    top_suppliers = db.query(Supplier).filter(Supplier.total_purchases > 0).order_by(
        Supplier.total_purchases.desc()
    ).limit(5).all()
    ...
    # 保留中の受領
    pending_receipts = db.query(PurchaseReceipt).filter(
        PurchaseReceipt.inspection_status == "pending"
    ).count()
