...
# Dependencies
async def get_payment_service(db: AsyncSession = Depends(get_db)) -> PaymentProcessingService:
...
async def create_payment(
    ...
    **Error Handling:**
    ...
    try:
        ...
        # Process payment
        result = await service.process_payment(payment_request, request.payment_provider)
    ...
    **Business Value:**
    ...
    try:
        ...
        for category, items in response_data["breakdown"].items():
            ...
            total_count = sum(item["count"] for item in items.values())
            
            for item_data in items.values():
                item_data["percentage"] = (
                    (item_data["amount"] / total_amount * 100) if total_amount > 0 else 0
                )
    ...
    **Forecasting Features:**
    ...
    try:
        ...
        trends = {
            ...
            "forecast": {
                "next_month_transactions": int(analytics["totals"]["transactions"] * 30 / days),
                "next_month_revenue": analytics["totals"]["successful_amount"] * 30 / days,
            }
        }
    ...
    **Security Features:**
    ...
    try:
        ...
        if not transaction_id:
            raise HTTPException(status_code=400, detail="Missing transaction_id in webhook")
        
        # Handle different event types
        if event_type in ["payment.completed", "payment.captured"]:
            background_tasks.add_task(
                _handle_payment_completion_webhook,
                transaction_id,
                webhook_data
            )
        elif event_type in ["payment.failed", "payment.declined"]:
            background_tasks.add_task(
                _handle_payment_failure_webhook,
                transaction_id,
                webhook_data
            )
...
async def _send_payment_confirmation(transaction_id: str, customer_email: Optional[str]):
...
async def _send_refund_notification(refund_id: str):
    """Send refund notification to customer"""
    # Implementation would send actual notification
    print(f"Sending refund notification for {refund_id}")


async def _update_analytics(transaction_id: str):
...
async def _handle_payment_completion_webhook(transaction_id: str, webhook_data: Dict[str, Any]):
    """Handle payment completion webhook"""
    print(f"Processing payment completion webhook for {transaction_id}")


async def _handle_payment_failure_webhook(transaction_id: str, webhook_data: Dict[str, Any]):
    """Handle payment failure webhook"""
    print(f"Processing payment failure webhook for {transaction_id}")


async def _handle_refund_webhook(transaction_id: str, webhook_data: Dict[str, Any]):
    """Handle refund webhook"""
    print(f"Processing refund webhook for {transaction_id}")


async def _handle_dispute_webhook(transaction_id: str, webhook_data: Dict[str, Any]):
