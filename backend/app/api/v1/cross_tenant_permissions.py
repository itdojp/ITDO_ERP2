...
def create_cross_tenant_rule(
    ...
    try:
        rule_model = service.create_permission_rule(rule_data, current_user)
        return CrossTenantPermissionRule.model_validate(rule_model)
...
def update_cross_tenant_rule(
    ...
    service = CrossTenantPermissionService(db)
    try:
        rule_model = service.update_permission_rule(rule_id, update_data, current_user)
        return CrossTenantPermissionRule.model_validate(rule_model)
    except (NotFound, PermissionDenied, BusinessLogicError) as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
