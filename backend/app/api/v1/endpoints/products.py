...
async def create_product(
    ...
    if (
        "name" not in product_data
        or "price" not in product_data
        or "sku" not in product_data
    ):
        raise HTTPException(
            status_code=422, detail="Missing required fields: name, price, sku"
        )
    ...
    price_history_store[product_id] = [
        {"price": float(product_data["price"]), "changed_at": now, "changed_by": None}
    ]
...
async def update_product(
    product_id: str, update_data: Dict[str, Any], db: AsyncSession = Depends(get_db)
) -> ProductCreateResponse:
    """Update product"""
...
async def bulk_create_products(
    bulk_data: Dict[str, List[Dict[str, Any]]], db: AsyncSession = Depends(get_db)
) -> BulkCreateResponse:
    ...
    for i, product_data in enumerate(products):
        try:
            ...
            # Check for duplicate SKU
            sku_exists = any(
                p["sku"] == product_data["sku"] for p in products_store.values()
            )
            if sku_exists:
                failed += 1
                details.append(
            ...
            created += 1
            details.append({"index": i, "id": product_id, "status": "created"})

        except Exception as e:
            failed += 1
            details.append({"index": i, "error": str(e), "data": product_data})
    ...
    for i, update_data in enumerate(updates):
        try:
            ...
            if not product_id:
                failed += 1
                details.append(
                    {"index": i, "error": "Missing product ID", "data": update_data}
                )
                continue
            ...
            if product_id not in products_store:
                failed += 1
                details.append(
                    {"index": i, "error": "Product not found", "data": update_data}
                )
                continue
            ...
            updated += 1
            details.append({"index": i, "id": product_id, "status": "updated"})

        except Exception as e:
            failed += 1
            details.append({"index": i, "error": str(e), "data": update_data})
...
async def get_product_price_history(
    ...
    return PriceHistoryResponse(product_id=product_id, history=history_items)
