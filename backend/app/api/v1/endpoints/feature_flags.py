...
async def get_feature_variant(
    ...
    return {"key": request.key, "variant": variant, "user_id": context.user_id}
...
async def create_feature_flag(
    ...
    if (
        request.strategy == FeatureFlagStrategy.PERCENTAGE
        and request.percentage is not None
    ):
        rules.append(
            FeatureFlagRule(
                strategy=FeatureFlagStrategy.PERCENTAGE, percentage=request.percentage
            )
        )
    elif request.strategy == FeatureFlagStrategy.USER_LIST and request.user_ids:
        rules.append(
            FeatureFlagRule(
                strategy=FeatureFlagStrategy.USER_LIST, user_ids=request.user_ids
            )
        )
    elif (
        request.strategy == FeatureFlagStrategy.ORGANIZATION
        and request.organization_ids
    ):
        rules.append(
            FeatureFlagRule(
        ...
        rules.append(
            FeatureFlagRule(
                strategy=FeatureFlagStrategy.ROLE_BASED, roles=request.roles
            )
        )
...
async def update_feature_flag(
    ...
    if any(
        [request.percentage, request.user_ids, request.organization_ids, request.roles]
    ):
        rules = []
...
async def update_rollout_percentage(
    ...
    percentage: float = Body(
        ..., ge=0, le=100, description="Rollout percentage (0-100)"
    ),
    current_user: User = Depends(get_current_user),
    service: FeatureFlagService = Depends(get_feature_flag_service),
):
