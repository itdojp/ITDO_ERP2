...
from app.models.performance import PerformanceMetric, OptimizationRule, QueryProfile
from app.models.user import User
from app.services.performance_optimizer import (
    performance_optimizer, MetricType, OptimizationType, AlertSeverity,
...
async def get_optimization_recommendations(
    ...
    if applied is not None:
        paginated_recommendations = [
            r for r in paginated_recommendations
...
async def get_system_health(
    ...
    disk_io = metrics.get('disk_io', {}).get('current', 0)
    ...
    return SystemHealthResponse(
        ...
        pending_optimizations=summary.get('total_recommendations', 0) - summary.get('applied_optimizations', 0),
...
async def _execute_benchmark(
    ...
    if execution_times:
        ...
        avg_memory_delta = int(sum(memory_deltas) / len(memory_deltas)) if memory_deltas else 0
    else:
        total_execution_time = 0.0
        average_execution_time = 0.0
        min_execution_time = 0.0
    ...
    logging.info(f"Benchmark {benchmark_id} completed: {average_execution_time:.3f}s avg")
...
async def _benchmark_computation_operation(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    result = sum(range(10000))  # Simple computation
...
async def get_query_profiles(
    ...
    query_optimizer = next(
        (opt for opt in performance_optimizer.optimizers 
    ...
    for query_hash, analysis in query_optimizer.query_profiles.items():
        if analysis.average_execution_time >= min_execution_time:
            profiles.append(QueryProfileResponse(
                query_hash=analysis.query_hash,
                query_text=analysis.query_text[:500] + "..." if len(analysis.query_text) > 500 else analysis.query_text,
    ...
    async def metric_generator():
        ...
        while True:
            try:
                ...
                current_metrics = await performance_optimizer.system_monitor.collect_metrics()
                
                # Filter metrics if specified
                if target_metrics:
                    filtered_metrics = {
                        k: v for k, v in current_metrics.metrics.items()
...
async def get_monitoring_status(
    ...
    return {
        ...
        "active_optimizers": [
            opt.optimizer_id for opt in performance_optimizer.optimizers
            if opt.enabled
        ],
