...
from app.models.integration import ExternalSystem, IntegrationEndpoint, DataMapping, SyncJob
from app.models.user import User
from app.services.external_integration_hub import (
    integration_hub, IntegrationType, AuthenticationType, SyncDirection,
    SyncStatus, DataFormat, IntegrationConfig, SyncResult
)
...
class SyncJobCreate(BaseModel):
    ...
    schedule_expression: Optional[str] = Field(None, description="Cron expression for recurring jobs")
...
async def list_external_systems(
    ...
    return [
        ExternalSystemResponse(
            ...
            base_url=system.base_url,
            authentication_type=system.auth_config.get('type') if system.auth_config else None,
            headers={},
            timeout=30,
            retry_attempts=3,
            rate_limit=None,
...
async def get_external_system(
    ...
    sync_stats = await db.execute(
        select(
            ...
            func.count(SyncJob.id).filter(SyncJob.status == SyncStatus.SUCCESS).label('successful_syncs'),
...
async def create_data_mapping(
    ...
    target_result = await db.execute(
        select(ExternalSystem.name)
        .where(ExternalSystem.id == mapping.target_system_id)
    )
...
async def list_data_mappings(
    ...
    for mapping, source_name, _ in mappings:
        ...
        responses.append(DataMappingResponse(
            id=mapping.id,
            name=mapping.name,
            description=None,
...
@router.post("/sync", response_model=SyncJobResponse, status_code=status.HTTP_201_CREATED)
...
async def _execute_sync_job(
    ...
    try:
        result = await integration_hub.sync_data(
...
@router.post("/endpoints", response_model=IntegrationEndpointResponse, status_code=status.HTTP_201_CREATED)
...
async def list_integration_endpoints(
    ...
    query = query.offset(skip).limit(limit).order_by(IntegrationEndpoint.created_at.desc())
...
async def get_integration_dashboard(
    ...
    systems_result = await db.execute(
        select(
            ...
            func.count(ExternalSystem.id).filter(ExternalSystem.is_active == True).label('active_systems')
        )
    )
    ...
    jobs_result = await db.execute(
        select(
            ...
            func.count(SyncJob.id).filter(SyncJob.status == SyncStatus.SUCCESS).label('successful_jobs'),
            func.count(SyncJob.id).filter(SyncJob.status == SyncStatus.FAILED).label('failed_jobs'),
            func.sum(SyncJob.records_processed).label('total_records'),
            func.avg(SyncJob.execution_time).label('avg_execution_time')
        ).where(
            and_(
                SyncJob.created_at >= start_date,
                SyncJob.created_at <= end_date
            )
        )
    )
    ...
    return {
        ...
        'systems': {
            ...
            'inactive': (systems_stats.total_systems or 0) - (systems_stats.active_systems or 0)
        },
        'sync_jobs': {
            'total': total_jobs,
        ...
        'data_processing': {
            ...
            'throughput_per_minute': (jobs_stats.total_records or 0) / ((jobs_stats.avg_execution_time or 1) / 60) if jobs_stats.avg_execution_time else 0
        },
