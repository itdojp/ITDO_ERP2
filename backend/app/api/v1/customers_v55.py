...
from app.models.customer import (
    ...
    CustomerGroup, CustomerInteraction, CustomerPreference, CustomerCredit
)
...
class CustomerCreate(BaseModel):
    ...
    def validate_addresses(cls, v):
        ...
        billing_primary = sum(1 for addr in v if addr.address_type == AddressType.BILLING and addr.is_primary)
        shipping_primary = sum(1 for addr in v if addr.address_type == AddressType.SHIPPING and addr.is_primary)
...
class CustomerSearchRequest(BaseModel):
    ...
    sort_by: Optional[str] = Field(None, regex="^(name|company|created_at|total_spent|last_order)$")
...
async def generate_customer_number(db: AsyncSession, prefix: str = "CUST") -> str:
    ...
    result = await db.execute(
        select(func.max(Customer.customer_number))
        .where(Customer.customer_number.like(f"{prefix}%"))
    )
...
async def validate_customer_group_exists(db: AsyncSession, group_id: UUID) -> bool:
    ...
    result = await db.execute(
        select(CustomerGroup).where(CustomerGroup.id == group_id)
    )
    return result.scalar_one_or_none() is not None

async def calculate_customer_metrics(db: AsyncSession, customer_id: UUID) -> Dict[str, Any]:
...
async def create_customer(
    ...
    billing_addresses = [a for a in customer.addresses if a.address_type == AddressType.BILLING]
    if billing_addresses and not any(a.is_primary for a in billing_addresses):
        billing_addresses[0].is_primary = True
    
    shipping_addresses = [a for a in customer.addresses if a.address_type == AddressType.SHIPPING]
...
async def list_customers(
    ...
    search: Optional[str] = Query(None, min_length=1, description="Search in name, company, or email"),
    sort_by: str = Query("created_at", regex="^(name|company|created_at|total_spent|last_order)$"),
    ...
    for customer in customers:
        ...
        response_item = CustomerListResponse(
            ...
            customer_number=customer.customer_number,
            company_name=customer.company_name,
            primary_contact_name=f"{primary_contact.first_name} {primary_contact.last_name}" if primary_contact else "N/A",
...
async def get_customer(
    ...
    if customer.credit_info:
        ...
        available_credit = credit.credit_limit  # Would calculate actual available credit in production
...
async def update_customer(
    ...
    if (customer_update.customer_number and 
        customer_update.customer_number != db_customer.customer_number):
        existing = await db.execute(
...
async def delete_customer(
    ...
    await db.execute(delete(CustomerCredit).where(CustomerCredit.customer_id == customer_id))
    await db.execute(delete(CustomerInteraction).where(CustomerInteraction.customer_id == customer_id))
...
async def list_customer_interactions(
    ...
    start_date: Optional[date] = Query(None, description="Filter interactions after this date"),
    end_date: Optional[date] = Query(None, description="Filter interactions before this date"),
    ...
    # Build query
    query = select(CustomerInteraction).where(CustomerInteraction.customer_id == customer_id)
...
async def get_customer_analytics(
    ...
    industry_query = (industry_query
                     .where(Customer.industry.isnot(None))
                     .group_by(Customer.industry)
                     .order_by(func.count(Customer.id).desc())
                     .limit(10))
    
    industry_result = await db.execute(industry_query)
    top_industries = [{"name": industry, "count": count} for industry, count in industry_result.fetchall()]
    ...
    customer_trend = [{"month": month.strftime("%Y-%m"), "count": count} for month, count in trend_result.fetchall()]
