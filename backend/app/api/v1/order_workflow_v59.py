...
class ApprovalRequest(BaseModel):
    ...
    @validator('decision')
    def validate_approval_decision(cls, v):
        if v not in [ApprovalStatus.APPROVED, ApprovalStatus.REJECTED, ApprovalStatus.ESCALATED]:
...
class OrderStateManager:
    ...
    async def validate_transition(
        ...
        if not force and new_status not in VALID_STATE_TRANSITIONS.get(current_status, set()):
            return {
                "valid": False,
                "reason": f"Invalid transition from {current_status} to {new_status}",
                "allowed_transitions": list(VALID_STATE_TRANSITIONS.get(current_status, set()))
            }
    ...
    async def _validate_business_rules(self, order: Order, new_status: OrderStatus) -> Dict[str, Any]:
...
class ApprovalEngine:
    ...
    async def process_approval_decision(
        ...
        if (approval_request.decision == ApprovalStatus.APPROVED and 
            approval_request.auto_process):
            # Trigger automation
...
class AutomationEngine:
    ...
    async def execute_workflow_automation(
        ...
        try:
            ...
            automation_result = await self._process_automation_trigger(order_id, trigger)
    ...
    async def _handle_order_created(self, order_id: UUID) -> Dict[str, Any]:
        ...
        return {
            ...
            "background_tasks": ["generate_order_confirmation", "check_inventory_levels"]
        }
    ...
    async def _execute_background_task(self, task_name: str):
...
class OrderWorkflowManager:
    ...
    async def transition_order_state(
        ...
        if transition_result["success"]:
            ...
            if trigger:
                automation_result = await self.automation_engine.execute_workflow_automation(
    ...
    def _estimate_completion_time(self, current_status: OrderStatus) -> Optional[datetime]:
    ...
    async def _execute_state_transition(
        ...
        try:
            ...
            logger.info(f"Order {order.id} status changed from {order.status} to {new_status} by user {user_id}")
    ...
    def _get_automation_trigger(self, new_status: OrderStatus) -> Optional[WorkflowTrigger]:
    ...
    async def _send_transition_notifications(
        ...
        for notification_type in notification_types:
            logger.info(f"Sending {notification_type} notification for order {order.id} transition {from_status} -> {to_status}")
...
async def create_approval_workflow(
    ...
    if not approval_workflow["approval_required"]:
        raise HTTPException(
            status_code=400, 
            detail="Order does not require approval"
        )
