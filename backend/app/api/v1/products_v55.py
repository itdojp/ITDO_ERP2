...
from app.models.product import Product, ProductCategory, ProductVariant, ProductImage, ProductAttribute
...
# Helper Functions
async def calculate_stock_status(inventory_quantity: int, min_stock: int = 10) -> StockStatus:
...
async def get_product_analytics(
    ...
    category_result = await db.execute(
        ...
        .select_from(
            base_query.subquery().join(ProductCategory, Product.category_id == ProductCategory.id)
        )
        .group_by(ProductCategory.name)
        .order_by(func.count(Product.id).desc())
        .limit(10)
    )
    top_categories = [{"name": name, "count": count} for name, count in category_result.fetchall()]
    
    # Low stock products
    low_stock_result = await db.execute(
        select(func.count(ProductVariant.id))
        .where(ProductVariant.inventory_quantity <= 10)
    )
