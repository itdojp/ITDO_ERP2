...
async def create_external_system(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create external system: {str(e)}")
...
async def list_external_systems(
    ...
    try:
        systems = await integration_service.list_external_systems(
            db, organization_id, integration_type, connection_status, is_active, page, per_page
        )
        return systems
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list external systems: {str(e)}")
...
async def get_external_system(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get external system: {str(e)}")


@router.put("/external-systems/{system_id}", response_model=ExternalSystemResponse)
async def update_external_system(
    system_id: str,
    request: ExternalSystemUpdateRequest,
    db: Session = Depends(get_db)
):
    """Update an external system configuration."""
    try:
        system = await integration_service.update_external_system(db, system_id, request.dict(exclude_unset=True))
        if not system:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="External system not found")
        return system
    except HTTPException:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update external system: {str(e)}")


@router.post("/external-systems/{system_id}/test-connection", response_model=Dict[str, Any])
async def test_system_connection(
    system_id: str,
    request: SystemConnectionTestRequest,
    db: Session = Depends(get_db)
):
    """Test connection to an external system."""
    try:
        result = await integration_service.test_system_connection(db, system_id, request.dict())
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
async def delete_external_system(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete external system: {str(e)}")
...
async def create_connector(
    request: IntegrationConnectorCreateRequest,
    db: Session = Depends(get_db)
):
...
async def list_connectors(
    organization_id: str = Query(..., description="Organization ID"),
    external_system_id: Optional[str] = Query(None, description="Filter by external system ID"),
    ...
    try:
        connectors = await integration_service.list_connectors(
            db, organization_id, external_system_id, sync_direction, entity_type, is_active, page, per_page
        )
        return connectors
    except Exception as e:
...
async def get_connector(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get connector: {str(e)}")
...
async def update_connector(
    ...
    try:
        connector = await integration_service.update_connector(db, connector_id, request)
        if not connector:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Connector not found")
        return connector
    except HTTPException:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update connector: {str(e)}")


@router.post("/connectors/{connector_id}/execute", response_model=IntegrationExecutionResponse)
async def execute_connector(
    ...
    try:
        execution = await integration_service.execute_connector(db, connector_id, request.dict())
        return execution
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to execute connector: {str(e)}")


@router.get("/connectors/{connector_id}/executions", response_model=ExecutionListResponse)
async def list_connector_executions(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list executions: {str(e)}")
...
async def create_data_mapping(
    request: DataMappingCreateRequest,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create data mapping: {str(e)}")
...
async def list_data_mappings(
    ...
    try:
        mappings = await integration_service.list_data_mappings(
            db, organization_id, connector_id, entity_type, sync_direction, is_active, page, per_page
        )
        return mappings
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list data mappings: {str(e)}")
...
async def get_data_mapping(
    ...
    try:
        ...
        if not mapping:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Data mapping not found")
        return mapping
    except HTTPException:
        raise
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get data mapping: {str(e)}")
...
async def update_data_mapping(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update data mapping: {str(e)}")
...
async def delete_data_mapping(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete data mapping: {str(e)}")
...
async def create_transformation(
    request: DataTransformationCreateRequest,
    db: Session = Depends(get_db)
):
    """Create a new data transformation."""
    try:
        transformation = await integration_service.create_transformation(db, request.dict())
        return transformation
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create transformation: {str(e)}")
...
async def list_transformations(
    ...
    try:
        transformations = await integration_service.list_transformations(
            db, organization_id, connector_id, transformation_type, language, is_active, page, per_page
        )
        return transformations
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list transformations: {str(e)}")
...
async def get_transformation(
    ...
    try:
        transformation = await integration_service.get_transformation(db, transformation_id)
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get transformation: {str(e)}")
...
async def update_transformation(
    ...
    try:
        transformation = await integration_service.update_transformation(db, transformation_id, request)
        if not transformation:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Transformation not found")
        return transformation
    except HTTPException:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update transformation: {str(e)}")


@router.post("/transformations/{transformation_id}/execute", response_model=Dict[str, Any])
async def execute_transformation(
    ...
    try:
        result = await integration_service.apply_data_transformation(db, transformation_id, request.input_data)
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to execute transformation: {str(e)}")
...
async def delete_transformation(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete transformation: {str(e)}")
...
async def create_webhook_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create webhook: {str(e)}")
...
async def list_webhook_endpoints(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list webhooks: {str(e)}")
...
async def get_webhook_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get webhook: {str(e)}")
...
async def update_webhook_endpoint(
    ...
    try:
        webhook = await integration_service.update_webhook_endpoint(db, webhook_id, request)
        if not webhook:
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Webhook endpoint not found")
        return webhook
    except HTTPException:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to update webhook: {str(e)}")


@router.post("/webhooks/process/{endpoint_path:path}", response_model=Dict[str, Any])
async def process_webhook_request(
    endpoint_path: str,
    request: WebhookRequestProcessing,
    db: Session = Depends(get_db)
):
    """Process incoming webhook request."""
    try:
        full_endpoint = f"/webhooks/{endpoint_path}"
        result = await integration_service.process_webhook_request(db, full_endpoint, request.dict())
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
async def delete_webhook_endpoint(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete webhook: {str(e)}")
...
async def create_integration_message(
    request: IntegrationMessageCreateRequest,
    db: Session = Depends(get_db)
):
    """Create a new integration message for async processing."""
    try:
        message = await integration_service.create_integration_message(db, request.dict())
        return message
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to create message: {str(e)}")
...
async def list_integration_messages(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list messages: {str(e)}")
...
async def get_integration_message(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get message: {str(e)}")
...
async def process_pending_messages(
    ...
    try:
        result = await integration_service.process_pending_messages(db, organization_id, limit)
        return result
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to process messages: {str(e)}")


@router.delete("/messages/{message_id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_integration_message(
    message_id: str,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to delete message: {str(e)}")
...
async def get_integration_health(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get integration health: {str(e)}")


@router.post("/analytics", response_model=IntegrationAnalyticsResponse)
async def get_integration_analytics(
    request: IntegrationAnalyticsRequest,
    db: Session = Depends(get_db)
):
    """Get comprehensive integration analytics and performance metrics."""
    try:
        analytics = await integration_service.get_integration_analytics(db, request.dict())
        return analytics
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
...
async def bulk_execute_connectors(
    request: BulkConnectorExecutionRequest,
    db: Session = Depends(get_db)
):
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to execute bulk operations: {str(e)}")
...
async def bulk_health_check_systems(
    ...
    try:
        result = await integration_service.bulk_health_check_systems(db, organization_id, system_ids)
        return result
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to perform bulk health check: {str(e)}")
...
async def list_executions(
    ...
    try:
        executions = await integration_service.list_executions(
            db, organization_id, connector_id, status, execution_type, start_date, end_date, page, per_page
        )
        return executions
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to list executions: {str(e)}")
...
async def get_execution(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get execution: {str(e)}")


@router.post("/executions/{execution_id}/retry", response_model=IntegrationExecutionResponse)
async def retry_execution(
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to retry execution: {str(e)}")
...
async def get_integration_types():
    ...
    integration_types = [
        {"value": "api", "label": "REST API", "description": "RESTful web service integration"},
        ...
        {"value": "message_queue", "label": "Message Queue", "description": "Message queue integration"},
        {"value": "webhook", "label": "Webhook", "description": "HTTP webhook integration"},
        {"value": "ftp", "label": "FTP/SFTP", "description": "File transfer protocol"},
        {"value": "email", "label": "Email", "description": "Email-based integration"},
        {"value": "cloud_storage", "label": "Cloud Storage", "description": "Cloud storage integration"},
        ...
        {"value": "accounting", "label": "Accounting", "description": "Accounting system integration"},
        {"value": "ecommerce", "label": "E-commerce", "description": "E-commerce platform integration"}
    ]
...
async def get_data_formats():
    ...
    data_formats = [
        ...
        {"value": "fixed_width", "label": "Fixed Width", "description": "Fixed-width text format"},
...
async def validate_data_mapping(
    ...
    try:
        result = await integration_service.validate_data_mapping(mapping_config, sample_data)
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to validate mapping: {str(e)}")
...
async def test_transformation_script(
    ...
    try:
        result = await integration_service.test_transformation_script(transformation_script, sample_input, language)
        return result
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=str(e))
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to test transformation: {str(e)}")
...
async def get_system_templates():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get system templates: {str(e)}")
...
async def get_integration_logs(
    ...
    try:
        logs = await integration_service.get_integration_logs(
            db, organization_id, entity_type, entity_id, action_type, start_date, end_date, page, per_page
        )
        return logs
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to get integration logs: {str(e)}")
