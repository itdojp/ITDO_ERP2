...
from app.schemas.inventory_complete_v30 import (
    ...
    CycleCountItemUpdate, CycleCountItemResponse,
    StockAlertResponse, InventoryStatsResponse, InventoryValuationResponse,
    StockLevelSummaryResponse
)
from app.crud.inventory_extended_v30 import (
    WarehouseCRUD, InventoryItemCRUD, CycleCountCRUD, StockAlertCRUD,
    NotFoundError, DuplicateError, InsufficientStockError
)
...
async def list_warehouses(
    ...
    filters = {}
    for key, value in {"search": search, "is_active": is_active, "warehouse_type": warehouse_type}.items():
...
async def list_inventory_items(
    ...
    for key, value in {
        ...
        "quality_status": quality_status, "low_stock": low_stock, "out_of_stock": out_of_stock,
...
async def get_inventory_summary(
    ...
    result = []
    for item in summary:
        result.append(StockLevelSummaryResponse(
...
async def get_inventory_statistics(
    ...
    - アラート数
    """
    from app.models.inventory_extended import Warehouse, InventoryItem
    from sqlalchemy import func
    
    total_warehouses = db.query(func.count(Warehouse.id)).scalar() or 0
    active_warehouses = db.query(func.count(Warehouse.id)).filter(Warehouse.is_active == True).scalar() or 0
...
@router.put("/cycle-counts/{count_id}/items/{item_id}", response_model=CycleCountItemResponse)
...
async def acknowledge_stock_alert(
    ...
    crud = StockAlertCRUD(db)
    try:
        alert = crud.acknowledge_alert(alert_id, current_user.id)
