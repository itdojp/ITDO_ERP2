...
async def get_workflow_definition(
    definition_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowDefinitionResponse:
...
async def update_workflow_definition(
    ...
    try:
        definition = await service.update_workflow_definition(
            definition_id, update_data.dict(exclude_unset=True)
        )
        if not definition:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
@router.post(
    "/definitions/{definition_id}/duplicate", response_model=WorkflowDefinitionResponse
)
...
@router.get(
    "/definitions/{definition_id}/steps", response_model=List[WorkflowStepResponse]
)
...
async def get_workflow_instance(
    instance_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowInstanceResponse:
...
async def cancel_workflow_instance(
    ...
    try:
        success = await service.cancel_workflow_instance(
            instance_id, reason, cancelled_by
        )
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
async def suspend_workflow_instance(
    ...
    try:
        success = await service.suspend_workflow_instance(
            instance_id, reason, suspended_by
        )
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
...
async def get_workflow_task(
    task_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowTaskResponse:
    ...
    try:
        ...
        if not task:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Workflow task not found"
            )
...
async def perform_task_action(
    ...
    try:
        ...
        return {
            "message": f"Task action '{action_data.action_type}' performed successfully"
        }
    except HTTPException:
        raise
    except Exception as e:
...
async def get_comment(
    comment_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowCommentResponse:
    """Get comment by ID."""
    try:
...
async def mention_users_in_comment(
    ...
    try:
        success = await service.mention_users_in_comment(
            comment_id, user_ids, mentioned_by
        )
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Comment not found"
...
@router.get(
    "/tasks/{task_id}/attachments", response_model=WorkflowAttachmentListResponse
)
async def list_task_attachments(
    task_id: str,
    attachment_type: Optional[str] = Query(
        None, description="Filter by attachment type"
    ),
    page: int = Query(1, ge=1, description="Page number"),
    per_page: int = Query(50, ge=1, le=100, description="Items per page"),
    service: WorkflowService = Depends(get_workflow_service),
...
async def get_attachment(
    attachment_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowAttachmentResponse:
...
@router.post(
    "/attachments/{attachment_id}/version", response_model=WorkflowAttachmentResponse
)
async def create_attachment_version(
    ...
    try:
        new_version = await service.create_attachment_version(
            attachment_id, attachment_data.dict()
        )
        if not new_version:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Attachment not found"
...
@router.get(
    ...
    response_model=WorkflowAttachmentListResponse,
)
...
async def share_attachment(
    ...
    try:
        success = await service.share_attachment(
            attachment_id, user_ids, access_level, shared_by
        )
        if not success:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND, detail="Attachment not found"
...
async def get_workflow_dashboard(
    ...
    try:
        dashboard_data = await service.get_workflow_dashboard(
...
async def analyze_bottlenecks(
    ...
    try:
        bottlenecks = await service.analyze_bottlenecks(
            organization_id, definition_id, period_days
        )
        return bottlenecks
    except Exception as e:
        raise HTTPException(
...
async def get_workflow_trends(
    ...
    try:
        trends = await service.get_workflow_trends(
            organization_id, definition_id, period_months
        )
        return trends
    except Exception as e:
        raise HTTPException(
...
async def generate_custom_report(
    organization_id: str = Query(..., description="Organization ID"),
    report_config: Dict[str, any] = Query(
...
async def get_workflow_template(
    template_id: str, service: WorkflowService = Depends(get_workflow_service)
) -> WorkflowTemplateResponse:
...
@router.post(
    "/templates/{template_id}/instantiate", response_model=WorkflowDefinitionResponse
)
...
@router.get(
    "/instances/{instance_id}/activities", response_model=WorkflowActivityListResponse
)
