...
from app.models.customer import Customer, CustomerInteraction, CustomerSegment
...
class CRMManager:
    ...
    async def update_customer_lifecycle_stage(
        ...
        await self.db.execute(
            ...
            .values(
                lifecycle_stage=request.new_stage.value,
        ...
        lifecycle_interaction = CustomerInteraction(
            ...
            created_by=user_id,
            created_at=datetime.utcnow(),
            tags=json.dumps(["lifecycle", "automated" if request.automated else "manual"])
        )
    ...
    async def get_customer_timeline(
        ...
        # Get interactions
        interactions_query = select(CustomerInteraction).where(
            CustomerInteraction.customer_id == customer_id
        ).order_by(desc(CustomerInteraction.created_at)).limit(limit)
        ...
        ).order_by(desc(Order.order_date)).limit(limit)
    ...
    async def _update_customer_last_contact(self, customer_id: UUID):
        """Update customer last contact date"""
        await self.db.execute(
            update(Customer)
...
class RFMAnalysisEngine:
    ...
    async def calculate_rfm_analysis(
        ...
        customer_metrics = await self._get_customer_metrics(start_date, end_date, request.customer_ids)
        ...
        recency_thresholds = self._calculate_percentile_thresholds(recency_values, request.recency_bins, reverse=True)
        frequency_thresholds = self._calculate_percentile_thresholds(frequency_values, request.frequency_bins)
        monetary_thresholds = self._calculate_percentile_thresholds(monetary_values, request.monetary_bins)
        
        # Calculate scores for each customer
        for metrics in customer_metrics:
            recency_score = self._calculate_score(metrics.days_since_last_order, recency_thresholds, reverse=True)
            frequency_score = self._calculate_score(metrics.order_count, frequency_thresholds)
            monetary_score = self._calculate_score(float(metrics.total_revenue), monetary_thresholds)
            
            # Determine RFM segment
            rfm_segment = self._determine_rfm_segment(recency_score, frequency_score, monetary_score)
    ...
    async def _get_customer_metrics(
        ...
        for row in rows:
            ...
            purchase_frequency_days = customer_lifespan_days / max(row.order_count - 1, 1) if row.order_count > 1 else customer_lifespan_days
    ...
    def _calculate_percentile_thresholds(self, values: List[float], bins: int, reverse: bool = False) -> List[float]:
    ...
    def _calculate_score(self, value: float, thresholds: List[float], reverse: bool = False) -> int:
...
class LTVCalculationEngine:
    ...
    async def calculate_customer_ltv(
        ...
        for metrics in customer_metrics:
            ...
            acquisition_cost = Decimal('50.00') if request.include_acquisition_cost else None
            net_ltv = predicted_ltv - acquisition_cost if acquisition_cost else predicted_ltv
            ...
            ltv_response = CustomerLTVResponse(
                ...
                purchase_frequency=metrics.purchase_frequency_days / 30.0,  # Convert to monthly
    ...
    async def _calculate_historic_ltv(
        ...
        monthly_frequency = 30.0 / metrics.purchase_frequency_days if metrics.purchase_frequency_days > 0 else 0.1
        ...
        ltv = metrics.avg_order_value * monthly_frequency * predicted_months * Decimal(str(discount_factor))
    ...
    async def _calculate_predictive_ltv(
        ...
        predicted_ltv = base_ltv * Decimal(str(recency_factor * frequency_factor * trend_factor))
    ...
    async def _calculate_cohort_ltv(
        ...
        days_since_first_order = (datetime.utcnow().date() - metrics.first_order_date).days
...
async def update_customer_lifecycle_stage(
    ...
    try:
        result = await crm_manager.update_customer_lifecycle_stage(stage_update, current_user.id)
        return result
...
async def get_customer_analytics(
    ...
    try:
        ...
        interaction_analytics = await get_interaction_analytics(db, start_date, end_date)
...
# Helper Functions
async def schedule_follow_up_task(interaction_id: UUID, follow_up_date: datetime):
    """Schedule follow-up task (background task)"""
    await asyncio.sleep(0.1)
    print(f"Follow-up scheduled for interaction {interaction_id} at {follow_up_date}")

async def update_customer_segments_task(customer_ids: List[UUID], segments: List[RFMSegment]):
...
async def get_customer_statistics(
    ...
    return {
        ...
        "activation_rate": round((active_customers / total_customers * 100) if total_customers > 0 else 0, 2),
        "growth_rate": round((new_customers / (total_customers - new_customers) * 100) if total_customers > new_customers else 0, 2)
    }
