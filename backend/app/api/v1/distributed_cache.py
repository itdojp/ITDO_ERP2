...
async def get_cache_value(key: str, current_user: User = Depends(get_current_user)):
    """Get value from distributed cache."""
    try:
        value = await distributed_cache.get(key)

        return CacheGetResponse(
            key=key, value=value, found=value is not None, source="distributed"
        )
...
async def set_cache_value(
    cache_request: CacheSetRequest, current_user: User = Depends(get_current_user)
):
...
async def delete_cache_value(key: str, current_user: User = Depends(get_current_user)):
...
async def warm_cache(
    warming_request: CacheWarmingRequest, current_user: User = Depends(get_current_user)
):
...
async def preload_critical_cache(current_user: User = Depends(get_current_user)):
...
async def get_cache_performance_metrics(current_user: User = Depends(get_current_user)):
...
async def get_cache_nodes_status(current_user: User = Depends(get_current_user)):
    ...
    try:
        ...
        return {
            ...
            "summary": {
                ...
                "total_memory_mb": sum(
                    n["memory_limit_mb"] for n in node_status.values()
                ),
                "used_memory_mb": sum(
                    n["memory_used_mb"] for n in node_status.values()
                ),
            },
            "retrieved_at": datetime.utcnow().isoformat(),
        }
...
async def get_cache_configuration(current_user: User = Depends(get_current_user)):
...
async def cache_health_check() -> None:
