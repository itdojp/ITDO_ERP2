...
from app.services.event_streaming_engine import (
    streaming_engine, EventType, EventPriority, ProcessingStatus,
    DeliveryMode, Event, BaseEventHandler
)
...
# Event Publishing
@router.post("/publish", response_model=Dict[str, Any], status_code=status.HTTP_201_CREATED)
...
async def list_event_subscriptions(
    ...
    query = query.offset(skip).limit(limit).order_by(EventSubscription.created_at.desc())
...
# Event Handlers
@router.post("/handlers", response_model=EventHandlerResponse, status_code=status.HTTP_201_CREATED)
...
async def get_event_history(
    ...
    if event_type:
        ...
        for event in events:
            ...
            filtered_events.append(EventResponse(
                id=event.id,
                type=event.type,
                source=event.source,
...
async def get_event_metrics(
    ...
    event_counts = await db.execute(
        select(
            ...
            func.count(EventStream.id).label('count')
        )
        .where(
            and_(
                EventStream.timestamp >= start_time,
                EventStream.timestamp <= end_time
            )
        )
    ...
    processing_stats = await db.execute(
        select(
            ...
            func.count(ProcessedEvent.id).filter(
                ProcessedEvent.status == ProcessingStatus.SUCCESS
            ).label('successful'),
            func.count(ProcessedEvent.id).filter(
