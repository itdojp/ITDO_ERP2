...
class InventoryBalance(BaseTable):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "product_id", "location_id", "lot_number", name="uq_product_location_lot"
        ),
...
class InventoryMovement(BaseTable):
    ...
    inventory_balance_id = Column(
        UUID(as_uuid=True), ForeignKey("inventory_balances.id")
    )
    ...
    location = relationship(
        "Location", foreign_keys=[location_id], back_populates="movements"
    )
...
class StockAdjustment(BaseTable):
    ...
    )  # cycle_count, physical_count, adjustment
...
class StockAdjustmentLine(BaseTable):
    ...
    adjustment_id = Column(
        UUID(as_uuid=True), ForeignKey("stock_adjustments.id"), nullable=False
    )
    product_id = Column(UUID(as_uuid=True), nullable=False)
...
class InventoryAlert(BaseTable):
    ...
    alert_type = Column(
        String(50), nullable=False
    )  # low_stock, overstock, expiry, etc.
...
class InventoryCycleItem(BaseTable):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    cycle_id = Column(
        UUID(as_uuid=True), ForeignKey("inventory_cycles.id"), nullable=False
    )
    product_id = Column(UUID(as_uuid=True), nullable=False)
    ...
    )  # pending, counted, verified, adjusted
    notes = Column(Text)
...
class InventoryManagementService:
    ...
    async def create_location(self, location_data: LocationCreate) -> Location:
        ...
        if location_data.parent_id:
            parent_query = select(Location).where(
        ...
        location = Location(**location_data.dict(), level=level, path=path)
    ...
    async def update_inventory_balance(
        ...
        if new_quantity < 0 and movement_type in [
        ...
        if unit_cost:
            if movement_type in [MovementType.RECEIPT, MovementType.PRODUCTION]:
                ...
                old_total_cost = (
                    balance.average_cost * old_quantity
                    if balance.average_cost
                    else Decimal("0")
                )
                new_total_cost = old_total_cost + (unit_cost * abs(quantity_change))
                balance.average_cost = (
                    new_total_cost / balance.quantity_on_hand
            ...
            balance.total_cost = (
                balance.quantity_on_hand * balance.average_cost
                if balance.average_cost
                else Decimal("0")
            )
    ...
    async def create_stock_transfer(
        ...
        if not from_location.accepts_shipments:
            raise HTTPException(
                status_code=400, detail="From location does not accept shipments"
            )
        if not to_location.accepts_receipts:
            raise HTTPException(
                status_code=400, detail="To location does not accept receipts"
            )
        ...
        if not from_balance or from_balance.quantity_available < quantity:
            raise HTTPException(
                status_code=400, detail="Insufficient inventory at source location"
            )
    ...
    async def create_stock_adjustment(
        self, adjustment_data: StockAdjustmentCreate
    ) -> StockAdjustment:
        ...
        for line_data in adjustment_data.adjustment_lines:
            ...
            adj_line = StockAdjustmentLine(
                ...
                variance_value=variance_qty * balance.unit_cost
                if balance and balance.unit_cost
                else Decimal("0"),
    ...
    async def _check_inventory_alerts(self, balance: InventoryBalance) -> dict:
        ...
        if (
            ...
            and balance.quantity_available <= balance.reorder_point
        ):
            alerts.append(
                {
                    "alert_type": "low_stock",
                    "severity": "high"
                    if balance.quantity_available <= balance.minimum_stock
                    else "medium",
    ...
    async def generate_demand_forecast(
        ...
        historical_data = await self._get_historical_demand(
            request.product_id, request.location_id, request.historical_periods
        )

        if len(historical_data) < 10:
            raise HTTPException(
                status_code=400, detail="Insufficient historical data for forecasting"
            )
        ...
        accuracy = await self._calculate_forecast_accuracy(
            historical_data, forecasts[: len(historical_data)]
        )
    ...
    async def _get_historical_demand(
        self, product_id: uuid.UUID, location_id: Optional[uuid.UUID], periods: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _moving_average_forecast(
        self, historical_data: List[Dict[str, Any]], periods: int, window: int = 7
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(periods):
            ...
            std_dev = (
                sum((x - avg_demand) ** 2 for x in recent_demands) / len(recent_demands)
            ) ** 0.5
    ...
    async def _exponential_smoothing_forecast(
        self, historical_data: List[Dict[str, Any]], periods: int, alpha: float = 0.3
    ) -> List[Dict[str, Any]]:
    ...
    async def _linear_regression_forecast(
        self, historical_data: List[Dict[str, Any]], periods: int
    ) -> List[Dict[str, Any]]:
    ...
    async def _calculate_forecast_accuracy(
        self, historical_data: List[Dict[str, Any]], forecasts: List[Dict[str, Any]]
    ) -> Decimal:
    ...
    async def _generate_forecast_recommendations(
        self, forecasts: List[Dict[str, Any]], historical_data: List[Dict[str, Any]]
    ) -> List[str]:
        ...
        avg_historical = sum(d["demand"] for d in historical_data) / len(
            historical_data
        )
        ...
        if avg_forecast > avg_historical * 1.2:
            recommendations.append(
                "Demand is expected to increase significantly. Consider increasing safety stock."
            )
        elif avg_forecast < avg_historical * 0.8:
            recommendations.append(
                "Demand is expected to decrease. Consider reducing inventory levels."
            )

        # Check for seasonality
        if len(forecasts) >= 7:
            weekly_variation = max(f["demand"] for f in forecasts[:7]) - min(
                f["demand"] for f in forecasts[:7]
            )
            if weekly_variation > avg_forecast * 0.3:
                recommendations.append(
                    "High demand variability detected. Implement dynamic safety stock."
                )
...
async def get_inventory_balances(
    ...
    balances = await service.get_inventory_balance(
        product_id, location_id, status, include_zero
    )
    return balances
...
async def create_inventory_movement(
    ...
    balance = await service.update_inventory_balance(
        ...
        if movement_data.movement_type
        in [MovementType.RECEIPT, MovementType.PRODUCTION]
    ...
    movement_query = (
        ...
        .where(InventoryMovement.inventory_balance_id == balance.id)
