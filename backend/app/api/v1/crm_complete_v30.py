...
from app.crud.crm_extended_v30 import (
    CRMCustomerCRUD, CustomerInteractionCRUD, SalesOpportunityCRUD, OpportunityActivityCRUD,
    CustomerActivityCRUD, CustomerSegmentCRUD, MarketingCampaignCRUD,
    NotFoundError, DuplicateError, InvalidStageError
)
from app.schemas.crm_complete_v30 import (
    CRMCustomerCreate, CRMCustomerUpdate, CRMCustomerResponse, CRMCustomerListResponse,
    CustomerInteractionCreate, CustomerInteractionUpdate, CustomerInteractionResponse, CustomerInteractionListResponse,
    SalesOpportunityCreate, SalesOpportunityUpdate, SalesOpportunityResponse, SalesOpportunityListResponse,
    ...
    CRMStatsResponse, CRMAnalyticsResponse
)
...
def list_crm_customers(
    ...
    customer_crud = CRMCustomerCRUD(db)
    customers, total = customer_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
@router.post("/interactions", response_model=CustomerInteractionResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/customers/{customer_id}/interactions", response_model=CustomerInteractionListResponse)
def list_customer_interactions(
    ...
    skip = (page - 1) * per_page
    interaction_crud = CustomerInteractionCRUD(db)
    interactions, total = interaction_crud.get_multi_by_customer(customer_id, skip=skip, limit=per_page)
...
@router.put("/interactions/{interaction_id}", response_model=CustomerInteractionResponse)
...
@router.post("/opportunities", response_model=SalesOpportunityResponse, status_code=status.HTTP_201_CREATED)
...
def list_sales_opportunities(
    ...
    opportunity_crud = SalesOpportunityCRUD(db)
    opportunities, total = opportunity_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
@router.post("/activities/opportunities", response_model=OpportunityActivityResponse, status_code=status.HTTP_201_CREATED)
...
@router.post("/activities/customers", response_model=CustomerActivityResponse, status_code=status.HTTP_201_CREATED)
...
def get_crm_stats(
    ...
    active_customers = db.query(CRMCustomer).filter(CRMCustomer.is_active == True).count()
    qualified_leads = db.query(CRMCustomer).filter(CRMCustomer.is_qualified == True).count()
    
    total_opportunities = db.query(SalesOpportunity).count()
    open_opportunities = db.query(SalesOpportunity).filter(SalesOpportunity.status == "open").count()
    
    # Pipeline metrics
    pipeline_query = db.query(SalesOpportunity).filter(SalesOpportunity.status == "open")
    total_pipeline_value = pipeline_query.with_entities(func.sum(SalesOpportunity.amount)).scalar() or Decimal('0')
    weighted_pipeline_value = pipeline_query.with_entities(func.sum(SalesOpportunity.weighted_amount)).scalar() or Decimal('0')
    
    avg_deal_size = total_pipeline_value / open_opportunities if open_opportunities > 0 else Decimal('0')
    
    # Win rate calculation
    closed_opportunities = db.query(SalesOpportunity).filter(
        SalesOpportunity.status.in_(["won", "lost"])
    ).count()
    won_opportunities = db.query(SalesOpportunity).filter(SalesOpportunity.status == "won").count()
    win_rate = (won_opportunities / closed_opportunities * 100) if closed_opportunities > 0 else Decimal('0')
    ...
    stage_stats = db.query(
        SalesOpportunity.stage, func.count(SalesOpportunity.id)
    ).filter(SalesOpportunity.status == "open").group_by(SalesOpportunity.stage).all()
    by_stage = {stage: count for stage, count in stage_stats}
    
    # Source breakdown
    source_stats = db.query(
        CRMCustomer.lead_source, func.count(CRMCustomer.id)
    ).filter(CRMCustomer.lead_source.isnot(None)).group_by(CRMCustomer.lead_source).all()
    by_source = {source: count for source, count in source_stats}
    
    # Segment breakdown
    segment_stats = db.query(
        CRMCustomer.customer_segment, func.sum(CRMCustomer.lifetime_value)
    ).filter(CRMCustomer.customer_segment.isnot(None)).group_by(CRMCustomer.customer_segment).all()
    ...
    ).group_by(SalesOpportunity.owner_id).order_by(
        func.sum(SalesOpportunity.amount).desc()
    ).limit(5).all()
...
def get_crm_analytics(
    ...
    customer_query = db.query(CRMCustomer).filter(
        CRMCustomer.created_at >= date_from,
        CRMCustomer.created_at <= date_to
    )
    ...
    lead_conversion_rate = (qualified_leads / new_customers * 100) if new_customers > 0 else Decimal('0')
    opportunity_win_rate = (deals_won / (deals_won + deals_lost) * 100) if (deals_won + deals_lost) > 0 else Decimal('0')
    
    won_opportunities = opp_query.filter(SalesOpportunity.status == "won").all()
    avg_deal_size = revenue_generated / len(won_opportunities) if won_opportunities else Decimal('0')
    
    customer_acquisition_cost = Decimal('0')  # would need campaign cost data
    lifetime_value = db.query(func.avg(CRMCustomer.lifetime_value)).scalar() or Decimal('0')
    ...
    while current_date <= end_date:
        ...
        day_customers = db.query(CRMCustomer).filter(
            CRMCustomer.created_at >= day_start,
            CRMCustomer.created_at <= day_end
        ).count()
        
        day_opportunities = db.query(SalesOpportunity).filter(
    ...
    # Performance by rep
    performance_by_rep = db.query(
        SalesOpportunity.owner_id,
        func.count(case([(SalesOpportunity.status == "won", 1)])).label("deals_won"),
        func.count(case([(SalesOpportunity.status == "lost", 1)])).label("deals_lost"),
        func.sum(case([(SalesOpportunity.status == "won", SalesOpportunity.amount)], else_=0)).label("revenue")
    ).filter(
        SalesOpportunity.created_at >= date_from,
        SalesOpportunity.created_at <= date_to
    ).group_by(SalesOpportunity.owner_id).all()
    ...
    performance_data = [
        {
            ...
            "win_rate": (won / (won + lost) * 100) if (won + lost) > 0 else Decimal('0')
        }
