...
from app.models.order import (
    Order, OrderItem, OrderShipment, OrderPayment, OrderNote, OrderHistory,
    OrderDiscount, OrderTax, OrderAddress, QuoteRequest, QuoteItem
)
...
class OrderDiscountCreate(BaseModel):
    ...
    def validate_discount_value(cls, v, values, field):
        discount_type = values.get('discount_type')
        if discount_type == DiscountType.PERCENTAGE and field.name == 'percentage' and v is None:
            raise ValueError('Percentage is required for percentage discount type')
        elif discount_type == DiscountType.FIXED_AMOUNT and field.name == 'amount' and v is None:
...
async def generate_order_number(db: AsyncSession, prefix: str = "ORD") -> str:
    ...
    result = await db.execute(
        ...
        .where(Order.order_number.like(f"{full_prefix}%"))
    )
...
async def check_inventory_availability(db: AsyncSession, variant_id: UUID, quantity: int) -> Dict[str, Any]:
    """Check inventory availability for a product variant"""
    result = await db.execute(
        select(InventoryItem, func.sum(InventoryItem.quantity - InventoryItem.reserved_quantity))
    ...
    if total_available >= quantity:
        return {"available": True, "total_available": total_available, "status": "in_stock"}
    elif total_available > 0:
        return {"available": False, "total_available": total_available, "status": "insufficient_stock"}
    else:
...
async def calculate_order_totals(items: List[OrderItem], discounts: List[OrderDiscount]) -> Dict[str, Decimal]:
...
async def update_order_status_history(db: AsyncSession, order_id: UUID, old_status: str, new_status: str, user_id: UUID, notes: Optional[str] = None):
...
async def create_order(
    order: OrderCreate,
    auto_confirm: bool = Query(False, description="Automatically confirm order if inventory available"),
    ...
    inventory_issues = []
    for item in order.items:
        availability = await check_inventory_availability(db, item.product_variant_id, item.quantity)
    ...
    initial_status = OrderStatus.CONFIRMED if auto_confirm and not inventory_issues else OrderStatus.PENDING
    ...
    for item_data in order.items:
        ...
        if item_data.discount_percentage:
            discount_amount = (unit_price * item_data.quantity) * (item_data.discount_percentage / 100)
...
async def list_orders(
    ...
    date_from: Optional[date] = Query(None, description="Filter orders created after this date"),
    date_to: Optional[date] = Query(None, description="Filter orders created before this date"),
    search: Optional[str] = Query(None, min_length=1, description="Search in order number or PO number"),
    sort_by: str = Query("created_at", regex="^(order_number|customer|status|total_amount|created_at)$"),
    ...
    for order in orders:
        ...
        if order.customer:
            customer_name = order.customer.company_name or f"{order.customer.customer_number}"
...
async def get_order(
    ...
    for item in order.items:
        ...
        item_response = OrderItemResponse(
            ...
            product_name=item.product_variant.product.name if item.product_variant.product else "Unknown Product",
    ...
    tracking_numbers = [shipment.tracking_number for shipment in order.shipments if shipment.tracking_number]
...
async def update_order(
    ...
    if db_order.status in [OrderStatus.SHIPPED, OrderStatus.DELIVERED, OrderStatus.COMPLETED, OrderStatus.CANCELLED]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
...
async def cancel_order(
    ...
    if order.status in [OrderStatus.SHIPPED, OrderStatus.DELIVERED, OrderStatus.COMPLETED, OrderStatus.CANCELLED]:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
    ...
    await update_order_status_history(
        db, order_id, old_status.value, OrderStatus.CANCELLED.value, current_user.id, f"Cancelled: {reason}"
    )
    
    await db.commit()

# Shipment Endpoints
@router.post("/{order_id}/shipments", response_model=OrderShipmentResponse, status_code=status.HTTP_201_CREATED)
async def create_shipment(
    ...
    if order.status != OrderStatus.SHIPPED:
        ...
        await update_order_status_history(
            db, order_id, old_status.value, OrderStatus.SHIPPED.value, current_user.id, "Shipment created"
        )
...
# Payment Endpoints
@router.post("/{order_id}/payments", response_model=OrderPaymentResponse, status_code=status.HTTP_201_CREATED)
async def create_payment(
    ...
    total_payments = await db.execute(
        select(func.sum(OrderPayment.amount))
        .where(
            and_(
...
@router.post("/quotes/{quote_id}/convert", response_model=OrderResponse, status_code=status.HTTP_201_CREATED)
...
async def export_orders(
    ...
    for order in orders:
        ...
        if order.customer:
            customer_name = order.customer.company_name or order.customer.customer_number
...
async def validate_order(
    ...
    for item in order.items:
        ...
        if not availability["available"]:
            ...
            validation_results["issues"].append(f"Insufficient inventory for item {item.id}")
    ...
    validation_results["total_validation_score"] = max(0, validation_results["total_validation_score"])
