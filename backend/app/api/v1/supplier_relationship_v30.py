...
from app.crud.supplier_relationship_v30 import (
    SupplierRelationshipCRUD, SupplierPerformanceReviewCRUD, SupplierNegotiationCRUD,
    SupplierContractCRUD, NotFoundError, DuplicateError, InvalidStatusError
)
from app.schemas.supplier_relationship_v30 import (
    SupplierRelationshipCreate, SupplierRelationshipUpdate, SupplierRelationshipResponse, SupplierRelationshipListResponse,
    SupplierPerformanceReviewCreate, SupplierPerformanceReviewUpdate, SupplierPerformanceReviewResponse, SupplierPerformanceReviewListResponse,
    SupplierNegotiationCreate, SupplierNegotiationUpdate, SupplierNegotiationResponse, SupplierNegotiationListResponse,
    SupplierContractCreate, SupplierContractUpdate, SupplierContractResponse, SupplierContractListResponse,
    SupplierRelationshipStatsResponse, SupplierPerformanceAnalyticsResponse
)

router = APIRouter(prefix="/supplier-relationships", tags=["supplier-relationships"])

@router.post("/relationships", response_model=SupplierRelationshipResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/relationships/{relationship_id}", response_model=SupplierRelationshipResponse)
def get_supplier_relationship(
    ...
    if not relationship:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Supplier relationship not found")
    return relationship
...
def list_supplier_relationships(
    ...
    relationship_crud = SupplierRelationshipCRUD(db)
    relationships, total = relationship_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
@router.put("/relationships/{relationship_id}", response_model=SupplierRelationshipResponse)
...
@router.post("/relationships/{relationship_id}/approve", response_model=SupplierRelationshipResponse)
...
@router.post("/performance-reviews", response_model=SupplierPerformanceReviewResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/performance-reviews/{review_id}", response_model=SupplierPerformanceReviewResponse)
...
@router.get("/relationships/{relationship_id}/performance-reviews", response_model=SupplierPerformanceReviewListResponse)
def list_relationship_performance_reviews(
    ...
    skip = (page - 1) * per_page
    review_crud = SupplierPerformanceReviewCRUD(db)
    reviews, total = review_crud.get_multi_by_relationship(relationship_id, skip=skip, limit=per_page)
...
@router.put("/performance-reviews/{review_id}", response_model=SupplierPerformanceReviewResponse)
...
@router.post("/performance-reviews/{review_id}/approve", response_model=SupplierPerformanceReviewResponse)
...
@router.post("/negotiations", response_model=SupplierNegotiationResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/negotiations/{negotiation_id}", response_model=SupplierNegotiationResponse)
...
def list_supplier_negotiations(
    ...
    negotiation_crud = SupplierNegotiationCRUD(db)
    negotiations, total = negotiation_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
@router.put("/negotiations/{negotiation_id}", response_model=SupplierNegotiationResponse)
...
@router.post("/negotiations/{negotiation_id}/complete", response_model=SupplierNegotiationResponse)
...
@router.post("/contracts", response_model=SupplierContractResponse, status_code=status.HTTP_201_CREATED)
...
def list_supplier_contracts(
    ...
    contract_crud = SupplierContractCRUD(db)
    contracts, total = contract_crud.get_multi(skip=skip, limit=per_page, filters=filters)
...
@router.get("/performance-analytics", response_model=SupplierPerformanceAnalyticsResponse)
def get_supplier_performance_analytics(
    ...
    from app.models.supplier_relationship_extended import SupplierRelationship, SupplierPerformanceReview
    ...
    avg_quality_rating = sum(quality_ratings) / len(quality_ratings) if quality_ratings else Decimal('0')
    avg_delivery_rating = sum(delivery_ratings) / len(delivery_ratings) if delivery_ratings else Decimal('0')
    avg_service_rating = sum(service_ratings) / len(service_ratings) if service_ratings else Decimal('0')
    avg_overall_rating = sum(overall_ratings) / len(overall_ratings) if overall_ratings else Decimal('0')
    
    # Top performing suppliers
    supplier_ratings = {}
    for review in reviews:
        if review.overall_rating and review.supplier_relationship_id not in supplier_ratings:
            supplier_ratings[review.supplier_relationship_id] = []
        if review.overall_rating:
            supplier_ratings[review.supplier_relationship_id].append(review.overall_rating)
    ...
    top_performers = sorted(supplier_avg_ratings.items(), key=lambda x: x[1], reverse=True)[:5]
    ...
    while current_date <= date_to.date():
        month_end = (current_date.replace(month=current_date.month % 12 + 1, day=1) - timedelta(days=1)) if current_date.month < 12 else current_date.replace(year=current_date.year + 1, month=1, day=1) - timedelta(days=1)
        
        month_reviews = [r for r in reviews 
                        if current_date <= r.created_at.date() <= month_end]
        
        if month_reviews:
            month_avg = sum(r.overall_rating for r in month_reviews if r.overall_rating) / len([r for r in month_reviews if r.overall_rating])
        else:
            month_avg = Decimal('0')
