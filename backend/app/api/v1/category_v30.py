...
from app.crud.category_v30 import (
    CategoryAttributeCRUD,
    CategoryCRUD,
    CategoryPricingRuleCRUD,
    DuplicateError,
...
from app.schemas.category_v30 import (
    ...
    CategoryHierarchyListResponse,
    CategoryImportRequest,
    CategoryListResponse,
    CategoryMoveRequest,
    CategoryPricingRuleCreate,
    CategoryPricingRuleListResponse,
    CategoryPricingRuleResponse,
...
def list_categories(
    ...
    category_crud = CategoryCRUD(db)
    categories, total = category_crud.get_multi(
    ...
    return CategoryListResponse(
        total=total, page=page, per_page=per_page, pages=pages, items=categories
    )
...
def move_category(
    ...
    try:
        ...
        return category_crud.move_category(
            move_request.category_id, move_request.new_parent_id, current_user["sub"]
        )
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except InvalidOperationError as e:
...
def delete_category(
    ...
    try:
        ...
        return (
            ...
            if success
            else {"message": "Failed to delete category"}
        )
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
    except InvalidOperationError as e:
...
def delete_category_attribute(
    ...
    try:
        ...
        return (
            ...
            if success
            else {"message": "Failed to delete attribute"}
        )
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
@router.post(
    "/{category_id}/pricing-rules",
    response_model=CategoryPricingRuleResponse,
    status_code=status.HTTP_201_CREATED,
)
...
@router.get(
    "/{category_id}/pricing-rules", response_model=List[CategoryPricingRuleResponse]
)
...
def list_all_pricing_rules(
    ...
    return CategoryPricingRuleListResponse(
        total=total, page=page, per_page=per_page, pages=pages, items=rules
    )
...
@router.post(
    "/pricing-rules/{rule_id}/activate", response_model=CategoryPricingRuleResponse
)
...
@router.post(
    "/pricing-rules/{rule_id}/deactivate", response_model=CategoryPricingRuleResponse
)
...
def delete_pricing_rule(
    ...
    try:
        ...
        return (
            ...
            if success
            else {"message": "Failed to delete pricing rule"}
        )
    except NotFoundError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
...
def get_category_hierarchy_view(
    ...
    """カテゴリ階層ビュー取得"""
    hierarchy_views = (
        db.query(CategoryHierarchyView)
        .order_by(
            CategoryHierarchyView.hierarchy_level, CategoryHierarchyView.full_path
        )
        .all()
    )

    return CategoryHierarchyListResponse(
        total=len(hierarchy_views), items=hierarchy_views
    )
...
def bulk_category_operations(
    ...
    for category_id in operation_request.category_ids:
        try:
            ...
            results["failed"].append({"category_id": category_id, "error": str(e)})
