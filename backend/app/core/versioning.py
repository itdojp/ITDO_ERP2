...
def generate_deprecation_warning(
    version: str, deprecation_date: datetime, removal_date: datetime
) -> str:
...
class VersionInfo:
    ...
    def is_deprecated(self) -> bool:
        ...
        return (
            ...
            and datetime.now() >= self.deprecation_date
        )
    ...
    def get_deprecation_header(self) -> str:
        ...
        if self.deprecation_date:
            header_parts.append(
                f'date="{self.deprecation_date.strftime("%a, %d %b %Y %H:%M:%S GMT")}"'
            )

        if self.removal_date:
            header_parts.append(
                f'sunset="{self.removal_date.strftime("%a, %d %b %Y %H:%M:%S GMT")}"'
            )
...
class APIVersionManager:
    ...
    def register_version(self, version: str, config: Dict[str, Any]) -> None:
    ...
    def deprecate_version(
        self, version: str, deprecation_date: datetime, removal_date: datetime
    ) -> None:
        ...
        logger.warning(
            f"Version {version} deprecated. Removal scheduled for {removal_date}"
        )
    ...
    def get_deprecation_info(self, version: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            if info.deprecation_date and info.removal_date
            else None,
        }
    ...
    def detect_breaking_changes(
        ...
        if removed_paths:
            breaking_changes.extend(
                [f"Removed endpoint: {path}" for path in removed_paths]
            )
        ...
        for path in old_paths.intersection(new_paths):
            ...
            for method in old_path_schema.keys():
                if method in new_path_schema:
                    old_props = self._extract_response_properties(
                        old_path_schema[method]
                    )
                    new_props = self._extract_response_properties(
                        new_path_schema[method]
                    )
...
class DeprecationWarningMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        ) or get_version_from_request(request)
        ...
        if self.version_manager.is_deprecated(version):
            ...
            if deprecation_info:
                response.headers["Deprecation"] = (
                    self.version_manager.get_deprecation_header(version)
                )
                ...
                if deprecation_info.get("warning"):
                    response.headers["Warning"] = (
                        f'299 - "{deprecation_info["warning"]}"'
                    )
