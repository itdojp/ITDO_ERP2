...
class IntelligentDataDiscovery:
    ...
    def _initialize_patterns(self) -> dict:
        ...
        self.pattern_library = {
            ...
            "credit_card": [r"\b\d{4}\s*\d{4}\s*\d{4}\s*\d{4}\b", r"\b\d{16}\b"],
            ...
            "currency": [r"\$\d+\.?\d*", r"\b\d+\.?\d*\s*(USD|EUR|GBP|JPY)\b"],
        }
            ...
            with engine.connect() as conn:
                ...
                for table in tables:
                    ...
                    try:
                        ...
                        classification = await self._classify_data_asset(
                            sample_data, table_name, schema_info
                        )
                        sensitivity = await self._detect_sensitivity(
                            sample_data, schema_info
                        )
        ...
        try:
            ...
            for file_path in data_files:
                try:
                    ...
                    # Classify asset
                    classification = await self._classify_data_asset(
                        sample_data, file_path.name, schema_info
                    )
                    sensitivity = await self._detect_sensitivity(
                        sample_data, schema_info
                    )
    ...
    async def _classify_data_asset(
        self, data: pd.DataFrame, asset_name: str, schema: Dict[str, Any]
    ) -> DataClassification:
        ...
        has_confidential_keywords = await self._detect_confidential_keywords(
            asset_name, data
        )
        ...
        if has_pii or "ssn" in asset_name.lower() or "personal" in asset_name.lower():
            ...
            has_financial
            or "payment" in asset_name.lower()
            or "transaction" in asset_name.lower()
        ):
    ...
    async def _detect_confidential_keywords(
        self, asset_name: str, data: pd.DataFrame
    ) -> bool:
...
class DataLineageTracker:
    ...
    def add_lineage_relationship(self, lineage: DataLineage) -> dict:
        ...
        logging.info(
            f"Added lineage: {lineage.source_asset_id} -> {lineage.target_asset_id}"
        )

    def trace_upstream_lineage(
                    ...
                    if nx.has_path(self.lineage_graph, source, asset_id):
                        try:
                            path = nx.shortest_path(
                                self.lineage_graph, source, asset_id
                            )
                            if (
                                len(path) == depth + 1
                            ):  # depth + 1 because path includes source and target
                                upstream_assets.append(
                                    {
                                        "asset_id": source,
                                        "path": path,
                                        "depth": depth,
                                        "transformations": self._get_path_transformations(
                                            path
                                        ),
                                    }
                                )
            ...
            for depth in range(1, max_depth + 1):
                for target in self.lineage_graph.nodes():
                    if nx.has_path(self.lineage_graph, asset_id, target):
                        try:
                            path = nx.shortest_path(
                                self.lineage_graph, asset_id, target
                            )
                            if len(path) == depth + 1:
    ...
    def get_lineage_impact_analysis(self, asset_id: str) -> Dict[str, Any]:
        ...
        max_depth = max(
            [asset["depth"] for asset in downstream["downstream_assets"]], default=0
        )
...
class DataGovernanceEngine:
    ...
    def add_governance_policy(self, policy: GovernancePolicy) -> dict:
        """Add data governance policy"""
        self.policies[policy.id] = policy
        logging.info(f"Added governance policy: {policy.name}")

    def add_quality_rule(self, rule: DataQualityRule) -> dict:
    ...
    async def evaluate_policy_compliance(
        self, asset: DataAsset, action: GovernanceAction, user_id: str
    ) -> Dict[str, Any]:
    ...
    def _policy_applies_to_asset(
        self, policy: GovernancePolicy, asset: DataAsset
    ) -> bool:
        ...
        for scope_pattern in policy.scope:
            ...
            if scope_pattern == "*" or scope_pattern == asset.id:
                return True
            elif scope_pattern.endswith("*") and asset.id.startswith(
                scope_pattern[:-1]
            ):
                return True
    ...
    async def evaluate_data_quality(
        self, asset: DataAsset, data: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    async def _evaluate_quality_rule(
        self, data: pd.DataFrame, rule: DataQualityRule
    ) -> Dict[str, Any]:
        ...
        try:
            if rule.rule_type == "completeness":
                ...
                if rule.column == "*":
                    ...
                    # Column completeness
                    completeness = (
                        (len(data) - data[rule.column].isnull().sum()) / len(data)
                    ) * 100

                result["metric_value"] = completeness
                result["passed"] = completeness >= rule.threshold
                result["description"] = (
                    f"Completeness: {completeness:.2f}% (threshold: {rule.threshold}%)"
                )
                ...
                if rule.column in data.columns:
                    ...
                    uniqueness = (
                        (unique_count / total_count) * 100 if total_count > 0 else 0
                    )

                    result["metric_value"] = uniqueness
                    result["passed"] = uniqueness >= rule.threshold
                    result["description"] = (
                        f"Uniqueness: {uniqueness:.2f}% (threshold: {rule.threshold}%)"
                    )
                ...
                if rule.column in data.columns:
                    if "regex" in rule.condition:
                        ...
                        valid_count = (
                            data[rule.column].astype(str).str.match(pattern).sum()
                        )
                        ...
                        result["description"] = (
                            f"Validity (regex): {validity:.2f}% (threshold: {rule.threshold}%)"
                        )
                ...
                if rule.condition and "=" in rule.condition:
                    ...
                    if col1 in data.columns and col2 in data.columns:
                        ...
                        result["description"] = (
                            f"Consistency: {consistency:.2f}% (threshold: {rule.threshold}%)"
                        )
    ...
    def generate_compliance_report(
        ...
        violations = len(
            [
                ...
                if log.details.get("compliance_check")
                and not log.details.get("compliant")
            ]
        )
        ...
        report = {
            ...
            "summary": {
                ...
                "compliance_rate": ((total_actions - violations) / total_actions * 100)
                if total_actions > 0
                else 100,
            },
            ...
            "active_quality_rules": len(
                [r for r in self.quality_rules.values() if r.enabled]
            ),
        }
...
class IntelligentDataCatalogSystem:
    ...
    def _initialize_default_governance(self) -> dict:
    ...
    async def discover_assets_auto(
        ...
        for source in sources:
            ...
            try:
                if source_type == "database":
                    assets = await self.discovery_engine.discover_database_assets(
                        source_location
                    )
                elif source_type == "filesystem":
                    assets = await self.discovery_engine.discover_file_assets(
                        source_location
                    )
                else:
                    continue
                ...
                discovery_results["errors"].append(
                    {"source": source_location, "error": str(e)}
                )

        return discovery_results

    async def _update_search_index(self, asset: DataAsset) -> dict:
    ...
    async def search_assets(
        ...
        results.sort(
            key=lambda x: len([t for t in query_tokens if t in x["name"].lower()]),
            reverse=True,
        )
    ...
    def _asset_matches_filters(self, asset: DataAsset, filters: Dict[str, Any]) -> bool:
        ...
        if (
            "sensitivity" in filters
            and asset.sensitivity.value not in filters["sensitivity"]
        ):
            return False
        ...
        for other_id, other_asset in self.data_assets.items():
            ...
            if asset.tags and other_asset.tags:
                tag_similarity = len(common_tags) / len(
                    set(asset.tags).union(set(other_asset.tags))
                )
    ...
    async def validate_asset_governance(
        self, asset_id: str, action: GovernanceAction, user_id: str
    ) -> Dict[str, Any]:
        ...
        return await self.governance_engine.evaluate_policy_compliance(
            asset, action, user_id
        )

    async def assess_data_quality(
        self, asset_id: str, data: pd.DataFrame
    ) -> Dict[str, Any]:
        ...
        quality_results = await self.governance_engine.evaluate_data_quality(
            asset, data
        )
    ...
    def get_catalog_metrics(self) -> Dict[str, Any]:
        ...
        by_classification = Counter(
            [asset.classification.value for asset in self.data_assets.values()]
        )

        # Group by sensitivity
        by_sensitivity = Counter(
            [asset.sensitivity.value for asset in self.data_assets.values()]
        )
        ...
        return {
            ...
            "active_policies": len(
                [p for p in self.governance_engine.policies.values() if p.enabled]
            ),
            "search_index_size": sum(
                len(assets) for assets in self.search_index.values()
            ),
        }
    ...
    def get_system_status(self) -> Dict[str, Any]:
        ...
        recent_logs = [
            log
            for log in self.governance_engine.audit_logs
...
async def main() -> None:
    ...
    recommendations = await catalog_system.get_asset_recommendations(
        "customer_data_001"
    )
    ...
    compliance = await catalog_system.validate_asset_governance(
        "customer_data_001", GovernanceAction.ACCESSED, "analyst"
    )
    ...
    quality_results = await catalog_system.assess_data_quality(
        "customer_data_001", sample_data
    )
    ...
    upstream = catalog_system.lineage_tracker.trace_upstream_lineage(
        "transaction_data_001"
    )
    print(f"Upstream assets: {upstream['total_upstream']}")

    downstream = catalog_system.lineage_tracker.trace_downstream_lineage(
        "customer_data_001"
    )
    ...
    compliance_report = catalog_system.governance_engine.generate_compliance_report(
        start_date, end_date
    )
