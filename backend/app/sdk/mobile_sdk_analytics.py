...
class PerformanceMonitor:
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            'measurement_period_minutes': len(self._metrics_buffer) / 6,  # Assuming 10s intervals
            'cpu': {
...
class UserBehaviorAnalyzer:
    ...
    def __init__(self):
        ...
        self._feature_usage: Dict[str, Dict[str, int]] = defaultdict(lambda: defaultdict(int))
    ...
    def end_session(self, session_id: str) -> Optional[SessionInfo]:
        ...
        if session:
            ...
            if session.api_calls > 0:
                session.avg_response_time_ms = session.total_duration_ms / session.api_calls
    ...
    def track_api_call(
        ...
        if session:
            ...
            session.avg_response_time_ms = (
                (current_avg * (session.api_calls - 1) + response_time_ms) / session.api_calls
            )
    ...
    def get_user_behavior_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        total_duration_hours = sum(s.total_duration_ms for s in user_sessions) / 1000 / 3600
        ...
        return {
            ...
            'avg_session_duration_minutes': (total_duration_hours * 60) / total_sessions if total_sessions > 0 else 0,
    ...
    def get_screen_flow_analysis(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'popular_screens': [
                {'screen': screen[0], 'visits': screen[1]}
                for screen in popular_screens
            ],
            'total_flows': len(self._screen_flows),
            'unique_screens': len(set(
                screen for flow in self._screen_flows.keys() for screen in flow
            )),
        }
...
class FunnelAnalyzer:
    ...
    def get_funnel_report(self, funnel_id: str) -> Optional[Dict[str, Any]]:
        ...
        for i in range(1, len(funnel.steps)):
            ...
            if drop_off_rate > 20:  # Significant drop-off
                drop_offs.append({
                    ...
                    'users_lost': prev_step.users_completed - current_step.users_completed,
                })
        ...
        return {
            ...
            'total_users_converted': funnel.steps[-1].users_completed if funnel.steps else 0,
            'steps': [
                {
            ...
            'last_calculated': funnel.last_calculated.isoformat() if funnel.last_calculated else None,
        }
...
class AnalyticsCollector:
    ...
    def start_session(self, user_id: Optional[str] = None) -> str:
        ...
        device_id = (
            ...
            if self.sdk.auth_manager.device_info else None
        )
    ...
    def _end_screen_view(self) -> None:
        ...
        if self._current_screen and self._screen_start_time:
            duration_ms = (datetime.now() - self._screen_start_time).total_seconds() * 1000
