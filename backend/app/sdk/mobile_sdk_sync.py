...
from .mobile_sdk_core import MobileERPSDK, SDKError, NetworkError, ValidationError
...
class OfflineStorage:
    ...
    def _initialize_database(self) -> None:
        ...
        with self._get_connection() as conn:
            ...
            # Create indexes
            conn.execute("CREATE INDEX IF NOT EXISTS idx_entity_data_type_id ON entity_data(entity_type, entity_id)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_entity_data_updated ON entity_data(updated_at)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sync_ops_type ON sync_operations(entity_type)")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_sync_ops_status ON sync_operations(status)")
...
class ConflictResolver:
    ...
    def __init__(self, default_strategy: ConflictResolution = ConflictResolution.SERVER_WINS):
...
class SyncProgressTracker:
    ...
    def start_session(self, session: SyncSession) -> None:
        ...
        self._notify_listeners('session_started', {
    ...
    def update_progress(
        ...
        if self._current_session.total_operations > 0:
            progress = (completed_operations / self._current_session.total_operations) * 100
        else:
            progress = 0.0
        ...
        self._notify_listeners('progress_updated', {
    ...
    def complete_session(self, status: SyncStatus, error_message: Optional[str] = None) -> None:
...
class SyncEngine:
    ...
    def __init__(self, sdk: MobileERPSDK, storage: OfflineStorage):
        self.sdk = sdk
        ...
        async with self._sync_lock:
            ...
            try:
                ...
                session = SyncSession(
                    ...
                    device_id=self.sdk.auth_manager.device_info.device_id if self.sdk.auth_manager.device_info else 'unknown',
                    ...
                    entity_configs=[self._entity_configs[et] for et in entity_types if et in self._entity_configs],
                )
                ...
                # Complete session
                session.status = SyncStatus.COMPLETED if session.failed_operations == 0 else SyncStatus.FAILED
    ...
    async def _sync_entity_type(
        ...
        if config.sync_direction in [SyncDirection.DOWNLOAD, SyncDirection.BIDIRECTIONAL]:
            await self._download_changes(session, config, force_full_sync)
    
    async def _upload_changes(self, session: SyncSession, config: EntitySyncConfig) -> None:
        ...
        try:
            ...
            if response['status'] != 200:
                raise NetworkError(f"Upload failed: {response.get('data', {}).get('message', 'Unknown error')}")
            ...
            for i, result in enumerate(upload_results.get('results', [])):
                ...
                if result.get('success'):
                    self.storage.update_operation_status(operation.operation_id, 'completed')
                    session.completed_operations += 1
                else:
                    error_msg = result.get('error', 'Upload failed')
                    self.storage.update_operation_status(operation.operation_id, 'failed', error_msg)
                    session.failed_operations += 1
    ...
    async def _download_changes(
        ...
        last_sync = self.storage.get_metadata(last_sync_key) if not force_full_sync else None
        ...
        try:
            ...
            if response['status'] != 200:
                raise NetworkError(f"Download failed: {response.get('data', {}).get('message', 'Unknown error')}")
            ...
            if download_data.get('sync_timestamp'):
                self.storage.set_metadata(last_sync_key, download_data['sync_timestamp'])
            
        except Exception as e:
            raise NetworkError(f"Download failed: {str(e)}")
    
    async def _apply_server_change(self, change: Dict[str, Any], config: EntitySyncConfig) -> None:
    ...
    async def _background_sync_loop(self) -> None:
        ...
        while True:
            try:
                ...
                ) if self._entity_configs else 15
...
class ConflictError(Exception):
    ...
    def __init__(self, conflict_data: Dict[str, Any]):
        ...
        super().__init__(f"Data conflict for {conflict_data['entity_type']}:{conflict_data['entity_id']}")


class DataSyncModule:
    """Main data synchronization module for SDK."""
    
    def __init__(self, sdk: MobileERPSDK, storage_path: str = "offline_storage.db"):
        self.sdk = sdk
