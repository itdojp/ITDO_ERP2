...
from .erp_app_architecture import ApplicationContext, ERPDataModel, ERPEntityType
...
class ConflictData(BaseModel):
    ...
    conflict_type: str = "field_conflict"  # field_conflict, schema_conflict, business_rule_conflict
...
class CollaborationWorkspace(BaseModel):
    ...
    permissions: Dict[str, Set[str]] = Field(default_factory=dict)  # user_id -> permissions
...
class ConflictResolver:
    ...
    async def _resolve_merge_automatic(
        ...
        merged_version = DataVersion(
            ...
            version_number=max(conflict.local_version.version_number, conflict.remote_version.version_number) + 1,
    ...
    async def _resolve_role_based(
        ...
        local_author_roles = self._get_user_roles(conflict.local_version.author_id, context)
        remote_author_roles = self._get_user_roles(conflict.remote_version.author_id, context)
        ...
        local_priority = max([role_priorities.get(role, 0) for role in local_author_roles], default=0)
        remote_priority = max([role_priorities.get(role, 0) for role in remote_author_roles], default=0)
    ...
    def _get_field_changes(self, base_data: Dict[str, Any], new_data: Dict[str, Any]) -> Dict[str, Any]:
...
class VersionedDataStore:
    ...
    def __init__(self):
        ...
        self.version_tree: Dict[str, Dict[str, List[str]]] = defaultdict(lambda: defaultdict(list))
    ...
    def store_version(self, version: DataVersion) -> None:
        ...
        if version.parent_version:
            self.version_tree[entity_key][version.parent_version].append(version.version_id)
    
    def get_version(self, entity_type: ERPEntityType, entity_id: str, version_id: str) -> Optional[DataVersion]:
    ...
    def get_latest_version(self, entity_type: ERPEntityType, entity_id: str) -> Optional[DataVersion]:
        """Get latest version of entity."""
        entity_key = f"{entity_type}:{entity_id}"
        return self.latest_versions.get(entity_key)
    
    def get_version_history(self, entity_type: ERPEntityType, entity_id: str) -> List[DataVersion]:
    ...
    def detect_conflicts(
        ...
        if (incoming_version.parent_version and 
            current_version.parent_version and
            incoming_version.parent_version != current_version.version_id and
            current_version.parent_version != incoming_version.version_id):
    ...
    def _find_common_ancestor(
        ...
        if version1.parent_version == version2.parent_version:
            parent_version = next((v for v in versions if v.version_id == version1.parent_version), None)
    ...
    def _find_conflicting_fields(
        ...
        for field in set(list(base_data.keys()) + list(data1.keys()) + list(data2.keys())):
...
class CollaborationManager:
    ...
    def _register_event_handlers(self) -> None:
        ...
        self.event_bus.register_event_handler(
            CollaborationEventType.DATA_UPDATE,
            self._handle_data_update
        )
        self.event_bus.register_event_handler(
            CollaborationEventType.DATA_CONFLICT,
            self._handle_data_conflict
        )
        self.event_bus.register_event_handler(
    ...
    async def _handle_websocket_message(self, connection: WebSocketConnection, message: str) -> None:
        ...
        try:
            ...
            if message_type == "ping":
                ...
                if workspace_id:
                    self.event_bus.subscribe_to_workspace(connection.session_id, workspace_id)
            
            elif message_type == "unsubscribe_workspace":
                workspace_id = data.get("workspace_id")
                if workspace_id:
                    self.event_bus.unsubscribe_from_workspace(connection.session_id, workspace_id)
    ...
    async def create_workspace(
        ...
        **kwargs
        ...
        await self.event_bus.publish_event(CollaborationEvent(
    ...
    async def join_workspace(self, workspace_id: str, user_id: str, session_id: str) -> bool:
    ...
    async def leave_workspace(self, workspace_id: str, user_id: str, session_id: str) -> bool:
    ...
    async def sync_data_version(
        ...
        # Check for conflicts
        conflict = self.version_store.detect_conflicts(
        ...
        if conflict:
            ...
            resolved_version = await self.conflict_resolver.resolve_conflict(conflict, context)
            ...
            if event.workspace_id:
                ...
                for participant_id in participants:
                    if participant_id != event.user_id:
                        await self.event_bus.send_direct_message(RealtimeMessage(
                ...
                if participant_id != event.user_id:
                    await self.event_bus.send_direct_message(RealtimeMessage(
                        ...
                        payload={
                            "user_id": event.user_id,
                            "workspace_id": event.workspace_id,
                            "participant_count": event.data.get("participant_count", 0)
                        }
                    ))
    ...
    def get_workspace_status(self, workspace_id: str) -> Optional[Dict[str, Any]]:
        ...
        for conflict in self.active_conflicts.values():
            if conflict.entity_type == workspace.entity_type and conflict.entity_id == workspace.entity_id:
