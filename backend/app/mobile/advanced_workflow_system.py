...
from .erp_app_architecture import ApplicationContext, ERPEntityType, WorkflowStep, ERPWorkflow
...
class DynamicApprovalRouter:
    ...
    def _get_matrix_approvers(self, thresholds: Dict[str, List[str]], amount: float) -> List[str]:
...
class WorkflowEngine:
    ...
    def __init__(self, auth_system: EnterpriseAuthenticationSystem):
        ...
        po_template = WorkflowTemplate(
            ...
            steps=[
                ...
                WorkflowStep(
                    ...
                    timeout_hours=48,
                    required_permissions=[PermissionLevel.APPROVE],
                    conditions={"amount_threshold": {"field": "total_amount", "value": 10000}}
                ),
                WorkflowStep(
    ...
    async def start_workflow(
        ...
        if (template.allowed_initiators and 
            initiator_id not in template.allowed_initiators and
            not self._user_has_allowed_role(initiator_id, template.allowed_initiators)):
            raise PermissionError(f"User {initiator_id} not allowed to initiate this workflow")
            ...
            if step_result.get("completed", False):
                ...
                if timeout_hours:
                    timeout_task = asyncio.create_task(
                        self._handle_step_timeout(instance_id, current_step.step_id, timeout_hours)
                    )
                    self.scheduled_tasks[f"{instance_id}_{current_step.step_id}_timeout"] = timeout_task
        
        except Exception as e:
            await self._fail_workflow(instance_id, f"Step execution failed: {str(e)}")
    
    async def _execute_step(self, instance: WorkflowInstance, step: WorkflowStep) -> Dict[str, Any]:
    ...
    async def _get_step_approvers(self, instance: WorkflowInstance, step: WorkflowStep) -> List[str]:
    ...
    async def approve_step(
        ...
        if action == ApprovalStatus.APPROVED:
            ...
            # Handle rejection
            rejection_count = sum(1 for action in instance.action_history 
                                if action.step_id == current_step.step_id and 
                                action.action_type == ApprovalStatus.REJECTED)
            
            # Check escalation rules
            await self._check_escalation(instance, EscalationTrigger.REJECTION, {
            ...
            if rejection_count >= 2:  # Multiple rejections
                await self._check_escalation(instance, EscalationTrigger.MULTIPLE_REJECTIONS, {
                    "rejection_count": rejection_count
                })
            else:
        ...
        # Emit approval event
        await self._emit_event("approval_action", {
    ...
    def _are_step_approvals_complete(self, instance: WorkflowInstance, step: WorkflowStep) -> bool:
        """Check if step approvals are complete."""
        template = self.templates.get(instance.template_id)
        
        # Get pending approvals for this step
        pending_approvals = [
            approval for approval in instance.active_approvals.values()
            if approval.step_id == step.step_id and approval.action_type == ApprovalStatus.PENDING
        ]
        ...
        if template.require_all_approvals:
            ...
            approved_count = sum(1 for action in instance.action_history
                               if action.step_id == step.step_id and 
                               action.action_type == ApprovalStatus.APPROVED)
            return approved_count > 0
    
    async def _handle_step_timeout(self, instance_id: str, step_id: str, timeout_hours: int) -> None:
        """Handle step timeout."""
        await asyncio.sleep(timeout_hours * 3600)  # Convert to seconds
        
        instance = self.instances.get(instance_id)
        if not instance or step_id not in [step.step_id for step in self.templates[instance.template_id].steps]:
            return
        ...
        if active_approvals:
            # Step has timed out
            await self._check_escalation(instance, EscalationTrigger.TIMEOUT, {
                "step_id": step_id,
            ...
            instance.active_approvals = {
                k: v for k, v in instance.active_approvals.items()
                if v.step_id != step_id
            }
            
            # Emit timeout event
            await self._emit_event("step_timeout", {
                "instance_id": instance_id,
                "step_id": step_id,
                "timeout_hours": timeout_hours
            })
    ...
    def _evaluate_escalation_conditions(self, conditions: Dict[str, Any], context: Dict[str, Any]) -> bool:
    ...
    async def _execute_escalation(
        ...
        await self._emit_event("workflow_escalated", {
            "instance_id": instance.instance_id,
            "rule_id": rule.rule_id,
            "escalated_to": rule.escalate_to,
            "context": context
    ...
    async def _send_escalation_notification(
        ...
        print(f"[Workflow] Escalation notification sent to {target} for rule {rule.name}")
    ...
    async def _notify_approvers(
        ...
        for approver_id in approvers:
            # Implementation would send actual notification
            print(f"[Workflow] Approval notification sent to {approver_id} for step {step.name}")
    
    async def _execute_action(self, instance: WorkflowInstance, step: WorkflowStep) -> Dict[str, Any]:
    ...
    def _evaluate_step_condition(self, instance: WorkflowInstance, step: WorkflowStep) -> bool:
    ...
    async def _send_notification(self, instance: WorkflowInstance, step: WorkflowStep) -> None:
    ...
    async def _complete_workflow(self, instance_id: str) -> None:
        ...
        await self._emit_event("workflow_completed", {
            ...
            "duration_minutes": (instance.completed_at - instance.started_at).total_seconds() / 60
        })
    ...
    async def _cancel_step_approvals(self, instance: WorkflowInstance, step_id: str) -> None:
        """Cancel pending approvals for step."""
        approvals_to_cancel = [
            approval_id for approval_id, approval in instance.active_approvals.items()
            if approval.step_id == step_id
        ]
    ...
    def _cancel_instance_tasks(self, instance_id: str) -> None:
        ...
        tasks_to_cancel = [
            task_key for task_key in self.scheduled_tasks.keys()
    ...
    def get_instance_status(self, instance_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "progress": {
                ...
                "percentage": (len(instance.completed_steps) / len(template.steps) * 100) if template and template.steps else 100
            },
            ...
            "last_action": instance.action_history[-1].dict() if instance.action_history else None
        }
    ...
    def get_user_pending_approvals(self, user_id: str) -> List[Dict[str, Any]]:
        ...
        for instance in self.instances.values():
            ...
            for approval in instance.active_approvals.values():
                if approval.approver_id == user_id and approval.action_type == ApprovalStatus.PENDING:
                    template = self.templates.get(instance.template_id)
        ...
        pending_approvals.sort(key=lambda x: (x["priority"], x["deadline"] or "9999-12-31"))
