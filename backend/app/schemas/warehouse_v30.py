...
class WarehouseBase(BaseModel):
    warehouse_code: str = Field(..., min_length=1, max_length=50, description="倉庫コード")
    warehouse_name: str = Field(..., min_length=1, max_length=200, description="倉庫名")
    warehouse_type: str = Field(default="distribution", regex="^(distribution|manufacturing|retail|cross_dock|3pl)$")

    @validator('warehouse_code')
    def validate_warehouse_code(cls, v):
        if not re.match(r'^[A-Z0-9_-]+$', v):
            raise ValueError('Warehouse code must contain only uppercase letters, numbers, underscores, and hyphens')
        return v
...
class WarehouseCreate(WarehouseBase):
    ...
    floor_load_capacity: Optional[Decimal] = Field(None, ge=0, description="床荷重容量(kg/m²)")
    ...
    # 運用時間
    operating_hours_start: Optional[str] = Field(None, regex="^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$")
    operating_hours_end: Optional[str] = Field(None, regex="^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$")
    ...
    # セキュリティ・コンプライアンス
    security_level: str = Field(default="standard", regex="^(basic|standard|high|maximum)$")
    access_control_system: bool = False
    camera_surveillance: bool = False
    fire_suppression_system: Optional[str] = Field(None, regex="^(sprinkler|gas|foam|none)$")
    security_certifications: List[str] = []
    
    # 技術・自動化
    wms_system: Optional[str] = Field(None, max_length=100)
    automation_level: str = Field(default="manual", regex="^(manual|semi_automated|automated)$")
    ...
    def validate_temperature_range(cls, v, values):
        if v is not None and 'temperature_min' in values and values['temperature_min'] is not None:
    ...
    def validate_humidity_range(cls, v, values):
        if v is not None and 'humidity_min' in values and values['humidity_min'] is not None:
    ...
    def validate_storage_area(cls, v, values):
        if v is not None and 'total_area' in values and values['total_area'] is not None:
...
class WarehouseUpdate(BaseModel):
    ...
    warehouse_type: Optional[str] = Field(None, regex="^(distribution|manufacturing|retail|cross_dock|3pl)$")
    address_line1: Optional[str] = Field(None, max_length=200)
    address_line2: Optional[str] = Field(None, max_length=200)
    city: Optional[str] = Field(None, max_length=100)
    state_province: Optional[str] = Field(None, max_length=100)
    ...
    automation_level: Optional[str] = Field(None, regex="^(manual|semi_automated|automated)$")
...
class WarehouseZoneBase(BaseModel):
    ...
    zone_type: str = Field(default="storage", regex="^(storage|receiving|shipping|staging|quality_control)$")
...
class WarehouseZoneUpdate(BaseModel):
    ...
    zone_type: Optional[str] = Field(None, regex="^(storage|receiving|shipping|staging|quality_control)$")
...
class WarehouseLocationCreate(WarehouseLocationBase):
    ...
    # アクセス・装備
    access_method: str = Field(default="manual", regex="^(manual|forklift|crane|automated)$")
...
class WarehouseLocationUpdate(BaseModel):
    ...
    access_method: Optional[str] = Field(None, regex="^(manual|forklift|crane|automated)$")
    pick_priority: Optional[int] = Field(None, ge=1, le=100)
    status: Optional[str] = Field(None, regex="^(available|occupied|blocked|damaged|maintenance)$")
...
class InventoryMovementBase(BaseModel):
    movement_type: str = Field(..., regex="^(receipt|shipment|adjustment|transfer|cycle_count)$")
...
class InventoryMovementCreate(InventoryMovementBase):
    ...
    # 品質・検査
    quality_status: str = Field(default="approved", regex="^(approved|rejected|quarantine|pending)$")
...
class InventoryMovementUpdate(BaseModel):
    ...
    expiration_date: Optional[datetime] = None
    actual_date: Optional[datetime] = None
    quality_status: Optional[str] = Field(None, regex="^(approved|rejected|quarantine|pending)$")
    inspection_required: Optional[bool] = None
    reason_code: Optional[str] = Field(None, max_length=50)
    reason_description: Optional[str] = None
    status: Optional[str] = Field(None, regex="^(pending|in_progress|completed|cancelled|error)$")
    notes: Optional[str] = None
...
class CycleCountCreate(CycleCountBase):
    warehouse_id: str
    count_scope: str = Field(default="zone", regex="^(zone|location|product|category|all)$")
    zone_ids: List[str] = []
...
class CycleCountUpdate(BaseModel):
    ...
    status: Optional[str] = Field(None, regex="^(planned|in_progress|completed|cancelled|on_hold)$")
    instructions: Optional[str] = None
    notes: Optional[str] = None
...
class WarehouseBulkOperationRequest(BaseModel):
    ...
    operation: str = Field(..., regex="^(activate|deactivate|update_status|assign_manager)$")
