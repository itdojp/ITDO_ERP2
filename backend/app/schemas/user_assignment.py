...
class UserAssignmentCreate(BaseModel):
    ...
    role_assignments: Optional[List[str]] = Field(
        None, description="List of role names to assign"
    )
    effective_date: Optional[datetime] = Field(
        None, description="Effective date of assignment"
    )
    assignment_reason: Optional[str] = Field(
        None, max_length=500, description="Reason for assignment"
    )
...
class UserAssignmentUpdate(BaseModel):
    ...
    role_assignments: Optional[List[str]] = Field(
        None, description="Updated role assignments"
    )
    effective_date: Optional[datetime] = Field(
        None, description="Updated effective date"
    )
    assignment_reason: Optional[str] = Field(
        None, max_length=500, description="Updated assignment reason"
    )
    is_primary: Optional[bool] = Field(
        None, description="Updated primary assignment status"
    )
...
class DepartmentUsersResponse(BaseModel):
    ...
    inactive_users: int
    users: List[UserSummary]
    sub_departments: Optional[List[dict]] = Field(
        None, description="Sub-department breakdown"
    )
...
class OrganizationAssignmentStats(BaseModel):
    ...
    department_stats: Optional[List[dict]] = Field(
        None, description="Per-department statistics"
    )
    ...
    # Trend data
    monthly_changes: dict = Field(
        default_factory=dict, description="Monthly assignment changes"
    )
...
class DepartmentAssignmentStats(BaseModel):
    ...
    headcount_utilization: Optional[float] = Field(
        None, description="Utilization percentage"
    )

    # Sub-department breakdown
    sub_department_stats: Optional[List[dict]] = Field(
        None, description="Sub-department statistics"
    )
...
class AssignmentValidationIssue(BaseModel):
    ...
    issue_type: (
        str  # 'orphaned_user', 'invalid_department', 'circular_assignment', etc.
    )
...
class AssignmentValidationResult(BaseModel):
    ...
    # Summary by type
    issue_summary: dict = Field(
        default_factory=dict, description="Issues grouped by type"
    )
...
class TransferRequest(BaseModel):
    ...
    from_organization_id: Optional[int] = Field(
        None, description="Current organization (for validation)"
    )
    from_department_id: Optional[int] = Field(
        None, description="Current department (for validation)"
    )
    ...
    effective_date: Optional[datetime] = Field(
        None, description="Transfer effective date"
    )
...
class TransferResponse(BaseModel):
    ...
    user_id: int
    user_name: str
    from_organization: Optional[dict] = Field(
        None, description="Previous organization info"
    )
    from_department: Optional[dict] = Field(
        None, description="Previous department info"
    )
...
class AssignmentSearchFilters(BaseModel):
    ...
    organization_ids: Optional[List[int]] = Field(
        None, description="Filter by organizations"
    )
    department_ids: Optional[List[int]] = Field(
        None, description="Filter by departments"
    )
    ...
    assignment_types: Optional[List[str]] = Field(
        None, description="Filter by assignment types"
    )
