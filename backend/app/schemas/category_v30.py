...
class CategoryBase(BaseModel):
    category_code: str = Field(..., min_length=1, max_length=50, description="カテゴリコード")
    category_name: str = Field(..., min_length=1, max_length=200, description="カテゴリ名")
    category_name_en: Optional[str] = Field(None, max_length=200, description="英語カテゴリ名")
    description: Optional[str] = Field(None, description="説明")

    @validator('category_code')
    def validate_category_code(cls, v):
        if not re.match(r'^[A-Z0-9_-]+$', v):
            raise ValueError('Category code must contain only uppercase letters, numbers, underscores, and hyphens')
        return v
...
class CategoryCreate(CategoryBase):
    ...
    # 分類設定
    category_type: str = Field(default="product", regex="^(product|service|expense|asset|customer|supplier)$")
    ...
    # 税務・会計設定
    tax_category: Optional[str] = Field(None, regex="^(standard|reduced|exempt|zero_rated)$")
    ...
    standard_cost_method: Optional[str] = Field(None, regex="^(standard|average|fifo|lifo)$")
    ...
    seasonality_pattern: Optional[str] = Field(None, regex="^(none|seasonal|promotional)$")
    demand_pattern: Optional[str] = Field(None, regex="^(stable|volatile|declining|growing)$")
    lifecycle_stage: Optional[str] = Field(None, regex="^(introduction|growth|maturity|decline)$")
    ...
    def validate_url_slug(cls, v):
        if v and not re.match(r'^[a-z0-9-]+$', v):
            raise ValueError('URL slug must contain only lowercase letters, numbers, and hyphens')
        return v
...
class CategoryUpdate(BaseModel):
    ...
    # 分類更新
    category_type: Optional[str] = Field(None, regex="^(product|service|expense|asset|customer|supplier)$")
    industry_vertical: Optional[str] = Field(None, max_length=100)
    business_unit: Optional[str] = Field(None, max_length=100)
...
class CategoryAttributeBase(BaseModel):
    ...
    def validate_attribute_code(cls, v):
        if not re.match(r'^[A-Za-z0-9_]+$', v):
            raise ValueError('Attribute code must contain only letters, numbers, and underscores')
        return v
...
class CategoryAttributeCreate(CategoryAttributeBase):
    ...
    # データ型・制約
    data_type: str = Field(default="text", regex="^(text|number|date|boolean|select|multi_select)$")
    ...
    unit_type: Optional[str] = Field(None, regex="^(length|weight|volume|currency|percentage|count|time)$")
...
class CategoryPricingRuleCreate(CategoryPricingRuleBase):
    ...
    # ルール設定
    rule_type: str = Field(default="markup", regex="^(markup|discount|fixed_price|cost_plus|competitive)$")
...
class CategoryPricingRuleUpdate(BaseModel):
    ...
    description: Optional[str] = None
    rule_type: Optional[str] = Field(None, regex="^(markup|discount|fixed_price|cost_plus|competitive)$")
...
class CategoryBulkOperationRequest(BaseModel):
    ...
    operation: str = Field(..., regex="^(activate|deactivate|delete|move|update_attributes)$")
