...
class SupplierRelationshipBase(BaseModel):
    supplier_id: str
    relationship_type: str = Field(default="vendor", regex="^(vendor|partner|strategic|preferred)$")
    partnership_level: str = Field(default="standard", regex="^(strategic|preferred|standard|conditional)$")

class SupplierRelationshipCreate(SupplierRelationshipBase):
    contract_type: Optional[str] = Field(None, regex="^(master_agreement|project_based|spot_buy|framework)$")
    ...
    business_continuity_risk: str = Field(default="low", regex="^(critical|high|medium|low)$")
    ...
    # 戦略的重要度
    strategic_importance: str = Field(default="medium", regex="^(critical|high|medium|low)$")
...
class SupplierRelationshipUpdate(BaseModel):
    relationship_type: Optional[str] = Field(None, regex="^(vendor|partner|strategic|preferred)$")
    partnership_level: Optional[str] = Field(None, regex="^(strategic|preferred|standard|conditional)$")
    contract_type: Optional[str] = Field(None, regex="^(master_agreement|project_based|spot_buy|framework)$")
    contract_end_date: Optional[date] = None
    ...
    strategic_importance: Optional[str] = Field(None, regex="^(critical|high|medium|low)$")
    status: Optional[str] = Field(None, regex="^(active|under_review|suspended|terminated)$")
    next_review_date: Optional[date] = None
    notes: Optional[str] = None
...
class SupplierPerformanceReviewBase(BaseModel):
    ...
    review_type: str = Field(default="quarterly", regex="^(monthly|quarterly|semi_annual|annual)$")
...
class SupplierPerformanceReviewUpdate(BaseModel):
    review_status: Optional[str] = Field(None, regex="^(draft|in_review|completed|approved)$")
    quality_rating: Optional[Decimal] = Field(None, ge=1, le=5)
    delivery_rating: Optional[Decimal] = Field(None, ge=1, le=5)
    service_rating: Optional[Decimal] = Field(None, ge=1, le=5)
    cost_rating: Optional[Decimal] = Field(None, ge=1, le=5)
...
class SupplierNegotiationBase(BaseModel):
    ...
    negotiation_type: str = Field(default="contract_renewal", regex="^(contract_renewal|price_adjustment|terms_revision|new_contract)$")
...
class SupplierNegotiationUpdate(BaseModel):
    status: Optional[str] = Field(None, regex="^(preparation|active|on_hold|completed|cancelled)$")
    current_phase: Optional[str] = Field(None, regex="^(planning|proposal|negotiation|agreement|finalization)$")
    target_completion_date: Optional[date] = None
    ...
    relationship_impact: Optional[str] = Field(None, regex="^(positive|neutral|negative)$")
...
class SupplierContractBase(BaseModel):
    ...
    contract_type: str = Field(default="purchase_agreement", regex="^(purchase_agreement|service_agreement|nda|framework)$")
    effective_date: date
...
class SupplierContractCreate(SupplierContractBase):
    ...
    currency: str = "JPY"
    pricing_model: Optional[str] = Field(None, regex="^(fixed|variable|cost_plus|market_based)$")
    ...
    # メタデータ
    confidentiality_level: str = Field(default="internal", regex="^(public|internal|confidential|restricted)$")
    tags: List[str] = []
    notes: Optional[str] = None
...
class SupplierContractUpdate(BaseModel):
    ...
    delivery_terms: Optional[str] = None
    lead_time_days: Optional[int] = Field(None, ge=0)
    status: Optional[str] = Field(None, regex="^(draft|under_review|approved|active|expired|terminated)$")
