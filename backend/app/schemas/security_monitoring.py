...
class SecurityDashboardResponse(BaseModel):
    ...
    organization_id: Optional[int] = Field(
        None, description="Organization filter applied"
    )

    threats: Dict[str, Any] = Field(
        ..., description="Threat statistics and recent threats"
    )
    metrics: Dict[str, Any] = Field(..., description="Security metrics")
    monitoring_status: Dict[str, bool] = Field(
        ..., description="Monitoring feature status"
    )
...
class SecurityReportResponse(BaseModel):
    ...
    top_activities: Dict[str, Dict[str, int]] = Field(
        ..., description="Top activities breakdown"
    )
    security_insights: Dict[str, int] = Field(
        ..., description="Security-specific insights"
    )
...
class ThreatListResponse(BaseModel):
    ...
    threats: List[SecurityThreatResponse] = Field(
...
class FailedLoginMonitoringResponse(BaseModel):
    ...
    failed_login_threats: List[SecurityThreatResponse] = Field(
        ..., description="Failed login threats"
    )
...
class BulkAccessMonitoringResponse(BaseModel):
    ...
    bulk_access_threats: List[SecurityThreatResponse] = Field(
        ..., description="Bulk access threats"
    )
    monitoring_config: Dict[str, Any] = Field(
        ..., description="Monitoring configuration"
    )
...
class PrivilegeEscalationMonitoringResponse(BaseModel):
    ...
    privilege_escalation_threats: List[SecurityThreatResponse] = Field(
        ..., description="Privilege escalation threats"
    )
    monitoring_enabled: bool = Field(
        ..., description="Whether privilege escalation monitoring is enabled"
    )
...
class FailedLoginAlertConfig(AlertConfigBase):
    ...
    time_window_minutes: int = Field(
        15, description="Time window for counting failed attempts"
    )
...
class BulkAccessAlertConfig(AlertConfigBase):
    ...
    threshold: int = Field(
        100, description="Number of access attempts to trigger alert"
    )
    time_window_minutes: int = Field(
        5, description="Time window for counting access attempts"
    )
...
class SecurityConfigResponse(BaseModel):
    ...
    privilege_escalation_monitoring: bool = Field(
        True, description="Privilege escalation monitoring enabled"
    )
    unusual_pattern_monitoring: bool = Field(
        True, description="Unusual pattern monitoring enabled"
    )
