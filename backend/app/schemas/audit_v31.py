...
class AuditLogEntryCreateRequest(BaseAuditSchema):
    ...
    event_category: str = Field(..., min_length=1, max_length=100, description="Event category")
    ...
    impersonated_user_id: Optional[str] = Field(None, description="Impersonated user ID")
    ...
    # Event details
    action_performed: Optional[str] = Field(None, max_length=200, description="Action performed")
    ...
    change_details: Dict[str, Any] = Field(default_factory=dict, description="Change details")
    ...
    correlation_id: Optional[str] = Field(None, max_length=200, description="Correlation ID")
...
class AuditLogSearchRequest(BaseAuditSchema):
    ...
    max_risk_score: Optional[int] = Field(None, ge=0, le=100, description="Maximum risk score")
...
class AuditLogBulkCreateRequest(BaseAuditSchema):
    ...
    entries: List[AuditLogEntryCreateRequest] = Field(
        ...
        min_items=1,
        max_items=1000,
        description="List of audit entries to create"
    )
...
class AuditRuleCreateRequest(BaseAuditSchema):
    ...
    alert_severity: AuditSeverity = Field(AuditSeverity.MEDIUM, description="Alert severity")
    ...
    def validate_evaluation_frequency(cls, v):
        ...
        if v not in valid_frequencies:
            raise ValueError(f'Evaluation frequency must be one of: {valid_frequencies}')
        return v
...
class AuditRuleTestRequest(BaseAuditSchema):
    ...
    test_data: Dict[str, Any] = Field(..., description="Test data to evaluate rule against")
...
class AuditAlertCreateRequest(BaseAuditSchema):
    ...
    # Triggering events
    triggering_event_ids: List[str] = Field(default_factory=list, description="Triggering event IDs")
...
class AuditAlertResponse(BaseAuditSchema):
    ...
    regulatory_notification_required: bool = Field(..., description="Regulatory notification required")
...
class AuditReportCreateRequest(BaseAuditSchema):
    ...
    organization_id: str = Field(..., description="Organization ID")
    report_name: str = Field(..., min_length=1, max_length=300, description="Report name")
    ...
    report_filters: Dict[str, Any] = Field(default_factory=dict, description="Report filters")
    report_config: Dict[str, Any] = Field(default_factory=dict, description="Report configuration")
    
    # Compliance framework
    compliance_framework: Optional[ComplianceFramework] = Field(None, description="Compliance framework")
    regulatory_requirements: List[str] = Field(default_factory=list, description="Regulatory requirements")
...
class AuditReportResponse(BaseAuditSchema):
    ...
    distributed_at: Optional[datetime] = Field(None, description="Distribution timestamp")
...
class AuditSessionCreateRequest(BaseAuditSchema):
    ...
    authentication_factors: List[str] = Field(default_factory=list, description="Authentication factors")
    
    # Session details
    ip_address: Optional[str] = Field(None, max_length=45, description="IP address")
    user_agent: Optional[str] = Field(None, max_length=1000, description="User agent")
    device_fingerprint: Optional[str] = Field(None, max_length=500, description="Device fingerprint")
...
class AuditSessionResponse(BaseAuditSchema):
    ...
    inactivity_duration: Optional[int] = Field(None, description="Inactivity duration in seconds")
...
class ComplianceAssessmentCreateRequest(BaseAuditSchema):
    ...
    framework_version: Optional[str] = Field(None, max_length=50, description="Framework version")
    requirement_id: str = Field(..., max_length=100, description="Requirement ID")
    requirement_name: str = Field(..., min_length=1, max_length=300, description="Requirement name")
    requirement_description: Optional[str] = Field(None, description="Requirement description")
    ...
    compliance_score: Optional[Decimal] = Field(None, ge=0, le=100, description="Compliance score")
    risk_level: str = Field("medium", description="Risk level")
    
    # Evidence and documentation
    evidence_items: List[Dict[str, Any]] = Field(default_factory=list, description="Evidence items")
...
class ComplianceAssessmentResponse(BaseAuditSchema):
    ...
    remediation_actions: List[Dict[str, Any]] = Field(..., description="Remediation actions")
    ...
    remediation_plan: Optional[Dict[str, Any]] = Field(None, description="Remediation plan")
    ...
    remediation_owner: Optional[str] = Field(None, description="Remediation owner user ID")
    ...
    assessment_frequency: Optional[str] = Field(None, description="Assessment frequency")
...
class ComplianceDashboardResponse(BaseAuditSchema):
    ...
    framework_statistics: Dict[str, Dict[str, Any]] = Field(..., description="Framework statistics")
...
class AuditMetricsResponse(BaseAuditSchema):
    ...
    false_positive_rate: Optional[Decimal] = Field(None, description="False positive rate %")
    average_resolution_time: Optional[Decimal] = Field(None, description="Average resolution time minutes")
    
    # Performance metrics
    processing_latency_avg: Optional[Decimal] = Field(None, description="Average processing latency ms")
    processing_latency_max: Optional[Decimal] = Field(None, description="Max processing latency ms")
    ...
    storage_growth_rate: Optional[Decimal] = Field(None, description="Storage growth rate MB/day")
    ...
    # Compliance metrics
    compliance_score: Optional[Decimal] = Field(None, description="Compliance score %")
    assessment_completion_rate: Optional[Decimal] = Field(None, description="Assessment completion rate %")
    remediation_completion_rate: Optional[Decimal] = Field(None, description="Remediation completion rate %")
    
    # System health
    system_uptime: Optional[Decimal] = Field(None, description="System uptime %")
    queue_backlog: int = Field(..., description="Queue backlog")
    resource_utilization: Dict[str, Any] = Field(..., description="Resource utilization")
