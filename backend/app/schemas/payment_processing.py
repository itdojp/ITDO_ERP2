...
class PaymentMethodInfoRequest(PaymentProcessingBase):
    ...
    card_number: Optional[str] = Field(
        None, description="Card number (testing only)", min_length=13, max_length=19
    )
    card_exp_month: Optional[int] = Field(
        None, description="Card expiration month", ge=1, le=12
    )
    card_exp_year: Optional[int] = Field(
        None, description="Card expiration year", ge=2024, le=2034
    )
    card_cvv: Optional[str] = Field(
        None, description="Card CVV", min_length=3, max_length=4
    )
    ...
    def validate_card_number(cls, v) -> dict:
...
class PaymentCreateRequest(PaymentProcessingBase):
    ...
    payment_method: PaymentMethod = Field(
        PaymentMethod.CREDIT_CARD, description="Payment method"
    )
    payment_provider: PaymentProvider = Field(
        PaymentProvider.STRIPE, description="Payment provider"
    )
    ...
    # Payment method info
    payment_method_info: Optional[PaymentMethodInfoRequest] = Field(
        None, description="Payment method details"
    )

    # Customer info for fraud detection
    customer_info: Optional[CustomerInfoRequest] = Field(
    ...
    @validator("currency")
    def validate_currency(cls, v) -> dict:
...
class PaymentRefundRequest(PaymentProcessingBase):
    ...
    amount: Optional[Decimal] = Field(
        None, description="Refund amount (null for full refund)", gt=0
    )
    reason: RefundReason = Field(
        RefundReason.CUSTOMER_REQUEST, description="Refund reason"
    )
    reason_details: Optional[str] = Field(
        None, description="Detailed reason for refund"
    )
...
class PaymentSearchRequest(PaymentProcessingBase):
    ...
    def validate_amount_range(cls, v, values) -> dict:
...
class PaymentMethodInfoResponse(PaymentProcessingBase):
    ...
    bank_account_last_four: Optional[str] = Field(
        None, description="Last 4 digits of account"
    )
...
class PaymentRefundResponse(PaymentProcessingBase):
    ...
    external_refund_id: Optional[str] = Field(
        None, description="External provider refund ID"
    )
    ...
    processed_at: Optional[datetime] = Field(
        None, description="When refund was processed"
    )
...
class PaymentTransactionResponse(PaymentProcessingBase):
    ...
    external_transaction_id: Optional[str] = Field(
        None, description="External provider transaction ID"
    )
    ...
    is_fraud_flagged: bool = Field(
        False, description="Whether flagged as potential fraud"
    )
    ...
    authorized_at: Optional[datetime] = Field(
        None, description="When payment was authorized"
    )
    captured_at: Optional[datetime] = Field(
        None, description="When payment was captured"
    )
    completed_at: Optional[datetime] = Field(
        None, description="When payment was completed"
    )
...
class PaymentResponse(PaymentProcessingBase):
    ...
    requires_action: bool = Field(
        False, description="Whether additional action is required"
    )
...
class PaymentSearchResponse(PaymentProcessingBase):
    ...
    def calculate_has_more(cls, v, values) -> dict:
...
class PaymentAnalyticsResponse(PaymentProcessingBase):
    ...
    breakdown: Dict[str, Dict[str, PaymentAnalyticsBreakdown]] = Field(
        ..., description="Detailed breakdowns"
    )

    # Trend data
    daily_trends: Optional[List[Dict[str, Any]]] = Field(
        None, description="Daily trend data"
    )
    hourly_patterns: Optional[List[Dict[str, Any]]] = Field(
        None, description="Hourly patterns"
    )

    # Top performers
    top_customers: Optional[List[Dict[str, Any]]] = Field(
        None, description="Top customers by volume"
    )
    top_payment_methods: Optional[List[Dict[str, Any]]] = Field(
        None, description="Most used payment methods"
    )
...
class PaymentProviderConfigResponse(PaymentProcessingBase):
    ...
    min_transaction_amount: Optional[Decimal] = Field(
