...
class WorkflowNodeBase(BaseModel):
    ...
    node_id: str = Field(..., max_length=100, description="Unique node ID within workflow")
    ...
    assign_to_role: Optional[str] = Field(None, max_length=100, description="Role-based assignment")
    assign_to_user: Optional[int] = Field(None, description="User-based assignment")
    auto_assign_rules: Optional[Dict[str, Any]] = Field(None, description="Auto-assignment rules")
    entry_conditions: Optional[Dict[str, Any]] = Field(None, description="Entry conditions")
    exit_conditions: Optional[Dict[str, Any]] = Field(None, description="Exit conditions")
    estimated_duration_hours: Optional[int] = Field(None, ge=0, description="Estimated duration in hours")
    timeout_hours: Optional[int] = Field(None, ge=0, description="Node timeout in hours")
...
class WorkflowBase(BaseModel):
    ...
    workflow_definition: Dict[str, Any] = Field(..., description="Workflow definition (nodes, connections)")
    allow_parallel_instances: bool = Field(True, description="Allow multiple instances")
    timeout_hours: Optional[int] = Field(None, ge=0, description="Workflow timeout in hours")
    tags: Optional[str] = Field(None, max_length=500, description="Workflow tags (comma-separated)")
...
class WorkflowInstanceBase(BaseModel):
    ...
    status: WorkflowInstanceStatus = Field(WorkflowInstanceStatus.PENDING, description="Instance status")
    context_data: Optional[Dict[str, Any]] = Field(None, description="Instance context data")
    input_data: Optional[Dict[str, Any]] = Field(None, description="Instance input data")
    output_data: Optional[Dict[str, Any]] = Field(None, description="Instance output data")
    priority: int = Field(5, ge=1, le=10, description="Instance priority (1-10)")
    reference_type: Optional[str] = Field(None, max_length=100, description="Reference object type")
    reference_id: Optional[str] = Field(None, max_length=100, description="Reference object ID")
...
class WorkflowCreate(WorkflowBase):
    ...
    nodes: List[WorkflowNodeBase] = Field(default_factory=list, description="Workflow nodes")
    connections: List[WorkflowConnectionBase] = Field(default_factory=list, description="Workflow connections")
...
class WorkflowInstanceUpdate(BaseModel):
    ...
    instance_name: Optional[str] = Field(None, max_length=200, description="Instance name")
    status: Optional[WorkflowInstanceStatus] = Field(None, description="Instance status")
    context_data: Optional[Dict[str, Any]] = Field(None, description="Instance context data")
    output_data: Optional[Dict[str, Any]] = Field(None, description="Instance output data")
...
class WorkflowInstanceListResponse(BaseModel):
    ...
    items: List[WorkflowInstanceResponse] = Field(..., description="List of workflow instances")
...
class WorkflowAnalyticsResponse(BaseModel):
    ...
    bottleneck_nodes: List[str] = Field(default_factory=list, description="Nodes with longest processing times")
...
class WorkflowPerformanceMetrics(BaseModel):
    ...
    most_used_workflows: List[Dict[str, Any]] = Field(default_factory=list, description="Most frequently used workflows")