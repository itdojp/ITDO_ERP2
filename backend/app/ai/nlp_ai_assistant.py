...
class NLPProcessor:
    ...
    async def _extract_intent(self, text: str) -> Tuple[IntentType, float]:
        ...
        if not intent_scores:
            ...
            if re.search(
                r"\b(what|how|when|where|why|which|who)\b", text
            ) or text.endswith("?"):
...
class ConversationManager:
        ...
        self.conversation_templates = {
            "data_query": {
                "states": [
                    ...
                    {
                        "state": "confirming",
                        "message": "Let me show you {metric} for {period}. Is this correct?",
                    },
            ...
            "report_generation": {
                "states": [
                    ...
                    {
                        "state": "gathering_info",
                        "message": "What parameters should I include?",
                    },
                    {
                        "state": "confirming",
                        "message": "I'll generate a {report_type} report with {parameters}. Proceed?",
                    },
    ...
    async def update_context(
        self, context: ConversationContext, intent: UserIntent, response: AIResponse
    ) -> None:
    ...
    async def _update_conversation_state(
        self, context: ConversationContext, intent: UserIntent
    ) -> None:
        ...
        if intent.intent_type == IntentType.DATA_QUERY:
            if context.state == ConversationState.INITIAL:
                if (
                    "metrics" in intent.query_parameters
                    and "time_period" in intent.query_parameters
                ):
                    ...
                    if "metrics" not in intent.query_parameters:
                        context.pending_clarifications.append(
                            "What specific metrics do you want to see?"
                        )
...
class ResponseGenerator:
    ...
    def __init__(self, knowledge_base: Dict[str, KnowledgeBase]) -> dict:
    ...
    async def generate_response(
        self, intent: UserIntent, context: ConversationContext
    ) -> AIResponse:
    ...
    async def _generate_goodbye_response(
        ...
        return {"type": ResponseType.TEXT, "text": text, "confidence": 0.9}
    ...
    async def _generate_default_response(
        ...
        return {
            ...
            + intent.intent_type.value.replace("_", " ")
            + ". How can I help you with that?",
            "quick_replies": ["Show me data", "Generate report", "Get help"],
            "expects_response": True,
            "confidence": 0.5,
        }
    ...
    async def _search_knowledge_base(self, query: str) -> Optional[Dict[str, Any]]:
        ...
        for entry in self.knowledge_base.values():
            ...
            if score > best_score and score > 0.3:  # Minimum relevance threshold
                best_score = score
                best_match = {"content": entry.content, "confidence": min(0.9, score)}
...
class ConversationalAIAssistant:
    ...
    def __init__(self, sdk: MobileERPSDK, ai_engine: AIIntelligenceEngine) -> dict:
        self.sdk = sdk
    ...
    async def _update_conversation_metrics(self) -> None:
        ...
        if recent_logs:
            ...
            high_confidence_intents = [
                log for log in recent_logs if log["intent_confidence"] > 0.8
            ]
    ...
    async def get_conversation_history(
        ...
        session_logs = [
            log for log in self.conversation_logs if log["session_id"] == session_id
        ]
    ...
    async def get_user_insights(self, user_id: str) -> Dict[str, Any]:
        ...
        user_logs = [log for log in self.conversation_logs if log["user_id"] == user_id]
        ...
        avg_confidence = (
            sum(log["intent_confidence"] for log in user_logs) / total_messages
        )
            ...
            "recent_activity": {
                ...
                "new_conversations_today": len(
                    [
                        log
                        for log in self.conversation_logs
                        if datetime.fromisoformat(log["timestamp"]).date()
                        == datetime.now().date()
                    ]
                ),
            },
    ...
    def _get_top_intents(self) -> List[Dict[str, Any]]:
        ...
        top_intents = sorted(intent_counts.items(), key=lambda x: x[1], reverse=True)[
            :5
        ]
        ...
        return [
            {
                "intent": intent,
                "count": count,
                "percentage": count / len(self.conversation_logs) * 100,
            }
