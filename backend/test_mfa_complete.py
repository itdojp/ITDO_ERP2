...
class SimpleMFAService:
    ...
    def disable_mfa(self, user: User) -> None:
        ...
        # Deactivate devices
        self.db.query(MFADevice).filter(MFADevice.user_id == user.id).update(
            {"is_active": False}
        )

        # Invalidate backup codes
        self.db.query(MFABackupCode).filter(MFABackupCode.user_id == user.id).update(
