...
class SimpleGoogleAuthService:
    ...
    def authenticate_or_create_user(self, google_info: dict, ip_address: str, 
                                  user_agent: str) -> tuple[User, bool]:
        ...
        if user:
            ...
            user.record_successful_login(self.db)
            user.log_activity(self.db, "google_account_linked", {"google_id": google_info["id"]})
            self.db.commit()
            return user, False
    ...
    def link_google_account(self, user: User, google_info: dict) -> None:
        ...
        if existing:
            raise Exception("このGoogleアカウントは既に別のユーザーにリンクされています")
    ...
    def unlink_google_account(self, user: User) -> None:
        ...
        if not user.has_password_set():
            raise Exception("パスワードが設定されていません。Googleアカウントのリンクを解除する前にパスワードを設定してください")
