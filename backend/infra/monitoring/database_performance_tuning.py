...
class QueryAnalyzer:
    ...
    def analyze_query(self, query: str, execution_metrics: Dict[str, Any]) -> QueryMetrics:
    ...
    def _determine_complexity(self, query: str, execution_time: float, rows_examined: int) -> QueryComplexity:
    ...
    def recommend_indexes(self, query_metrics: List[QueryMetrics]) -> List[IndexRecommendation]:
        ...
        for table, patterns in table_access_patterns.items():
            ...
            if len(columns) > 0:
                ...
                impact_score = (total_execution_time * query_frequency) / len(query_metrics)
    ...
    def _extract_tables_from_query(self, query: str) -> List[str]:
        ...
        if 'from ' in query_lower:
            ...
            if parts:
                ...
                if table_name and not table_name in ['select', 'where', 'order', 'group']:
...
class DatabasePerformanceTuningSystem:
        ...
        while True:
            try:
                ...
                for query_data in recent_queries:
                    ...
                    if metrics.complexity in [QueryComplexity.COMPLEX, QueryComplexity.CRITICAL]:
        ...
        while True:
            try:
                ...
                if recent_metrics:
                    ...
                    recommendations = self.index_optimizer.recommend_indexes(recent_metrics)
                    self.metrics['indexes_recommended'] = len(recommendations)
                    
                    # Log top recommendations
                    for rec in recommendations[:3]:
                        logger.info(f"Index recommendation: {rec.table_name}.{rec.columns} "
    ...
    async def _queue_optimization_task(self, metrics: QueryMetrics):
        ...
        task = {
            ...
            'priority': 'high' if metrics.complexity == QueryComplexity.CRITICAL else 'medium',
    ...
    async def _execute_optimization_task(self, task: Dict[str, Any]):
        ...
        try:
            if task['type'] == 'query_optimization':
                ...
                logger.info(f"Optimized query {metrics.query_id}: "
    ...
    def get_performance_report(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'recent_health': self.health_monitor.health_history[-10:] if self.health_monitor.health_history else [],
