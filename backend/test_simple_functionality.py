...
def list_products(
    category_id: Optional[str] = None, search: Optional[str] = None, limit: int = 50
) -> List[Dict[str, Any]]:
...
def get_product_stats() -> Dict[str, Any]:
    ...
    avg_price = sum(p.get("price", 0) for p in products_db.values()) / max(
        total_products, 1
    )
    ...
    return {
        ...
        "low_stock_products": sum(
            1 for p in products_db.values() if p.get("stock_quantity", 0) < 10
        ),
    }
