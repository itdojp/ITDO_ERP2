...
interface PayrollRecord {
  ...
  pay_date: string;
  status: 'draft' | 'calculated' | 'approved' | 'processed' | 'paid' | 'cancelled';
  base_salary: number;
...
export const PayrollManagementPage: React.FC = () => {
  ...
  const mockPayrollRecords: PayrollRecord[] = [
    {
      ...
      federal_tax: 1360.00,
      state_tax: 340.00,
      social_security: 421.60,
      medicare: 98.60,
      health_insurance: 250.00,
      retirement_contribution: 340.00,
      other_deductions: 50.00,
      total_deductions: 2860.20,
      net_pay: 3939.80,
      currency: 'USD',
    ...
    },
    {
      ...
      federal_tax: 1180.00,
      state_tax: 295.00,
      social_security: 365.80,
      medicare: 85.55,
      health_insurance: 300.00,
      retirement_contribution: 295.00,
    ...
    },
    {
      ...
      medicare: 59.89,
      health_insurance: 275.00,
      retirement_contribution: 206.50,
      other_deductions: 0,
    ...
    },
    {
      ...
      medicare: 55.46,
      health_insurance: 225.00,
      retirement_contribution: 191.25,
      other_deductions: 15.00,
    ...
    },
    {
      ...
      retirement_contribution: 215.10,
      other_deductions: 30.00,
      total_deductions: 1849.70,
      net_pay: 2452.30,
      currency: 'USD',
      payment_method: 'direct_deposit',
      bank_account: '****3456',
  ...
  const { data: payrollData, isLoading, error, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      if (filters.department) {
        filteredRecords = filteredRecords.filter(record => record.department === filters.department);
      }
      
      // Calculate summary
      const summary: PayrollSummary = {
        total_records: mockPayrollRecords.length,
        total_employees: new Set(mockPayrollRecords.map(r => r.employee_id)).size,
        total_gross_pay: mockPayrollRecords.reduce((sum, r) => sum + r.gross_pay, 0),
        total_net_pay: mockPayrollRecords.reduce((sum, r) => sum + r.net_pay, 0),
        total_taxes: mockPayrollRecords.reduce((sum, r) => sum + r.federal_tax + r.state_tax + r.social_security + r.medicare, 0),
        total_deductions: mockPayrollRecords.reduce((sum, r) => sum + r.total_deductions, 0),
        processed_count: mockPayrollRecords.filter(r => r.status === 'processed' || r.status === 'paid').length,
        pending_count: mockPayrollRecords.filter(r => r.status === 'draft' || r.status === 'calculated').length,
        avg_net_pay: mockPayrollRecords.reduce((sum, r) => sum + r.net_pay, 0) / mockPayrollRecords.length
      };
    ...
    queryFn: async (): Promise<PayrollAnalytics> => {
      ...
      return {
        department_breakdown: [
          { department: 'Engineering', employee_count: 15, total_gross: 102000, total_net: 71400, avg_salary: 6800 },
          { department: 'Sales', employee_count: 12, total_gross: 86400, total_net: 60480, avg_salary: 7200 },
          { department: 'Marketing', employee_count: 8, total_gross: 47200, total_net: 33040, avg_salary: 5900 },
          { department: 'Finance', employee_count: 6, total_gross: 24780, total_net: 17346, avg_salary: 4130 },
          { department: 'HR', employee_count: 4, total_gross: 15300, total_net: 10710, avg_salary: 3825 }
        ],
        monthly_trends: [
          { month: 'Oct', total_payroll: 245000, employee_count: 42, avg_per_employee: 5833 },
          { month: 'Nov', total_payroll: 258000, employee_count: 43, avg_per_employee: 6000 },
          { month: 'Dec', total_payroll: 275000, employee_count: 44, avg_per_employee: 6250 },
          { month: 'Jan', total_payroll: 275680, employee_count: 45, avg_per_employee: 6127 }
        ],
        tax_breakdown: [
          { tax_type: 'Federal Tax', amount: 55126, percentage_of_gross: 20.0 },
          { tax_type: 'State Tax', amount: 13782, percentage_of_gross: 5.0 },
          { tax_type: 'Social Security', amount: 17082, percentage_of_gross: 6.2 },
  ...
  const columns: GridColDef[] = useMemo(() => [
    ...
    },
    {
      ...
      getActions: (params) => {
        ...
        if (params.row.status === 'calculated' || params.row.status === 'approved') {
          actions.push(
            <GridActionsCellItem
              icon={<ProcessedIcon />}
  ...
  const generatePayslipPdf = () => {
    if (selectedPayroll) {
      window.open(`/api/v1/payroll/${selectedPayroll.id}/payslip.pdf`, '_blank');
    }
  ...
  return (
    <Box sx={{ height: '100vh', display: 'flex', flexDirection: 'column' }}>
          ...
          <Grid item xs={12} md={2.4}>
            <Card sx={{ bgcolor: 'primary.50' }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">{payrollData?.summary.total_employees || 0}</Typography>
          ...
          <Grid item xs={12} md={2.4}>
            <Card sx={{ bgcolor: 'success.50' }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">${payrollData?.summary.total_gross_pay?.toFixed(0) || '0'}</Typography>
          ...
          <Grid item xs={12} md={2.4}>
            <Card sx={{ bgcolor: 'info.50' }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">${payrollData?.summary.total_net_pay?.toFixed(0) || '0'}</Typography>
          ...
          <Grid item xs={12} md={2.4}>
            <Card sx={{ bgcolor: 'error.50' }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">${payrollData?.summary.total_taxes?.toFixed(0) || '0'}</Typography>
          ...
          <Grid item xs={12} md={2.4}>
            <Card sx={{ bgcolor: 'warning.50' }}>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">{payrollData?.summary.pending_count || 0}</Typography>
      ...
      <Dialog open={processDialogOpen} onClose={() => setProcessDialogOpen(false)}>
        ...
        <DialogContent>
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            This will mark the payroll as processed and ready for payment distribution.
          </Typography>
        </DialogContent>
        <DialogActions>
      ...
      {/* Payslip Dialog */}
      <Dialog open={payslipDialogOpen} onClose={() => setPayslipDialogOpen(false)}>
        <DialogTitle>Generate Payslip</DialogTitle>
        <DialogContent>
          <Typography>
            Generate payslip for {selectedPayroll?.employee_name} ({selectedPayroll?.payroll_number})?
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
