...
export const SalesDashboardPage: React.FC = () => {
  ...
  const { data: topProducts } = useQuery({
    ...
    queryFn: async (): Promise<TopProduct[]> => {
      ...
      const response = await apiClient.get(
        `/api/v1/sales/top-products?${params}`,
      );
      return response.data || [];
    },
  });
  ...
  const { data: recentOrders } = useQuery({
    ...
    queryFn: async (): Promise<RecentOrder[]> => {
      const response = await apiClient.get(
        "/api/v1/sales-orders?limit=10&sort=date_desc",
      );
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          Showing data from {new Date(dateRangeObj.start).toLocaleDateString()}{" "}
          to {new Date(dateRangeObj.end).toLocaleDateString()}
        </Typography>
      </Paper>
                ...
                <Box flex={1}>
                  ...
                  <Stack direction="row" alignItems="center" spacing={1} mt={1}>
                    ...
                    >
                      ...
                      {metrics?.revenue_growth?.toFixed(1) || "0"}%
                    </Typography>
                  </Stack>
                </Box>
              </Stack>
                ...
                <Box flex={1}>
                  ...
                  <Stack direction="row" alignItems="center" spacing={1} mt={1}>
                    ...
                    >
                      ...
                      {metrics?.orders_growth?.toFixed(1) || "0"}%
                    </Typography>
                  </Stack>
                </Box>
              </Stack>
                ...
                <Box flex={1}>
                  ...
                  <Stack direction="row" alignItems="center" spacing={1} mt={1}>
                    ...
                    >
                      ...
                      {metrics?.customer_growth?.toFixed(1) || "0"}%
                    </Typography>
                  </Stack>
                </Box>
              </Stack>
                ...
                {salesTargets?.map((target, index) => (
                  <Box key={index}>
                    ...
                    <LinearProgress
                      ...
                      color={
                        target.achievement_rate >= 100
                          ? "success"
                          : target.achievement_rate >= 80
                            ? "warning"
                            : "error"
                      }
                    ...
                    >
                      <Typography variant="caption" color="text.secondary">
                        ${target.actual_revenue.toFixed(0)} / $
                        {target.target_revenue.toFixed(0)}
                      </Typography>
                      <Typography variant="caption" color="text.secondary">
