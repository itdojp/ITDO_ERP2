...
export const ProductImportPage: React.FC = () => {
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File): Promise<ImportPreview> => {
      ...
      const response = await apiClient.post('/api/v1/products-basic/import', formData, {
  ...
  const handleDownloadTemplate = () => {
    // Create CSV template
    const headers = [
      'code', 'name', 'sku', 'standard_price', 'cost_price', 'sale_price',
      'status', 'product_type', 'description', 'category', 'weight', 'length',
      'width', 'height', 'barcode', 'supplier', 'lead_time_days',
      'minimum_stock_level', 'reorder_point', 'is_active', 'is_purchasable',
      'is_sellable', 'is_trackable'
    ];
    
    const sampleRow = [
      'PROD001', 'Sample Product', 'SKU001', '99.99', '49.99', '89.99',
      'active', 'product', 'Sample product description', 'Electronics',
      '1.5', '10', '15', '20', '1234567890123', 'Sample Supplier',
      '7', '10', '5', 'true', 'true', 'true', 'true'
    ];
            ...
            <Card>
              ...
              <CardContent>
                ...
                <Typography variant="body2" sx={{ pl: 2 }}>
                  Use 'true' or 'false' for is_active, is_purchasable, is_sellable, is_trackable
                </Typography>
              </CardContent>
            </Card>
          </Grid>
              ...
              <CardContent>
                <Grid container spacing={3} sx={{ mb: 3 }}>
                  <Grid item xs={12} md={3}>
                    <Box textAlign="center">
                      <Typography variant="h4" color="primary">{importPreview.total_rows}</Typography>
                      <Typography variant="body2" color="text.secondary">Total Rows</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <Box textAlign="center">
                      <Typography variant="h4" color="success.main">{importPreview.valid_rows}</Typography>
                      <Typography variant="body2" color="text.secondary">Valid Rows</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <Box textAlign="center">
                      <Typography variant="h4" color="warning.main">{importPreview.warnings.length}</Typography>
                      <Typography variant="body2" color="text.secondary">Warnings</Typography>
                    </Box>
                  </Grid>
                  <Grid item xs={12} md={3}>
                    <Box textAlign="center">
                      <Typography variant="h4" color="error.main">{importPreview.errors.length}</Typography>
                ...
                {importPreview.errors.length > 0 && (
                  <Alert severity="error" sx={{ mb: 2 }}>
                    Found {importPreview.errors.length} validation errors. Please fix these before importing.
                  </Alert>
                )}

                {importPreview.warnings.length > 0 && (
                  <Alert severity="warning" sx={{ mb: 2 }}>
                    Found {importPreview.warnings.length} warnings. Review these before proceeding.
                  </Alert>
                )}
            ...
            <Card>
              ...
              <CardContent>
                <Stack spacing={2}>
                  ...
                  {(importPreview.errors.length > 3 || importPreview.warnings.length > 3) && (
              ...
              <CardContent sx={{ textAlign: 'center', py: 4 }}>
                {importResult.success ? (
                  <>
                    ...
                    <Typography variant="h4" gutterBottom>Import Complete!</Typography>
                    <Typography variant="body1" color="text.secondary" paragraph>
                      Successfully imported {importResult.imported_count} products
                  ...
                  <>
                    ...
                    <Typography variant="h4" gutterBottom>Import Failed</Typography>
                    <Typography variant="body1" color="text.secondary" paragraph>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
                  Import completed in {importResult.duration_seconds.toFixed(1)} seconds
                </Typography>
