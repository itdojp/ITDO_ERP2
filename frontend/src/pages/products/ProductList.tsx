...
export const ProductList: React.FC = () => {
  ...
  const { data: productData, isLoading, error, refetch } = useQuery({
    queryKey: ['products', search, statusFilter, typeFilter, activeFilter, page, perPage, sortBy, sortOrder],
    queryFn: async (): Promise<ProductListResponse> => {
      ...
      const response = await apiClient.get(`/api/v1/products-basic?${params}`);
      return {
        products: response.data || [],
        total: response.data?.length || 0,
        page,
        pages: Math.ceil((response.data?.length || 0) / perPage),
        per_page: perPage
      };
    },
  });
  ...
  const { data: statistics } = useQuery({
    ...
    queryFn: async (): Promise<ProductStatistics> => {
      const response = await apiClient.get('/api/v1/products-basic/statistics');
          ...
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{statistics.active_products}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">{statistics.low_stock_products}</p>
              </div>
            </div>
          </div>
          <div className="bg-white rounded-lg shadow p-4">
