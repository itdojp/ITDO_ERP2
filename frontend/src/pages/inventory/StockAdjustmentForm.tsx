...
export const StockAdjustmentForm: React.FC = () => {
  ...
  const { data: searchResults, isLoading: isSearching } = useQuery({
    ...
    queryFn: async (): Promise<Product[]> => {
      ...
      const response = await apiClient.get(
        `/api/v1/products-basic/search?q=${encodeURIComponent(searchTerm)}`,
      );
  ...
  const { data: productDetails } = useQuery({
    ...
    queryFn: async (): Promise<Product> => {
      const response = await apiClient.get(
        `/api/v1/inventory/product/${selectedProduct?.id}`,
      );
      return response.data;
    },
  ...
  const adjustmentMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      alert(
        `Stock adjustment completed successfully! New quantity: ${result.new_quantity}`,
      );
      navigate("/inventory");
    },
    onError: (error) => {
  ...
  const onSubmit = async (data: StockAdjustmentFormData) => {
    ...
    if (adjustmentType === "absolute" && data.new_quantity !== undefined) {
      submissionData.quantity_change =
        data.new_quantity - (productDetails?.current_stock || 0);
    }
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600">
            Adjust inventory quantities with proper documentation
          </p>
        </div>
        <button
                      ...
                      {productDetails && (
                        <p className="text-sm text-gray-600 mt-1">
                          ...
                          <span className="font-medium">
                            {productDetails.current_stock} {productDetails.unit}
                          </span>
                        </p>
                      )}
                    </div>
              ...
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-700 mt-1">
                    Stock adjustments are permanent and will be logged with your
