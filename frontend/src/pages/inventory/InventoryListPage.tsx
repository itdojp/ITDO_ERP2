...
export const InventoryListPage: React.FC = () => {
  ...
  const syncMutation = useMutation({
    mutationFn: async (productIds: number[]) => {
      await apiClient.post('/api/v1/inventory/sync', { product_ids: productIds });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['inventory-list'] });
      setSelectedRows([]);
  ...
  const columns: GridColDef[] = useMemo(() => [
    ...
    },
    {
      ...
      renderCell: (params) => (
        <Box textAlign="center">
          ...
          {params.row.movement_trend === 'up' && <TrendingUpIcon color="success" fontSize="small" />}
          {params.row.movement_trend === 'down' && <TrendingDownIcon color="error" fontSize="small" />}
        </Box>
      ),
    },
    {
    ...
    },
    {
      ...
      type: 'number',
      renderCell: (params) => (
        <Typography variant="body2" color={params.value > 0 ? 'warning.main' : 'text.secondary'}>
          {params.value}
        </Typography>
      ),
    },
    {
      ...
      renderCell: (params) => {
        const getStatusConfig = (status: string) => {
          switch (status) {
            case 'in_stock': return { color: 'success' as const, label: 'In Stock' };
            case 'low_stock': return { color: 'warning' as const, label: 'Low Stock' };
            case 'out_of_stock': return { color: 'error' as const, label: 'Out of Stock' };
            case 'overstock': return { color: 'info' as const, label: 'Overstock' };
    ...
    },
    {
      ...
      getActions: (params) => [
        ...
        <GridActionsCellItem
          icon={<MovementIcon />}
          label="History"
          onClick={() => navigate(`/inventory/movements/${params.row.product_id}`)}
        />,
        <GridActionsCellItem
          icon={<LocationIcon />}
          label="Location"
          onClick={() => navigate(`/warehouse/products/${params.row.product_id}`)}
        />,
      ],
    },
  ], [navigate]);
    ...
    >
      ...
      <Stack spacing={3}>
        ...
        <Autocomplete
          ...
          onChange={(_, value) => handleFilterChange('warehouse_location', value?.name || value || '')}
          renderInput={(params) => (
        ...
        <FormControl fullWidth>
          ...
          <Select
            ...
            onChange={(e) => handleFilterChange('movement_trend', e.target.value)}
          ...
          <Grid item xs={12} md={3}>
            <Card>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">{inventoryData?.items.length || 0}</Typography>
          ...
          <Grid item xs={12} md={3}>
            <Card>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">${inventoryData?.total_value?.toFixed(0) || '0'}</Typography>
          ...
          <Grid item xs={12} md={3}>
            <Card>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">{inventoryData?.low_stock_count || 0}</Typography>
          ...
          <Grid item xs={12} md={3}>
            <Card>
              <CardContent sx={{ display: 'flex', alignItems: 'center', py: 2 }}>
                ...
                <Box>
                  <Typography variant="h6">{inventoryData?.alerts || 0}</Typography>
      ...
      <Dialog open={adjustmentDialogOpen} onClose={() => setAdjustmentDialogOpen(false)}>
        ...
        <DialogActions>
          ...
          <Button
            ...
            variant="contained"
            disabled={adjustmentMutation.isPending || !adjustmentQuantity || !adjustmentReason}
          >
