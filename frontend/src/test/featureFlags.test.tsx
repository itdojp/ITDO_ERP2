...
const server = setupServer(
  ...
  rest.post("/api/v1/feature-flags/flags", (req, res, ctx) => {
    return res(ctx.json({ message: "Feature flag created successfully" }));
  }),
);
...
describe("Feature Flag Hooks", () => {
  ...
  describe("useFeatureVariant", () => {
    function TestVariantComponent({ flagKey }: { flagKey: string }) {
      const { variant, isLoading, error } = useFeatureVariant(flagKey, [
        "A",
        "B",
      ]);
    ...
    it("should return variant assignment", async () => {
      ...
      await waitFor(() => {
        expect(screen.getByTestId("variant-result")).toHaveTextContent(
          "Variant: B",
        );
      });
    });
  ...
  describe("useFeatureFlags", () => {
    ...
    it("should evaluate multiple flags", async () => {
      render(
        <TestBulkComponent
          flagKeys={["test-enabled-flag", "test-disabled-flag"]}
        />,
      );
  ...
  it("should render fallback when flag is disabled", async () => {
    render(
      <FeatureFlagWrapper
        ...
        fallback={
          <div data-testid="fallback-content">Feature not available</div>
        }
      >
        <div data-testid="protected-content">Protected Feature</div>
      </FeatureFlagWrapper>,
  ...
  it("should toggle flag state when chip is clicked", async () => {
    ...
    server.use(
      rest.put("/api/v1/feature-flags/flags/:key", (req, res, ctx) => {
        return res(ctx.json({ message: "Feature flag updated successfully" }));
      }),
    );
...
describe("Feature Flags Service", () => {
  describe("evaluateFlag", () => {
    it("should call correct API endpoint", async () => {
      const result = await featureFlagsService.evaluateFlag(
        "test-enabled-flag",
        {
          userId: "test-user",
        },
