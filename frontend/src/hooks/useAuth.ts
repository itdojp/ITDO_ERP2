...
export const useAuth = () => {
  ...
  const loginWithGoogle = useCallback(async () => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'Googleログインに失敗しました');
    }
  }, []);
  ...
  const verifyMFA = useCallback(async (
    ...
    try {
      ...
      throw new Error(error.response?.data?.detail || 'MFA認証に失敗しました');
    }
  }, []);
  ...
  const register = useCallback(async (userData: {
    ...
    try {
      ...
      throw new Error(error.response?.data?.detail || 'アカウント作成に失敗しました');
    }
  }, []);
  ...
  const requestPasswordReset = useCallback(async (email: string) => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'リクエストの処理に失敗しました');
    }
  }, []);
  ...
  const verifyResetToken = useCallback(async (token: string) => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'トークンの検証に失敗しました');
    }
  }, []);
  ...
  const resetPassword = useCallback(async (
    ...
    try {
      ...
      throw new Error(error.response?.data?.detail || 'パスワードのリセットに失敗しました');
    }
  }, []);
  ...
  const setupMFA = useCallback(async (method: string = 'totp') => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'MFAセットアップに失敗しました');
    }
  }, []);

  const verifyMFASetup = useCallback(async (code: string, deviceName: string) => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'MFA設定の確認に失敗しました');
    }
  }, []);
  ...
  const disableMFA = useCallback(async (password: string) => {
    try {
      ...
      throw new Error(error.response?.data?.detail || 'MFAの無効化に失敗しました');
    }
  }, [authState]);
