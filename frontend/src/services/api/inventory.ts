...
export const inventoryApi = {
  ...
  getInventory: async (params: InventoryQueryParams = {}): Promise<InventoryListResponse> => {
  ...
  getMovements: async (params: MovementsQueryParams = {}): Promise<MovementsListResponse> => {
    const response = await apiClient.get('/api/v1/inventory/movements', { params });
    return response.data;
  },

  // Stock adjustments
  adjustStock: async (adjustment: StockAdjustment): Promise<InventoryMovement> => {
    const response = await apiClient.post('/api/v1/inventory/adjust', adjustment);
    return response.data;
  },

  // Transfer stock between locations
  transferStock: async (transfer: InventoryTransfer): Promise<InventoryMovement> => {
    const response = await apiClient.post('/api/v1/inventory/transfer', transfer);
    return response.data;
  },

  // Reserve stock
  reserveStock: async (productId: string, location: string, quantity: number): Promise<InventoryItem> => {
  ...
  releaseStock: async (productId: string, location: string, quantity: number): Promise<InventoryItem> => {
  ...
  getZones: async (location: string): Promise<string[]> => {
    const response = await apiClient.get(`/api/v1/inventory/locations/${location}/zones`);
    return response.data.zones;
  },

  // Physical count update
  updatePhysicalCount: async (inventoryId: string, actualQuantity: number): Promise<InventoryItem> => {
    const response = await apiClient.post(`/api/v1/inventory/${inventoryId}/count`, {
