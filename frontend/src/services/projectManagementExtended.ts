...
import { projectManagementService, ListResponse, ProjectMember, Budget, BudgetCreate, BudgetUpdate, ProgressReport, GanttData, Task, TaskCreate, TaskUpdate, ProjectMemberCreate, ProjectMemberUpdate } from './projectManagement';
...
export const projectManagementServiceExtended = {
  ...
  members: {
    list: async (params?: { projectId?: number }): Promise<ListResponse<ProjectMember>> => {
      if (params?.projectId) {
    ...
    add: async (data: ProjectMemberCreate): Promise<ProjectMember> => {
      const response = await projectManagementService.members.add(data.projectId, data);
      // Convert ListResponse to single ProjectMember
      return response.items[0];
    },
    
    update: async (memberId: number, data: ProjectMemberUpdate): Promise<ProjectMember> => {
  ...
  budgets: {
    ...
    create: async (data: BudgetCreate): Promise<Budget> => {
      ...
      return {
        ...
        variancePercentage: ((data.budgetAmount - data.actualCost) / data.budgetAmount) * 100,
        revenue: data.revenue || 0,
        profit: (data.revenue || 0) - data.actualCost,
        profitRate: data.revenue ? ((data.revenue - data.actualCost) / data.revenue) * 100 : 0,
  ...
  tasks: {
    list: async (params?: { projectId?: number }): Promise<ListResponse<Task>> => {
      if (params?.projectId) {
        const result = await projectManagementService.tasks.list(params.projectId);
        return {
          items: result.tasks,
