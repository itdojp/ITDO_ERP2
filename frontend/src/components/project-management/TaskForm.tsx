...
const createSchema = yup.object({
  ...
  estimatedHours: yup.number().min(0, '見積時間は0以上で入力してください').nullable(),
...
const updateSchema = yup.object({
  ...
  estimatedHours: yup.number().min(0, '見積時間は0以上で入力してください').nullable(),
  actualHours: yup.number().min(0, '実績時間は0以上で入力してください').nullable(),
  priority: yup.string().oneOf(['high', 'medium', 'low']),
  status: yup.string().oneOf(['not_started', 'in_progress', 'completed', 'on_hold']),
  progressPercentage: yup.number().min(0).max(100, '進捗率は0〜100で入力してください'),
});
...
export const TaskForm: React.FC<TaskFormProps> = ({
  ...
  useEffect(() => {
    if (isEdit) {
      ...
      } else if (watchProgress > 0 && watchProgress < 100 && watchStatus === 'not_started') {
