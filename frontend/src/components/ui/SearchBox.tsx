...
export const SearchBox: React.FC<SearchBoxProps> = ({
  ...
  }, [suggestions, asyncSuggestions, inputValue, minSearchLength, loadSuggestions]);

  // Load async suggestions with debounce
  const loadAsyncSuggestions = useCallback(async (query: string) => {
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    const currentSuggestions = filteredSuggestions.length > 0 ? filteredSuggestions : 
      (showRecentSearches && recentSearches.length > 0 ? recentSearches : []);
    ...
    switch (e.key) {
      ...
      case 'Tab':
        if (autoComplete && currentSuggestions.length > 0) {
          e.preventDefault();
          const suggestionToSelect = highlightedIndex >= 0 
  ...
  }, [isOpen, filteredSuggestions, highlightedIndex, handleSuggestionSelect, showRecentSearches, recentSearches, autoComplete]);
  ...
  const renderSuggestionItem = useCallback((suggestion: string, index: number) => {
    ...
    return (
      ...
      >
        {renderSuggestion ? renderSuggestion(suggestion) : highlightText(suggestion, inputValue)}
      </div>
    );
  }, [highlightedIndex, renderSuggestion, highlightText, inputValue, handleSuggestionSelect]);
  ...
  const renderSuggestions = () => {
    ...
    const hasRecent = showRecentSearches && recentSearches.length > 0 && !inputValue;
    ...
    return (
      <div className="absolute z-50 w-full mt-1 bg-white border border-gray-300 rounded-md shadow-lg" data-testid="suggestions-dropdown">
  ...
  const renderFilterDropdown = () => {
    ...
    return (
      <div className="absolute z-50 right-0 mt-1 w-64 bg-white border border-gray-300 rounded-md shadow-lg" data-testid="filter-dropdown">
  ...
  const renderCategories = () => {
    ...
    return (
      <div className="flex items-center space-x-1 mb-2" data-testid="category-selector">
  ...
  const renderSearchScopes = () => {
    ...
    return (
      <div className="flex items-center space-x-1 mb-2" data-testid="scope-selector">
    ...
    >
      ...
      <div className="relative">
        <div className={cn(
          ...
          <div className="pl-3 flex-shrink-0" data-testid="search-icon">
            {loading ? (
              <div className="w-5 h-5 border-2 border-gray-300 border-t-blue-500 rounded-full animate-spin" data-testid="search-loading" />
            ) : (
      ...
      {showAdvanced && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-96" data-testid="advanced-search-modal">
