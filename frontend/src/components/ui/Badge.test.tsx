...
describe('Badge', () => {
  ...
  it('renders with outlined style', () => {
    render(<Badge variant="primary" outlined>Test</Badge>);
    
    const badge = screen.getByText('Test').parentElement;
    expect(badge).toHaveClass('border-blue-500', 'text-blue-500', 'bg-transparent');
  });
  ...
  it('renders as closable with close button', () => {
    const onClose = vi.fn();
    render(<Badge closable onClose={onClose}>Closable</Badge>);
  ...
  it('renders with gradient background', () => {
    ...
    expect(badge).toHaveClass('bg-gradient-to-r', 'from-purple-400', 'to-pink-400');
  });
  ...
  it('handles accessibility attributes', () => {
    render(<Badge role="status" aria-label="Notification count">3</Badge>);
  ...
  it('handles overflow text with title', () => {
    ...
    expect(badge).toHaveAttribute('title', 'Very long badge text that overflows');
  });
