...
describe('FileUpload', () => {
  ...
  it('respects file type restrictions', () => {
    const onUpload = vi.fn();
    const onError = vi.fn();
    render(<FileUpload accept=".txt,.doc" onUpload={onUpload} onError={onError} />);
  ...
  it('supports custom upload button', () => {
    render(<FileUpload customButton={<button data-testid="custom-upload-btn">Upload Files</button>} />);
  ...
  it('supports custom error messages', () => {
    ...
    expect(screen.getByText('Custom file too large message')).toBeInTheDocument();
  });

  it('supports file validation callback', () => {
    const validate = vi.fn().mockReturnValue({ valid: false, message: 'Custom validation error' });
  ...
  it('supports custom loading component', () => {
    const LoadingComponent = () => <div data-testid="custom-loading">Uploading...</div>;
  ...
  it('supports file metadata display', () => {
    ...
    const file = new File(['content'], 'test.txt', { type: 'text/plain' });
    Object.defineProperty(file, 'lastModified', { value: new Date('2023-01-01').getTime() });
