...
export const FileUploader: React.FC<FileUploaderProps> = ({
  ...
  const processFiles = useCallback(async (fileList: FileList) => {
    ...
    const validFiles = processedFiles.filter(f => f.status === 'pending').map(f => f.file);
    if (validFiles.length > 0) {
      onFileSelect?.(validFiles);
    }
  }, [files.length, maxFiles, maxFileSize, acceptedTypes, showPreview, onFileSelect]);
    ...
    <div className="w-full">
        ...
        <div className="space-y-4">
          ...
          <div>
            <p className="text-lg font-medium text-gray-700">
              {dragAndDrop ? 'ファイルをドラッグ＆ドロップまたはクリック' : 'クリックしてファイルを選択'}
            </p>
      ...
      {files.length > 0 && (
        <div className="mt-6 space-y-3">
          ...
          {files.map((fileItem) => (
            <div key={fileItem.id} className="flex items-center space-x-4 p-3 border border-gray-200 rounded-lg">
              {/* Preview */}
          ...
          {onFileUpload && files.some(f => f.status === 'pending') && (
            <div className="flex justify-end pt-4">
              <button
                ...
                type="button"
              >
                アップロード ({files.filter(f => f.status === 'pending').length})
              </button>
            </div>
          )}
        </div>
