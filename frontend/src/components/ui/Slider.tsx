...
export const Slider = forwardRef<HTMLInputElement, SliderProps>(
  (
      ...
      return (
        <input
          ...
          style={{
            ...
            width: vertical
              ? "100%"
              : sizeClasses.handle.split(" ")[0].replace("w-", ""),
            height: vertical
              ? sizeClasses.handle.split(" ")[1].replace("h-", "")
              : "100%",
          }}
        />
      );
      ...
      return Object.entries(marks).map(([value, label]) => {
        ...
        return (
          <div key={value} className="absolute" style={markPosition}>
            <div
              className={`w-1 h-1 bg-gray-400 rounded-full ${vertical ? "" : "transform -translate-x-1/2"}`}
            />
            <div
              className={`text-xs text-gray-500 mt-2 ${vertical ? "ml-3" : "transform -translate-x-1/2"}`}
            >
              {label}
            </div>
          </div>
    ...
    if (loading) {
      return (
        <div
          className={`relative ${vertical ? "flex flex-col" : ""} ${className}`}
        >
