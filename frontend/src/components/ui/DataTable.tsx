...
export const DataTable = <T extends Record<string, any>>({
  ...
  const handleSort = (column: Column<T>) => {
    ...
    if (currentSortConfig?.key === column.key) {
      newSortConfig = currentSortConfig.direction === 'asc' 
  ...
  if (loading && !dataSource.length) {
    return (
      <div className={`${className}`}>
        <div className={`overflow-auto ${scroll?.x ? 'overflow-x-auto' : ''} ${scroll?.y ? 'overflow-y-auto' : ''}`} 
             style={{ maxHeight: scroll?.y }}>
          <table className={`w-full ${getSizeClasses()} ${bordered ? 'border-collapse border border-gray-200' : ''} ${tableClassName}`}>
            <thead className={`bg-gray-50 ${headerClassName}`}>
              <tr>
                {columns.map(column => (
                  <th key={column.key} className={`${getCellPadding()} text-left font-medium text-gray-900 border-b border-gray-200`}>
                    {column.title}
                  </th>
                ))}
              </tr>
      ...
      <div className={`overflow-auto ${scroll?.x ? 'overflow-x-auto' : ''} ${scroll?.y ? 'overflow-y-auto' : ''}`} 
        ...
        <table className={`w-full ${getSizeClasses()} ${bordered ? 'border-collapse border border-gray-200' : ''} ${tableClassName}`}>
          <thead className={`bg-gray-50 ${headerClassName}`}>
            <tr>
              {rowSelection && (
                <th className={`${getCellPadding()} text-left font-medium text-gray-900 border-b border-gray-200`} style={{ width: '40px' }}>
          ...
          <tbody className={`${bodyClassName} ${hoverable ? 'divide-y divide-gray-200' : ''}`}>
            {loading && renderLoadingRow()}
            {!loading && !sortedData.length && renderEmptyRow()}
            {!loading && sortedData.map((record, index) => {
              const key = getRowKey(record, index);
              ...
              return (
                <React.Fragment key={key}>
                  ...
                  >
                    {rowSelection && (
                      <td className={`${getCellPadding()} border-b border-gray-200`}>
                        <input
