...
const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  (
    ...
    const textareaElement = (
      <div className="relative">
        <textarea
          ...
          role={role}
          style={
            autoResize ? { minHeight: `${rows * 1.5}rem`, ...style } : style
          }
          className={[
        ...
        {icon && (
          <div className="absolute top-3 right-3 text-gray-400">{icon}</div>
        )}
      </div>
    );
    ...
    const countElement = showCount && maxLength && (
      <div
        className={`text-sm text-right mt-2 ${isOverLimit ? "text-red-500" : "text-gray-500"}`}
      >
