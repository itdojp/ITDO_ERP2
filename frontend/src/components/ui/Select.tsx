...
export const Select = forwardRef<HTMLDivElement, SelectProps>(({
  ...
  const selectedValues = Array.isArray(currentValue) ? currentValue : [currentValue].filter(v => v !== '');
  ...
  const filteredOptions = useCallback(() => {
    ...
    if (allowCreate && searchValue && !filtered.some(opt => opt.label.toLowerCase() === searchValue.toLowerCase())) {
      filtered = [
        { value: searchValue, label: `Create "${searchValue}"`, group: '__create__' },
        ...filtered
      ];
    }
  ...
  const renderSelectedContent = () => {
    ...
    if (multiple) {
      const displayValues = maxTagCount && selectedValues.length > maxTagCount
        ? selectedValues.slice(0, maxTagCount)
        : selectedValues;
        
      const remainingCount = selectedValues.length - (maxTagCount || selectedValues.length);
  ...
  const renderDropdown = () => {
    ...
    const hasResults = ungrouped.length > 0 || Object.keys(grouped).length > 0;
          ...
          <>
            ...
            {Object.entries(grouped).map(([groupName, groupOptions]) => (
              <div key={groupName}>
                ...
                {groupOptions.map((option, index) => renderOption(option, ungrouped.length + index))}
              </div>
            ))}
          </>
        )}
    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 flex-1 min-w-0">
          {option.icon && !isCreateOption && <span className="flex-shrink-0">{option.icon}</span>}
