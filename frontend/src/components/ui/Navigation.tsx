...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const renderBreadcrumbItems = () => {
    ...
    if (maxItems && displayItems.length > maxItems) {
      ...
      displayItems = [firstItem, { id: 'ellipsis', label: '...', href: '#' }, ...lastItems];
    }
      ...
      <div key={item.id} className={cn(level > 0 && 'ml-4')}>
        <a
          ...
          className={cn(
            ...
            isCurrent && variant === 'pills' && 'bg-blue-500 text-white rounded-full',
            isCurrent && variant === 'tabs' && 'border-blue-500 text-blue-600',
            isCurrent && variant === 'underline' && 'border-blue-500 text-blue-600'
          )}
          onClick={(e) => {
            if (hasChildren) {
              e.preventDefault();
  ...
  if (loading) {
    return (
      <div data-testid="navigation-loading" className="flex items-center justify-center py-4">
        {loadingComponent || (
    ...
    >
      ...
      <div
        ...
        className={cn(
          ...
          collapsible && 'md:flex',
          collapsible && !isCollapsed ? 'block' : collapsible ? 'hidden' : 'block'
        )}
      >
        {type === 'breadcrumb' ? (
