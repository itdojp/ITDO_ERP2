...
describe("Calendar", () => {
  ...
  it("supports minimum date restriction", () => {
    ...
    const currentMonthDay5 = dayFives.find(
      (day) => !day.closest("button")?.classList.contains("text-gray-400"),
    );
  ...
  it("renders custom day content", () => {
    const renderDay = (date: Date) => (
      <div data-testid={`custom-day-${date.getDate()}`}>{date.getDate()}</div>
    );
  ...
  it("renders custom header", () => {
    const renderHeader = (date: Date) => (
      <div data-testid="custom-header">Custom {date.toLocaleDateString()}</div>
    );
  ...
  it("supports custom loading component", () => {
    const LoadingComponent = () => (
      <div data-testid="custom-loading">Loading calendar...</div>
    );
  ...
  it("supports custom date cells", () => {
    const renderDateCell = (date: Date, isSelected: boolean) => (
      <div
        data-testid={`custom-cell-${date.getDate()}`}
