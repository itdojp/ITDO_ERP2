...
describe('Upload', () => {
  ...
  it('displays file list', async () => {
    const defaultFileList = [
      { uid: '1', name: 'existing.txt', status: 'done' as const, url: 'https://example.com/existing.txt' }
    ];
  ...
  it('removes files from list', async () => {
    ...
    expect(onRemove).toHaveBeenCalledWith(expect.objectContaining({ uid: '1' }));
  });
  ...
  it('shows upload status', async () => {
    const fileList = [
      { uid: '1', name: 'uploading.txt', status: 'uploading' as const, percent: 50 },
  ...
  it('renders with custom className', () => {
    ...
    const uploadElement = screen.getByText(/drag and drop/i).closest('.custom-upload');
