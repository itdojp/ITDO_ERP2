...
export const Toolbar: React.FC<ToolbarProps> = ({
  ...
  const visibleToolsList =
    showOverflow && filteredTools.length > maxVisibleTools
  ...
  const handleKeyDown = useCallback(
    (e: React.KeyboardEvent, toolIndex: number) => {
      ...
      switch (e.key) {
        ...
        case "ArrowUp":
          ...
          const prevIndex =
            orientation === "horizontal"
              ? e.key === "ArrowLeft"
                ? (toolIndex - 1 + totalTools) % totalTools
                : toolIndex
              : e.key === "ArrowUp"
                ? (toolIndex - 1 + totalTools) % totalTools
                : toolIndex;
  ...
  const handleContextMenu = useCallback(
    (e: React.MouseEvent, tool: ToolbarTool) => {
  ...
  const handleDragStart = useCallback(
    (e: React.DragEvent, tool: ToolbarTool) => {
  ...
  const handleDrop = useCallback(
    (e: React.DragEvent, targetTool: ToolbarTool) => {
  ...
  const groupedTools = visibleToolsList.reduce(
    (groups, tool) => {
  ...
  const renderToolShortcut = (tool: ToolbarTool) => {
    ...
    return (
      <span
        ...
        data-testid={`tool-shortcut-${tool.id}`}
      >
        {tool.shortcut}
      </span>
    );
  ...
  const renderToolDescription = (tool: ToolbarTool) => {
    ...
    return (
      <div
        ...
        data-testid={`tool-description-${tool.id}`}
      >
        {tool.description}
      </div>
    );
  ...
  const renderDropdown = (tool: ToolbarTool) => {
    if (tool.type !== "dropdown" || openDropdown !== tool.id || !tool.items)
      return null;
  ...
  const renderTool = (tool: ToolbarTool, index?: number) => {
    ...
    if (tool.type === "group") {
      return <div key={tool.id}>{renderToolGroup(tool)}</div>;
    }
    ...
    return (
      <div key={tool.id} className="relative">
        ...
        >
          ...
          {tool.type === "dropdown" && <span className="ml-1 text-xs">▼</span>}
        </button>
    ...
    return (
      <div className="relative">
        <button
          ...
          onClick={() =>
            setOpenDropdown(openDropdown === "overflow" ? null : "overflow")
          }
      ...
      {showSeparators && groupKeys.length > 1
        ? groupKeys.map((groupKey, groupIndex) => (
            <React.Fragment key={groupKey}>
              {groupIndex > 0 &&
                renderSeparator(`${groupKeys[groupIndex - 1]}-${groupKey}`)}
              {groupedTools[groupKey].map((tool, toolIndex) =>
                renderTool(tool, toolIndex),
              )}
            </React.Fragment>
          ))
