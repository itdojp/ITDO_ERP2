...
describe('Result', () => {
  ...
  it('renders different status types', () => {
    const statuses = ['success', 'error', 'info', 'warning', '404', '403', '500'] as const;
    
    statuses.forEach(status => {
      const { unmount } = render(<Result status={status} title={`Status ${status}`} />);
  ...
  it('renders with subtitle', () => {
    render(<Result title="Success" subTitle="Operation completed successfully" />);
    expect(screen.getByText('Success')).toBeInTheDocument();
  ...
  it('supports different layouts', () => {
    ...
    layouts.forEach(layout => {
      const { unmount } = render(<Result layout={layout} title={`Layout ${layout}`} />);
  ...
  it('supports collapsible content', () => {
    render(<Result title="Collapsible" collapsible>
  ...
  it('renders with help text', () => {
    render(<Result title="Help" helpText="Need assistance? Contact support." />);
    expect(screen.getByText('Need assistance? Contact support.')).toBeInTheDocument();
  });
  ...
  it('renders with custom data attributes', () => {
    render(<Result title="Data Attrs" data-testid="custom-result" data-status="success" />);
  ...
  it('supports custom title element', () => {
    const customTitle = <h1 data-testid="custom-title">Custom Title Element</h1>;
