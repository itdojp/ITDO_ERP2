...
describe('Menu', () => {
  ...
  it('supports different orientations', () => {
    ...
    orientations.forEach(orientation => {
      const { unmount } = render(<Menu items={mockMenuItems} orientation={orientation} />);
  ...
  it('supports different variants', () => {
    ...
    variants.forEach(variant => {
      const { unmount } = render(<Menu items={mockMenuItems} variant={variant} />);
      expect(screen.getByTestId('menu-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('handles Enter key on focused item', () => {
    ...
    const testItems = [
      { id: '1', label: 'Item 1', onClick: onItemClick },
    ];
  ...
  it('supports multi-selection', () => {
    const onSelect = vi.fn();
    render(<Menu items={mockMenuItems} selectable multiple onSelect={onSelect} />);
  ...
  it('supports menu positioning', () => {
    ...
    positions.forEach(position => {
      const { unmount } = render(<Menu items={mockMenuItems} position={position} />);
      expect(screen.getByTestId('menu-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('supports custom loading component', () => {
    ...
    render(<Menu items={mockMenuItems} loading loadingComponent={<LoadingComponent />} />);
    
    expect(screen.getByTestId('custom-loading')).toBeInTheDocument();
  });

  it('supports menu search', () => {
    render(<Menu items={mockMenuItems} searchable searchPlaceholder="Search menu..." />);
  ...
  it('supports menu item descriptions', () => {
    const itemsWithDescriptions = [
      { id: '1', label: 'Export', description: 'Export data to file', onClick: vi.fn() },
      { id: '2', label: 'Import', description: 'Import data from file', onClick: vi.fn() },
    ];
  ...
  it('supports custom data attributes', () => {
    render(<Menu items={mockMenuItems} data-category="navigation" data-id="main-menu" />);
