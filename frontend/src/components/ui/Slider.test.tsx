...
describe("Slider", () => {
  ...
  it("applies size classes correctly", () => {
    ...
    let track = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="h-1"]');
    expect(track).toBeInTheDocument();

    rerender(<Slider size="md" />);
    track = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="h-2"]');
    expect(track).toBeInTheDocument();

    rerender(<Slider size="lg" />);
    track = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="h-3"]');
    expect(track).toBeInTheDocument();
  });

  it("applies color variants correctly", () => {
    const { rerender } = render(<Slider color="primary" defaultValue={50} />);
    let filledTrack = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="bg-blue-500"]');
    expect(filledTrack).toBeInTheDocument();

    rerender(<Slider color="success" defaultValue={50} />);
    filledTrack = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="bg-green-500"]');
    expect(filledTrack).toBeInTheDocument();

    rerender(<Slider color="warning" defaultValue={50} />);
    filledTrack = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="bg-yellow-500"]');
    expect(filledTrack).toBeInTheDocument();

    rerender(<Slider color="danger" defaultValue={50} />);
    filledTrack = screen
      .getByRole("slider")
      .parentElement?.querySelector('[class*="bg-red-500"]');
    expect(filledTrack).toBeInTheDocument();
  });
  ...
  it("handles reverse orientation", () => {
    ...
    const filledTrack = screen
      .getByRole("slider")
      .parentElement?.querySelector('[style*="right"]');
    expect(filledTrack).toBeInTheDocument();
  });
  ...
  it("displays custom track style", () => {
    ...
    const track = screen
      .getByRole("slider")
      .parentElement?.querySelector('[style*="background-color: red"]');
    expect(track).toBeInTheDocument();
  });
