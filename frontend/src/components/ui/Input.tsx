...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  (
    ...
    const renderPrefix = () => {
      ...
      return <span className="flex-shrink-0 text-gray-500 mr-2">{prefix}</span>;
    };
    ...
    const renderSuffix = () => {
      ...
      if (shouldShowCount) {
        suffixElements.push(
          <span
            key="count"
            className={`flex-shrink-0 ml-2 text-xs ${characterCount > maxLength! ? "text-red-500" : "text-gray-400"}`}
          >
      ...
      return suffixElements.length > 0 ? (
        <div className="flex items-center">{suffixElements}</div>
      ) : null;
    };
...
const Password = forwardRef<
  HTMLInputElement,
  Omit<InputProps, "type" | "visibilityToggle">
>((props, ref) => {
  ...
  (
    ...
    return (
      <div className={`relative ${className}`}>
        ...
        {showCount && maxLength && (
          <div
            className={`absolute bottom-2 right-2 text-xs ${characterCount > maxLength ? "text-red-500" : "text-gray-400"}`}
          >
            {characterCount}/{maxLength}
          </div>
        )}
