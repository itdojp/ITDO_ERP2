...
export interface DataTableProps<T = any> {
  ...
  getCellProps?: (row: T, column: DataTableColumn<T>) => React.HTMLAttributes<HTMLTableCellElement>;
  ariaLabel?: string;
  ...
  onCellClick?: (row: T, column: DataTableColumn<T>, event: React.MouseEvent) => void;
...
export const DataTable = <T extends Record<string, any>>(props: DataTableProps<T>) => {
  const {
    ...
    onPerformanceReport,
    className,
    'data-testid': dataTestId = 'datatable-container',
  ...
  useEffect(() => {
    if (enablePerformanceMonitoring) {
      ...
      performanceRef.current.rowCount = displayData.length;
      performanceRef.current.columnCount = visibleColumns.length;
      ...
      return () => {
        ...
        const metrics = {
          ...
          rowCount: performanceRef.current.rowCount,
          columnCount: performanceRef.current.columnCount,
          timestamp: new Date()
        };
        onPerformanceReport?.(metrics);
      };
    }
  }, [enablePerformanceMonitoring, displayData.length, visibleColumns.length, onPerformanceReport]);
  ...
  // Render table header
  const renderHeader = () => (
    <thead className={cn('bg-gray-50', stickyHeader && 'sticky top-0 z-10')} data-testid="datatable-header">
      <tr>
        {selectable && (
  ...
  const renderPagination = () => {
    ...
    return (
      <div className="flex items-center justify-between p-4 border-t" data-testid="datatable-pagination">
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64" data-testid="datatable-loading">
  ...
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-64" data-testid="datatable-empty">
