...
describe('Select', () => {
  ...
  it('selects single option', () => {
    ...
    expect(onChange).toHaveBeenCalledWith('option1', expect.objectContaining({ value: 'option1', label: 'Option 1' }));
  });
  ...
  it('handles multiple selection', () => {
    ...
    expect(onChange).toHaveBeenLastCalledWith(['option1', 'option2'], expect.any(Array));
  });
  ...
  it('removes tags in multiple mode', () => {
    const onChange = jest.fn();
    render(<Select options={mockOptions} multiple value={['option1', 'option2']} onChange={onChange} />);
    
    const removeButtons = screen.getAllByRole('button');
    const firstRemoveButton = removeButtons.find(btn => btn.closest('[class*="bg-blue-100"]'));
  ...
  it('handles disabled state', () => {
    ...
    expect(select.parentElement).toHaveClass('opacity-50', 'cursor-not-allowed');
  });
  ...
  it('clears selection when clear button is clicked', () => {
    const onChange = jest.fn();
    render(<Select options={mockOptions} clearable value="option1" onChange={onChange} />);
  ...
  it('applies status classes correctly', () => {
    const { rerender } = render(<Select options={mockOptions} status="error" />);
    let select = screen.getByRole('combobox').parentElement;
  ...
  it('shows "not found" content when no options match', async () => {
    const user = userEvent.setup();
    render(<Select options={mockOptions} searchable notFoundContent="No matches" />);
  ...
  it('limits displayed tags with maxTagCount', () => {
    render(<Select options={mockOptions} multiple value={['option1', 'option2', 'option4']} maxTagCount={2} />);
  ...
  it('allows creating new options when allowCreate is true', async () => {
    const user = userEvent.setup();
    const onChange = jest.fn();
    render(<Select options={mockOptions} searchable allowCreate onChange={onChange} />);
    ...
    expect(onChange).toHaveBeenCalledWith('New Option', expect.objectContaining({ value: 'New Option' }));
  });

  it('calls onDropdownVisibleChange when dropdown opens/closes', () => {
    const onDropdownVisibleChange = jest.fn();
    render(<Select options={mockOptions} onDropdownVisibleChange={onDropdownVisibleChange} />);
  ...
  it('applies custom classNames', () => {
    ...
    const dropdown = screen.getByText('Option 1').closest('[class*="custom-dropdown"]');
  ...
  it('uses custom filter function', async () => {
    ...
    render(<Select options={mockOptions} searchable filterOption={customFilter} />);
  ...
  it('deselects option in multiple mode when clicking selected option', () => {
    const onChange = jest.fn();
    render(<Select options={mockOptions} multiple value={['option1']} onChange={onChange} />);
