...
const TestInput: React.FC<{
  ...
  value?: string;
}> = ({ name, placeholder, type = 'text', onChange, onBlur, value: propValue }) => {
...
describe('Form', () => {
  ...
  it('validates field on blur', async () => {
    ...
    render(
      <Form validationRules={validationRules} validateOnChange={false} onSubmit={jest.fn()}>
        <FormItem name="name" label="Name">
          <TestInput name="name" />
        </FormItem>
      </Form>
  ...
  it('validates min and max length', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('password must be at most 20 characters')).toBeInTheDocument();
    });
  });
    ...
    const TestFormWithValidation = () => {
      ...
      return (
        <div>
          ...
          <button type="button" onClick={() => formApi.validateFields(['name'])}>
            Validate Name Only
          </button>
        </div>
      );
  ...
  it('displays help text', () => {
    render(
      <Form onSubmit={jest.fn()}>
        <FormItem name="password" label="Password" help="Password must be at least 8 characters">
          <TestInput name="password" type="password" />
        </FormItem>
      </Form>
    );
