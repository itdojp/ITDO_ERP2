...
describe('Carousel', () => {
  ...
  it('pauses autoplay on hover', () => {
    render(<Carousel items={mockItems} autoplay autoplayInterval={1000} pauseOnHover />);
  ...
  it('supports different variants', () => {
    ...
    variants.forEach(variant => {
      const { unmount } = render(<Carousel items={mockItems} variant={variant} />);
      expect(screen.getByTestId('carousel-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('supports responsive items per view', () => {
    ...
    expect(screen.getByTestId('carousel-container')).toHaveClass('responsive-carousel');
  });
  ...
  it('closes zoom on escape key', () => {
    ...
    expect(screen.queryByTestId('carousel-zoom-overlay')).not.toBeInTheDocument();
  });
  ...
  it('supports custom loading component', () => {
    const LoadingComponent = () => <div data-testid="custom-loading">Loading carousel...</div>;
    render(<Carousel items={mockItems} loading loadingComponent={<LoadingComponent />} />);
  ...
  it('handles slide change events', () => {
    ...
    render(<Carousel items={mockItems} onSlideChange={onSlideChange} showArrows />);
  ...
  it('supports custom arrow components', () => {
    const CustomPrevArrow = () => <button data-testid="custom-prev">Previous</button>;
    const CustomNextArrow = () => <button data-testid="custom-next">Next</button>;
  ...
  it('supports custom counter format', () => {
    render(<Carousel items={mockItems} showCounter counterFormat={(current, total) => `${current} of ${total}`} />);
  ...
  it('supports custom data attributes', () => {
    render(<Carousel items={mockItems} data-category="media" data-id="main-carousel" />);
  ...
  it('supports variable width slides', () => {
    const variableItems = [
      { id: '1', content: <div data-testid="slide-1" style={{ width: '200px' }}>Slide 1</div> },
      { id: '2', content: <div data-testid="slide-2" style={{ width: '300px' }}>Slide 2</div> },
    ];
