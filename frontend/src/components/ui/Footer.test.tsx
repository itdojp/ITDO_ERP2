...
describe("Footer", () => {
  ...
  it("renders footer with company info", () => {
    ...
    expect(
      screen.getByText("Building amazing products since 2020"),
    ).toBeInTheDocument();
  });
  ...
  it("renders copyright notice", () => {
    ...
    expect(
      screen.getByText("© 2023 Acme Corp. All rights reserved."),
    ).toBeInTheDocument();
  });
  ...
  it("auto-generates copyright with current year", () => {
    ...
    expect(
      screen.getByText(`© ${currentYear} Acme Corp. All rights reserved.`),
    ).toBeInTheDocument();
  });

  it("renders logo", () => {
    const logo = (
      <img data-testid="footer-logo" src="/logo.png" alt="Company Logo" />
    );
  ...
  it("supports different variants", () => {
    ...
    variants.forEach((variant) => {
      const { unmount } = render(
        <Footer companyName="Test" variant={variant} />,
      );
  ...
  it("supports custom loading component", () => {
    const LoadingComponent = () => (
      <div data-testid="custom-loading">Loading footer...</div>
    );
    render(
      <Footer
        companyName="Test"
    ...
    const linksWithClick = [
      {
        id: "1",
        title: "Support",
        links: [{ id: "1-1", label: "Help Center", onClick }],
      },
    ];
