...
export const DataGrid = <T extends Record<string, any>>({
  ...
  const visibleColumns = useMemo(() => {
    return columns.filter(
      (col) => col.visible !== false && !hiddenColumns.includes(col.key),
    );
    ...
    if (internalGlobalFilter) {
      filtered = filtered.filter((row) =>
        Object.values(row).some((value) =>
          String(value)
            .toLowerCase()
            .includes(internalGlobalFilter.toLowerCase()),
        ),
      );
    }
      ...
      if (multiSort) {
        if (existingSort) {
          if (existingSort.direction === "asc") {
            ...
            newSortConfig = internalSortConfig.filter(
              (s) => s.key !== columnKey,
            );
          }
        } else {
  ...
  const handleCellEdit = useCallback(
    (rowId: any, columnKey: string, value: any) => {
        ...
        case "ArrowDown":
          ...
          if (rowIndex < totalRows - 1) {
            const nextCell = document.querySelector(
              `[data-testid="cell-${displayData[rowIndex + 1][rowKey]}-${visibleColumns[colIndex].key}"]`,
            ) as HTMLElement;
            nextCell?.focus();
          }
          break;
        ...
        case "ArrowRight":
          ...
          if (colIndex < totalCols - 1) {
            const rightCell = document.querySelector(
              `[data-testid="cell-${displayData[rowIndex][rowKey]}-${visibleColumns[colIndex + 1].key}"]`,
            ) as HTMLElement;
            rightCell?.focus();
          }
          break;
  ...
  // Render toolbar
  const renderToolbar = () => {
    if (!toolbar && !filterable && !exportable && !columnVisibility)
      return null;
  ...
  const renderBulkActions = () => {
    ...
    return (
      <div
        className="flex items-center space-x-2 p-2 bg-blue-50 border-b"
        ...
        {selectable && (
          <th className="w-10 px-4 py-3">
            <input
              type="checkbox"
              checked={
                internalSelectedRows.length === displayData.length &&
            ...
            <div className="flex items-center justify-between">
              ...
              {sortable && column.sortable !== false && (
                <div className="flex flex-col ml-2">
                  {internalSortConfig.find((s) => s.key === column.key)
                    ?.direction === "asc" && (
          ...
          {visibleColumns.map((column, colIndex) => {
            ...
            return (
              <td
                ...
                onDoubleClick={() =>
                  editable && handleCellEdit(rowId, column.key, value)
                }
  ...
  const renderGroupedRows = () => {
    ...
    return Object.entries(groupedData as Record<string, T[]>).map(
    ...
    return (
      ...
      >
        <div className="text-sm text-gray-600">
          ...
          of {totalRows || sortedData.length} entries
        </div>
