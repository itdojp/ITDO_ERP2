...
describe('Navigation', () => {
  ...
  it('supports different sizes', () => {
    ...
    sizes.forEach(size => {
      const { unmount } = render(<Navigation items={mockNavItems} size={size} />);
  ...
  it('supports different variants', () => {
    ...
    variants.forEach(variant => {
      const { unmount } = render(<Navigation items={mockNavItems} variant={variant} />);
      expect(screen.getByTestId('navigation-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('supports custom loading component', () => {
    const LoadingComponent = () => <div data-testid="custom-loading">Loading nav...</div>;
    render(<Navigation items={mockNavItems} loading loadingComponent={<LoadingComponent />} />);
  ...
  it('supports theme variants', () => {
    ...
    themes.forEach(theme => {
      const { unmount } = render(<Navigation items={mockNavItems} theme={theme} />);
      const nav = screen.getByTestId('navigation-container');
  ...
  it('supports custom className', () => {
    ...
    expect(screen.getByTestId('navigation-container')).toHaveClass('custom-nav');
  });
  ...
  it('supports menu positioning', () => {
    ...
    positions.forEach(position => {
      const { unmount } = render(<Navigation items={mockNavItems} menuPosition={position} />);
      expect(screen.getByTestId('navigation-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('supports custom data attributes', () => {
    render(<Navigation items={mockNavItems} data-category="main-nav" data-id="primary" />);
  ...
  it('supports breadcrumb with max items', () => {
    ...
    render(<Navigation items={manyBreadcrumbs} type="breadcrumb" maxItems={5} />);
