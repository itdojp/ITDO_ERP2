...
describe('Tooltip', () => {
  ...
  it('supports different variants', async () => {
    const variants: Array<'dark' | 'light' | 'primary' | 'success' | 'warning' | 'danger'> = [
    ...
    for (const variant of variants) {
      ...
      await waitFor(() => {
        const tooltip = screen.getByText('Tooltip text');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip.closest('[data-variant]')).toHaveAttribute('data-variant', variant);
      });
  ...
  it('supports different sizes', async () => {
    ...
    for (const size of sizes) {
      ...
      await waitFor(() => {
        const tooltip = screen.getByText('Tooltip text');
        expect(tooltip).toBeInTheDocument();
        expect(tooltip.closest('[data-size]')).toHaveAttribute('data-size', size);
      });
  ...
  it('renders with arrow', async () => {
    ...
    await waitFor(() => {
      const tooltip = screen.getByText('Tooltip text').closest('[data-testid="tooltip"]');
      expect(tooltip?.querySelector('[data-testid="tooltip-arrow"]')).toBeInTheDocument();
    });
  });
  ...
  it('renders with custom className', async () => {
    ...
    await waitFor(() => {
      const tooltip = screen.getByText('Tooltip text').closest('.custom-tooltip');
      expect(tooltip).toBeInTheDocument();
    });
  });

  it('supports JSX content', async () => {
    const content = <div><strong>Bold</strong> tooltip</div>;
