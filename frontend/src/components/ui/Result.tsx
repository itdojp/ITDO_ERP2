...
const Result: React.FC<ResultProps> = ({
  ...
  const getDefaultContent = () => {
    const contentMap = {
      ...
      warning: {
        title: "Warning",
        subTitle: "Please check the following warnings.",
      },
      "404": {
        title: "404",
  ...
  const renderIcon = () => {
    ...
    const iconStyle =
      typeof getStatusColor() === "object" ? getStatusColor() : {};
  ...
  const renderContent = () => {
    ...
    const displaySubTitle =
      subTitle || (showDetails ? defaultContent.subTitle : subTitle);
    ...
    return (
      <div className={layout === "horizontal" ? "ml-4" : "mt-4"}>
        ...
        {helpText && (
          <div className="text-sm text-gray-500 mt-4">{helpText}</div>
        )}
      </div>
    );
  };
