...
describe('Stepper', () => {
  ...
  it('renders with different sizes', () => {
    ...
    sizes.forEach(size => {
      const { unmount } = render(<Stepper steps={basicSteps} current={0} size={size} />);
      expect(screen.getByText('Step 1')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('supports custom className', () => {
    render(<Stepper steps={basicSteps} current={0} className="custom-stepper" />);
    expect(screen.getByText('Step 1')).toBeInTheDocument();
  });
  ...
  it('supports custom step renderer', () => {
    const customRenderer = (step: any, index: number) => (
      <div data-testid={`custom-step-${index}`}>
        Custom: {step.title}
      </div>
    );
