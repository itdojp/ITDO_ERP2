...
describe('Sidebar', () => {
  const mockSidebarItems = [
    { id: '1', label: 'Dashboard', icon: '📊', href: '/dashboard', current: true },
    { id: '2', label: 'Users', icon: '👥', href: '/users' },
    { id: '3', label: 'Settings', icon: '⚙️', href: '/settings', children: [
  ...
  it('supports different positions', () => {
    ...
    positions.forEach(position => {
      const { unmount } = render(<Sidebar items={mockSidebarItems} position={position} />);
      const sidebar = screen.getByTestId('sidebar-container');
  ...
  it('supports different sizes', () => {
    ...
    sizes.forEach(size => {
      const { unmount } = render(<Sidebar items={mockSidebarItems} size={size} />);
  ...
  it('supports different variants', () => {
    ...
    variants.forEach(variant => {
      const { unmount } = render(<Sidebar items={mockSidebarItems} variant={variant} />);
      expect(screen.getByTestId('sidebar-container')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('handles backdrop click to close', () => {
    const onClose = vi.fn();
    render(<Sidebar items={mockSidebarItems} overlay showBackdrop onClose={onClose} />);
  ...
  it('supports custom loading component', () => {
    const LoadingComponent = () => <div data-testid="custom-loading">Loading sidebar...</div>;
    render(<Sidebar items={mockSidebarItems} loading loadingComponent={<LoadingComponent />} />);
  ...
  it('supports themes', () => {
    ...
    themes.forEach(theme => {
      const { unmount } = render(<Sidebar items={mockSidebarItems} theme={theme} />);
      const sidebar = screen.getByTestId('sidebar-container');
  ...
  it('supports toggle callbacks', () => {
    ...
    render(<Sidebar items={mockSidebarItems} collapsible onToggle={onToggle} />);
  ...
  it('supports custom className', () => {
    ...
    expect(screen.getByTestId('sidebar-container')).toHaveClass('custom-sidebar');
  });

  it('supports tooltip in collapsed mode', () => {
    render(<Sidebar items={mockSidebarItems} collapsible collapsed showTooltips />);
  ...
  it('supports custom data attributes', () => {
    render(<Sidebar items={mockSidebarItems} data-category="navigation" data-id="main-sidebar" />);
  ...
  it('supports auto-collapse on item click', () => {
    const onToggle = vi.fn();
    render(<Sidebar items={mockSidebarItems} overlay autoClose onToggle={onToggle} />);
