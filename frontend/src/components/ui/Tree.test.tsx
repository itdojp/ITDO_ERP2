...
describe('Tree', () => {
    ...
    },
    {
      ...
      children: [
        { 
          ...
          title: 'Child 2-1',
          children: [
            { key: '2-1-1', title: 'Grandchild 2-1-1' }
          ]
        }
      ]
    },
  ...
  it('supports multiple selection', () => {
    const onSelect = vi.fn();
    render(<Tree treeData={basicTreeData} onSelect={onSelect} selectable multiple />);
  ...
  it('supports line style customization', () => {
    ...
    lineStyles.forEach(style => {
      const { unmount } = render(<Tree treeData={basicTreeData} showLine={{ style }} />);
      expect(screen.getByText('Parent 1')).toBeInTheDocument();
      unmount();
    });
  });
  ...
  it('handles node loading states', async () => {
    const loadingTreeData = [
      { key: '1', title: 'Parent', loading: true }
    ];
