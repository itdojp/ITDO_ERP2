      ...
      <div className="p-6">
        {activeTab === 'profile' && (
          <div className="space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-sm text-gray-900">{profileData.phone || '未設定'}</p>
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    {isEditing ? (
                      ...
                      <p className="text-sm text-gray-900">{profileData.address || '未設定'}</p>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-900">{formatDate(profileData.joinedDate)}</p>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">最終ログイン</label>
                    <p className="text-sm text-gray-900">{formatLastLogin(profileData.lastLogin)}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">自己紹介</h3>
              {isEditing ? (
                <textarea
                ...
                <p className="text-sm text-gray-700">{profileData.bio || '自己紹介が設定されていません。'}</p>
              )}
            </div>
          </div>
        )}
            ...
            <div>
              ...
              <div className="flex items-center justify-between p-4 border border-gray-200 rounded-lg">
                <div>
                  ...
                  <p className="text-sm text-gray-500">ログイン時にSMSで認証コードを受信</p>
                </div>
        ...
        {activeTab === 'stats' && (
          <div className="space-y-6">
            <div>
              <h3 className="text-lg font-medium text-gray-900 mb-4">利用統計</h3>
              <div className="grid grid-cols-2 gap-6">
                <div className="bg-blue-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-blue-600">{profileData.stats.totalLogins}</div>
                  <div className="text-sm text-gray-600">総ログイン回数</div>
                </div>
                <div className="bg-green-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-green-600">{profileData.stats.tasksCompleted}</div>
                  <div className="text-sm text-gray-600">完了タスク数</div>
                </div>
                <div className="bg-purple-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-purple-600">{profileData.stats.documentsCreated}</div>
                  <div className="text-sm text-gray-600">作成文書数</div>
                </div>
                <div className="bg-orange-50 p-4 rounded-lg">
                  <div className="text-2xl font-bold text-orange-600">{profileData.stats.lastActiveHours}</div>
                  <div className="text-sm text-gray-600">最終活動時間（時間前）</div>
                </div>
              </div>
            </div>
          </div>
